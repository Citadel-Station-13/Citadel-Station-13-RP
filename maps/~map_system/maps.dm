...
/datum/map
	...
	var/list/persist_levels = list() // Z-levels where SSpersistence should persist between rounds. Defaults to GLOB.using_map.station_levels if unset.
	var/list/secret_levels = list() // Z-levels that (non-admin) ghosts can't get to
	var/list/hidden_levels = list() // Z-levels who's contents are hidden, but not forbidden (gateways)
	var/list/empty_levels = list()   // Empty Z-levels that may be used for various things
	var/list/mappable_levels = list()// List of levels where mapping or other similar devices might work fully
	var/list/below_blocked_levels = list()// List of levels where mapping or other similar devices might work fully
	// End Static Lists

	// Z-levels available to various consoles, such as the crew monitor. Defaults to GLOB.using_map.station_levels if unset.
	var/list/map_levels
	...
	var/list/lavaland_levels_z = list()
...
/datum/map/New()
	...
	if(zlevel_datum_type)
		for(var/type in subtypesof(zlevel_datum_type))
			new type(src)
	if(!map_levels?.len)
		map_levels = GLOB.using_map.station_levels.Copy()
	if(!mappable_levels?.len)
		mappable_levels = GLOB.using_map.station_levels.Copy()
	if(!persist_levels?.len)
		persist_levels = GLOB.using_map.station_levels.Copy()
	if(!mappable_levels?.len)
		mappable_levels = GLOB.using_map.station_levels.Copy()
...
/datum/map/proc/get_zlevel_time(var/z)
	...
	// We have to invent a time
		...
		var/datum/time/T = new (station_time_in_ticks)
		return T
...
/datum/map/proc/get_map_levels(var/srcz, var/long_range = FALSE, var/om_range = -1)
	...
	//Traditional behavior, if not in an overmap sector
		...
		else if (srcz in GLOB.using_map.station_levels)
			return GLOB.using_map.station_levels.Copy()
