...
/obj/effect/step_trigger/teleporter/to_underdark/Initialize()
	...
	teleport_x = x
	teleport_y = y
	for(var/z_num in SSmapping.legacy_map_config.zlevels)
		var/datum/map_z_level/Z = SSmapping.legacy_map_config.zlevels[z_num]
		if(Z.name == "Underdark")
			teleport_z = Z.z
...
/obj/effect/step_trigger/lost_in_space/bluespace/Trigger(A)
	...
	if(ismob(A) && prob(5))//lucky day
		var/destturf = locate(rand(5,world.maxx-5),rand(5,world.maxy-5),pick(SSmapping.legacy_map_config.station_levels))
