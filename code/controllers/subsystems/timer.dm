...
/datum/controller/subsystem/timer/fire(resumed = FALSE)
	...
	if(lit && lit < last_check && head_offset < last_check && last_invoke_warning < last_check)
		...
		subsystem_log("Timer bucket reset. world.time: [world.time], head_offset: [head_offset], practical_offset: [practical_offset]")
		for (var/i in 1 to length(bucket_list))
			var/datum/timedevent/bucket_head = bucket_list[i]
			if (!bucket_head)
				continue

			subsystem_log("Active timers at index [i]:")

			var/datum/timedevent/bucket_node = bucket_head
			var/anti_loop_check = 1000
			do
				subsystem_log(get_timer_debug_string(bucket_node))
				bucket_node = bucket_node.next
				anti_loop_check--
			while(bucket_node && bucket_node != bucket_head && anti_loop_check)
		subsystem_log("Active timers in the second_queue queue:")
