...
#define BUCKET_POS(timer) ((round(DS2TICKS(timer.timeToRun - timer.timer_subsystem.head_offset)) % BUCKET_LEN) + 1)
// #define BUCKET_POS(timer) (((round((timer.timeToRun - timer.timer_subsystem.head_offset) / world.tick_lag)+1) % BUCKET_LEN)||BUCKET_LEN) - OLD FROM /tg/
/// Gets the maximum time at which timers will be invoked from buckets, used for deferring to secondary queue.
#define TIMER_MAX(timer_ss) (timer_ss.head_offset + (BUCKET_AMOUNT_SECONDS SECONDS) - world.tick_lag + TICKS2DS(timer_ss.practical_offset - 1))
...
// #define TIMER_MAX (world.time + TICKS2DS(min(BUCKET_LEN-(timer_ss.practical_offset-DS2TICKS(world.time - timer_ss.head_offset))-1, BUCKET_LEN-1))) - OLD FROM /tg/
