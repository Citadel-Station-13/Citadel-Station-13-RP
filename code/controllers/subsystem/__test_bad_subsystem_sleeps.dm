...
/datum/controller/subsystem/__test_bad_subsystem_sleeps
...
/datum/controller/subsystem/__test_bad_subsystem_sleeps/fire(resumed)
	if(first_fire)
		...
		mc_init_stage_at_start_of_sleep = Master.init_stage_completed
		sleep(10 SECONDS)
		is_currently_sleeping = FALSE
		should_resume = TRUE
		pause()
		if(mc_init_stage_at_start_of_sleep != Master.init_stage_completed)
