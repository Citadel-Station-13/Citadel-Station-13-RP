...
/datum/controller/subsystem/dbcore/stat_entry()
	return "P:[length(all_queries)]|Active:[length(queries_active)]|Standby:[length(queries_standby)]"
...
/datum/controller/subsystem/dbcore/proc/Connect()
	...
	if (.)
		...
		// cit edit: inform subsys that we got reconencted
...
/datum/controller/subsystem/dbcore/proc/InitializeRound()
	...
	GLOB.round_number = text2num(GLOB.round_id) // legacy
 ...
 * Directly runs SQL strings asynchronously, and only returns when they're done or if they overtimed
...
/datum/controller/subsystem/dbcore/proc/dangerously_block_on_multiple_unsanitized_queries(list/query_strings, timeout = 10 SECONDS)
	...
	for(var/datum/db_query/query as anything in queries)
		UNTIL(query.status < DB_QUERY_FINISHED || world.time > expire_at)
