...
SUBSYSTEM_DEF(points_of_interest)
	...
	var/list/obj/singularity/narsie/narsies = list()
...
/datum/controller/subsystem/points_of_interest/proc/on_poi_element_added(atom/new_poi)
	...
	if(ismob(new_poi))
		...
		BINARY_INSERT_PROC_COMPARE(new_poi_datum, mob_points_of_interest, /datum/point_of_interest/mob_poi, new_poi_datum, I_compare_to, COMPARE_KEY)
		points_of_interest_by_target_ref[REF(new_poi)] = new_poi_datum
	else
		new_poi_datum = new /datum/point_of_interest(new_poi)
		BINARY_INSERT_PROC_COMPARE(new_poi_datum, other_points_of_interest, /datum/point_of_interest, new_poi_datum, I_compare_to, COMPARE_KEY)
		points_of_interest_by_target_ref[REF(new_poi)] = new_poi_datum
		...
		// NAR'SIE HELPER
		else if(istype(new_poi, /obj/singularity/narsie))
			narsies += new_poi
...
/datum/controller/subsystem/points_of_interest/proc/on_poi_element_removed(atom/old_poi)
	...
	if(ismob(old_poi))
		...
		if(istype(old_poi, /obj/item/disk/nuclear))
			...
			real_nuclear_disks -= nuke_disk
		// NAR'SIE HELPER
		else if(istype(old_poi, /obj/singularity/narsie))
			narsies -= old_poi
...
/datum/point_of_interest/proc/I_compare_to(datum/point_of_interest/rhs)
...
/datum/point_of_interest/mob_poi/I_compare_to(datum/point_of_interest/mob_poi/rhs)
