...
/**
 * The multiplication factor for openturf shadower darkness.
 * Lighting will be multiplied by this.
 */
...
/datum/controller/subsystem/zmimic/Initialize(timeofday)
	admin_notice(SPAN_MINORANNOUNCE("Initializing Z-Mimic."), R_DEBUG)
	return ..()

/// For admin proc-call
/datum/controller/subsystem/zmimic/proc/update_all()
#warn Consider adding shitcode
...
// disable()
	...
	var/turf/T // Putting the declaration up here totally speeds it up, right? // Right??
	var/num_upd = 0
	var/num_del = 0
	var/num_amupd = 0
