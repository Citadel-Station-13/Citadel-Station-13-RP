...
/datum/controller/subsystem/characters/proc/save_character(datum/character_data/char, mob/persisting)
	...
	if(char.character_id)
		...
		var/datum/db_query/update_query = SSdbcore.NewQuery(
			"UPDATE [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("character")] \
		...
		var/datum/db_query/insert_query = SSdbcore.NewQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("character")] \
...
/datum/controller/subsystem/characters/proc/load_character(name, playerid, character_type)
	...
	var/datum/db_query/load_query = SSdbcore.NewQuery(
		"SELECT `id` FROM \
		[fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("character")] WHERE playerid = :id AND canonical_name = :name AND character_type = :type",
		list(
			"id" = playerid,
			"canonical_name" = name,
			"type" = character_type,
...
/datum/controller/subsystem/characters/proc/fetch_character(id, force = FALSE)
	...
	var/datum/db_query/load_query = SSdbcore.NewQuery(
		...
		[fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("character")] WHERE id = :id",
		list(
			"id" = id,
		)
	)
...
/datum/controller/subsystem/characters/proc/query_characters(playerid, fetch = FALSE, force = FALSE)
	...
	var/datum/db_query/iteration_query = SSdbcore.ExecuteQuery(
		"SELECT id FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("character")] WHERE playerid = :id",
		list(
			"id" = playerid
		)
	)
...
/datum/controller/subsystem/characters/proc/character_played(id)
	...
	var/datum/db_query/mark_query = SSdbcore.ExecuteQuery(
		"UPDATE [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("character")] SET last_played = NOW() WHERE id = :id",
		list(
			"id" = id
		)
	)
