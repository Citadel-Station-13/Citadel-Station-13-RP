...
/datum/controller/occupations/proc/AssignRole(mob/new_player/player, rank, latejoin = FALSE)
...
/datum/controller/occupations/proc/FreeRole(rank)	//making additional slot on the fly
	var/datum/job/job = GetJob(rank)
...
/datum/controller/occupations/proc/EquipRank(mob/living/carbon/human/H, rank, joined_late = FALSE)
	if(!H)
		return null
	...
	if(job.req_admin_notify)
		to_chat(H, SPAN_BOLDANNOUNCE("You are playing a job that is important for Game Progression. If you have to disconnect, please notify the admins via adminhelp."))
