 ...
 */
/datum/controller/subsystem/pathfinder/proc/get_path_jps(atom/movable/actor = GLOB.generic_pathfinding_actor, turf/goal, turf/start = get_turf(actor), target_distance = 1, max_path_length = 128, slack)
	var/datum/pathfinding/jps/instance = new(actor, start, goal, target_distance, max_path_length, slack)
	return run_pathfinding(instance)

/**
 * Please see [code/__HELPERS/pathfinding/astar.dm] for details on what JPS does/is.
 */
/datum/controller/subsystem/pathfinder/proc/get_path_astar(atom/movable/actor = GLOB.generic_pathfinding_actor, turf/goal, turf/start = get_turf(actor), target_distance = 1, max_path_length = 128, slack)
	var/datum/pathfinding/astar/instance = new(actor, start, goal, target_distance, max_path_length, slack)
	return run_pathfinding(instance)

/datum/controller/subsystem/pathfinder/proc/default_ai_pathfinding(datum/ai_holder/polaris/holder, turf/goal, min_dist = 1, max_path = 128)
