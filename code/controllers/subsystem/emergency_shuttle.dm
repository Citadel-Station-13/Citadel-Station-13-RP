...
/datum/controller/subsystem/emergencyshuttle/proc/shuttle_arrived()
	if (!shuttle.location)	//at station
		if (autopilot)
			...
			if (evac)
				emergency_shuttle_docked.Announce(replacetext(replacetext(using_map_legacy().emergency_shuttle_docked_message, "%dock_name%", "[using_map_legacy().dock_name]"),  "%ETD%", "[estimated_time] minute\s"))
			else
				priority_announcement.Announce(replacetext(replacetext(using_map_legacy().shuttle_docked_message, "%dock_name%", "[using_map_legacy().dock_name]"),  "%ETD%", "[estimated_time] minute\s"))
...
/datum/controller/subsystem/emergencyshuttle/proc/call_evac()
	...
	evac = 1
	emergency_shuttle_called.Announce(replacetext(using_map_legacy().emergency_shuttle_called_message, "%ETA%", "[estimated_time] minute\s"))
...
/datum/controller/subsystem/emergencyshuttle/proc/call_transfer()
	...
	priority_announcement.Announce(replacetext(replacetext(using_map_legacy().shuttle_called_message, "%dock_name%", "[using_map_legacy().dock_name]"),  "%ETA%", "[estimated_time] minute\s"))
...
/datum/controller/subsystem/emergencyshuttle/proc/recall()
	...
	if (evac)
		emergency_shuttle_recalled.Announce(using_map_legacy().emergency_shuttle_recall_message)
		...
		priority_announcement.Announce(using_map_legacy().shuttle_recall_message)
