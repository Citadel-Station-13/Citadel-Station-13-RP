...
/datum/controller/subsystem/garbage/Shutdown()
	...
	for(var/path in items)
		...
		if (I.hard_deletes)
			...
			dellog += "\tTime Spent Hard Deleting (ms): [I.hard_delete_time]ms"
			dellog += "\tHighest Time Spend Hard Deleting (ms): [I.hard_delete_max]ms"
...
#define REFS_WE_EXPECT 2
	...
	var/current_index = length(queue)
	while(current_index > 0)
		if(MC_TICK_CHECK)
			break

		var/list/queue_entry = queue[current_index]
