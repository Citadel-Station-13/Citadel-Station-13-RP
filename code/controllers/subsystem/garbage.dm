...
/datum/controller/subsystem/garbage/Shutdown()
	...
	for(var/path in items)
		...
		if (I.failures)
			del_log += "\tFailures: [I.failures]"
		del_log += "\tqdel() Count: [I.qdels]"
		del_log += "\tDestroy() Cost: [I.destroy_time]ms"

		if (I.hard_deletes)
			del_log += "\tTotal Hard Deletes [I.hard_deletes]"
			del_log += "\tTime Spent Hard Deleting: [I.hard_delete_time]ms"
			del_log += "\tHighest Time Spend Hard Deleting: [I.hard_delete_max]ms"
			if (I.hard_deletes_over_threshold)
				del_log += "\tHard Deletes Over Threshold: [I.hard_deletes_over_threshold]"
		if (I.slept_destroy)
			del_log += "\tTotal Sleeps: [I.slept_destroy]"
		if (I.no_respect_force)
			del_log += "\tTotal Ignored force: [I.no_respect_force]"
		if (I.no_hint)
			del_log += "\tTotal No hint: [I.no_hint]"
		if(LAZYLEN(I.extra_details))
			del_log += "\tDeleted Metadata: [json_encode(I.extra_details)]"

	log_qdel("", del_log.Join("\n"))
...
#define IS_DELETED(datum, gcd_at_time) (isnull(##datum) || ##datum.gc_destroyed != gcd_at_time)
	...
	for (var/i in 1 to length(queue))
		...
		if (IS_DELETED(D, queued_at_time))
			++gcedlasttick
			++totalgcs
			pass_counts[level]++
			#ifdef REFERENCE_TRACKING
