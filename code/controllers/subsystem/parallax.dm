...
/datum/controller/subsystem/parallax/proc/get_parallax_vis_contents(z)
	. = list()
	// overmaps
	var/obj/overmap/entity/visitable/v = SSovermaps.get_enclosing_overmap_entity(z)
	if(istype(v))
...
/datum/controller/subsystem/parallax/proc/get_parallax_motion(z)
	...
	var/obj/overmap/entity/visitable/ship/V = SSovermaps.get_enclosing_overmap_entity(z)
	if(!istype(V))
		return
	if(!V.is_moving())
		return
