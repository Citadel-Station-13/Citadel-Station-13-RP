...
/datum/controller/master
	...
	// skip_ticks would be here but we dont use it for bombs
...
/datum/controller/master/New()
	...
	if (Master != src)
		if (istype(Master)) //If there is an existing MC take over his stuff and delete it
			...
			tim_sort(subsystem_types, GLOBAL_PROC_REF(cmp_subsystem_init))
...
/datum/controller/master/proc/Loop(init_stage)
	...
	for (var/thing in subsystems)
		...
		SS.queue_prev = null
		SS.state = SS_IDLE
		// CIT: Set precomputed variables
		SS.recompute_wait_dt()

		if ((SS.subsystem_flags & (SS_TICKER|SS_BACKGROUND)) == SS_TICKER)
			tickersubsystems += SS
	...
	tim_sort(tickersubsystems, GLOBAL_PROC_REF(cmp_subsystem_priority))
	...
	while (1)
		...
		if (CheckQueue(subsystems_to_check) <= 0 || CheckQueue(tickersubsystems) <= 0) //error processing queue
...
/datum/controller/master/proc/AttemptProfileDump(delay)
	...
	// snippet of /datum/controller/subsystem/profiler/proc/DumpFile(allow_yield)
