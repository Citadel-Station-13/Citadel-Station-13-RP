proc/sql_poll_population()
	...
	if(!dbcon.IsConnected())
		...
		var/datum/db_query/query = dbcon_old.NewQuery("INSERT INTO `tgstation`.`population` (`playercount`, `admincount`, `time`) VALUES ([playercount], [admincount], '[sqltime]')")
		if(!query.Execute())
...
proc/sql_commit_feedback()
	...
	if(!dbcon.IsConnected())
		...
		var/datum/db_query/max_query = dbcon.NewQuery("SELECT MAX(roundid) AS max_round_id FROM erro_feedback")
		max_query.Execute()
		...
		for(var/datum/feedback_variable/item in content)
			...
			var/datum/db_query/query = dbcon.NewQuery("INSERT INTO erro_feedback (id, roundid, time, variable, value) VALUES (null, [newroundid], Now(), '[variable]', '[value]')")
			if(!query.Execute())
				var/err = query.ErrorMsg()
				log_game("SQL ERROR during death reporting. Error : \[[err]\]\n")
