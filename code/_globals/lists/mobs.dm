...
/proc/all_hair_styles()
	...
	for(var/path in subtypesof(/datum/sprite_accessory/hair))
		...
		.[S.id] = S
		by_name[S.name] = S
	tim_sort(by_name, GLOBAL_PROC_REF(text_asc), associative = FALSE)
	GLOB.legacy_hair_lookup = by_name
	tim_sort(., GLOBAL_PROC_REF(name_asc), associative = TRUE)
...
/proc/all_ear_styles()
	...
	for(var/path in subtypesof(/datum/sprite_accessory/ears))
		...
		.[S.id] = S
		by_type[S.type] = S
	tim_sort(by_type, GLOBAL_PROC_REF(name_asc), associative = TRUE)
