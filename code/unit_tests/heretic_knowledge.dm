...
/datum/unit_test/heretic_knowledge/Run()
	...
	var/list/blacklist = list(/datum/prototype/eldritch_knowledge/spell,/datum/prototype/eldritch_knowledge/curse,/datum/prototype/eldritch_knowledge/final,/datum/prototype/eldritch_knowledge/summon)
	var/list/all_possible_knowledge = subtypesof(/datum/prototype/eldritch_knowledge) - blacklist

	var/list/list_to_check = GLOB.heretic_start_knowledge.Copy()
	var/i = 0
	while(i < length(list_to_check))
		var/datum/prototype/eldritch_knowledge/eldritch_knowledge = allocate(list_to_check[++i])
	...
	if(length(all_possible_knowledge) != length(all_possible_knowledge & list_to_check))
		...
		for(var/X in unreachables)
			var/datum/prototype/eldritch_knowledge/eldritch_knowledge = X
