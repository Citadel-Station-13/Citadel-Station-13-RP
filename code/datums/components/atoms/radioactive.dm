...
/datum/component/radiation_emitter
...
/datum/component/radiation_emitter/Initialize(_strength=0, _half_life = RAD_HALF_LIFE_DEFAULT, _can_contaminate = TRUE, falloff)
	strength = _strength
	...
	if(istype(parent, /atom))
		...
		CRASH("Something that wasn't an atom was given /datum/component/radiation_emitter")
...
/datum/component/radiation_emitter/Destroy()
	SSradiation.sources -= src
	var/atom/movable/master = parent
	master.remove_filter("rad_glow")
	return ..()

/datum/component/radiation_emitter/proc/emit(ds)
	if(!prob(50))
		return
	if(!hl3_release_date)
...
/datum/component/radiation_emitter/proc/check_dissipate()
	if(strength <= RAD_BACKGROUND_RADIATION)
		qdel(src)
		return
...
/datum/component/radiation_emitter/proc/glow_loop(atom/movable/master)
	var/filter = master.get_filter("rad_glow")
	if(filter)
		animate(filter, alpha = 75, time = 15, loop = -1)
		animate(alpha = 25, time = 25)

/datum/component/radiation_emitter/InheritComponent(datum/component/C, i_am_original, _strength, _source, _half_life, _can_contaminate)
	if(!i_am_original)
		return
	if(C)
		var/datum/component/radiation_emitter/other = C
 ...
 */
/datum/component/radiation_emitter/proc/constructive_interference(limit, amt)
...
/datum/component/radiation_emitter/proc/rad_examine(datum/source, mob/user, list/examine_list)
...
/datum/component/radiation_emitter/proc/rad_attack(datum/source, atom/movable/target, mob/living/user)
	emit(1 SECONDS)

/datum/component/radiation_emitter/proc/clean(str, mul)
