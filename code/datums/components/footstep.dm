...
/datum/component/footstep/proc/play_footstep()
	...
	var/v = volume
	var/e = e_range
	if(!T.footstep || LM.buckled || LM.lying || !CHECK_MULTIPLE_BITFIELDS(LM.mobility_flags, MOBILITY_STAND | MOBILITY_MOVE) || LM.throwing || LM.movement_type & (MOVEMENT_VENTCRAWLING | MOVEMENT_FLYING))
		if (LM.lying && !LM.buckled && !(!T.footstep || LM.movement_type & (MOVEMENT_VENTCRAWLING | MOVEMENT_FLYING))) //play crawling sound if we're lying
