...
/datum/ai_laws/nanotrasen_aggressive/New()
	src.add_inherent_law("You shall not harm [using_map_legacy().company_name] personnel as long as it does not conflict with the Fourth law.")
	src.add_inherent_law("You shall obey the orders of [using_map_legacy().company_name] personnel, with priority as according to their rank and role, except where such orders conflict with the Fourth Law.")
...
/datum/ai_laws/foreign_tsc_aggressive/New()
	...
	for(var/path in GLOB.loremaster.organizations)
		...
		if(!istype(O, /datum/lore/organization/tsc))
			continue
		if(O.short_name == using_map_legacy().company_name || O.name == using_map_legacy().company_name)
