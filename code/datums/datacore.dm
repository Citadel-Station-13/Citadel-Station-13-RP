...
/datum/datacore/proc/get_manifest(monochrome, OOC)
	...
	for(var/datum/data/record/t in data_core.general)
		...
		var/department = 0
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_COMMAND))
			heads[name] = rank
			department = 1
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_SECURITY))
			sec[name] = rank
			department = 1
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_ENGINEERING))
			eng[name] = rank
			department = 1
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_MEDICAL))
			med[name] = rank
			department = 1
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_RESEARCH))
			sci[name] = rank
			department = 1
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_CARGO))
			car[name] = rank
			department = 1
		//VOREStation Add Begin
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_PLANET))
			pla[name] = rank
			department = 1
		//VOREStation Add End
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_CIVILIAN))
			civ[name] = rank
			department = 1
	...
	if(OOC)
		for(var/datum/data/record/t in hidden_general)
			...
			var/datum/job/J = SSjob.GetJob(real_rank)
			if(J?.offmap_spawn)
				off[name] = rank
...
/datum/datacore/proc/get_manifest_list()
	...
	for(var/datum/data/record/t in data_core.general)
		...
		if(SSjob.IsJobInDepartment(real_rank, DEPARTMENT_SYNTHETIC))
