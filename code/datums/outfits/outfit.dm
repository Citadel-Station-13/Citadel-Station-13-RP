...
/datum/outfit/proc/equip_base(mob/living/carbon/human/H)
	...
	if(l_ear)
		H.equip_to_slot_or_del(new l_ear(H),SLOT_ID_LEFT_EAR)
	if(r_ear)
		H.equip_to_slot_or_del(new r_ear(H),SLOT_ID_RIGHT_EAR)
	if(glasses)
		H.equip_to_slot_or_del(new glasses(H),SLOT_ID_GLASSES)
	if(id)
		H.equip_to_slot_or_del(new id(H),SLOT_ID_WORN_ID)
	if(l_pocket)
		H.equip_to_slot_or_del(new l_pocket(H),SLOT_ID_LEFT_POCKET)
	if(r_pocket)
