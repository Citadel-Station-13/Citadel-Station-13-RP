...
/datum/outfit/proc/equip_base(mob/living/carbon/human/H)
	...
	if(uniform)
		H.equip_to_slot_or_del(new uniform(H),SLOT_ID_UNIFORM, ignore_fluff = TRUE, silent = TRUE)
	if(suit)
		H.equip_to_slot_or_del(new suit(H),SLOT_ID_SUIT, ignore_fluff = TRUE, silent = TRUE)
	if(back)
		H.equip_to_slot_or_del(new back(H),SLOT_ID_BACK, ignore_fluff = TRUE, silent = TRUE)
	if(belt)
		H.equip_to_slot_or_del(new belt(H),SLOT_ID_BELT, ignore_fluff = TRUE, silent = TRUE)
	if(gloves)
		H.equip_to_slot_or_del(new gloves(H),SLOT_ID_GLOVES, ignore_fluff = TRUE, silent = TRUE)
	if(shoes)
		H.equip_to_slot_or_del(new shoes(H),SLOT_ID_SHOES, ignore_fluff = TRUE, silent = TRUE)
	if(mask)
		H.equip_to_slot_or_del(new mask(H),SLOT_ID_MASK, ignore_fluff = TRUE, silent = TRUE)
	if(head)
		H.equip_to_slot_or_del(new head(H),SLOT_ID_HEAD, ignore_fluff = TRUE, silent = TRUE)
	if(l_ear)
		H.equip_to_slot_or_del(new l_ear(H),SLOT_ID_LEFT_EAR, ignore_fluff = TRUE, silent = TRUE)
	if(r_ear)
		H.equip_to_slot_or_del(new r_ear(H),SLOT_ID_RIGHT_EAR, ignore_fluff = TRUE, silent = TRUE)
	if(glasses)
		H.equip_to_slot_or_del(new glasses(H),SLOT_ID_GLASSES, ignore_fluff = TRUE, silent = TRUE)
	if(id)
		H.equip_to_slot_or_del(new id(H),SLOT_ID_WORN_ID, ignore_fluff = TRUE, silent = TRUE)
	if(l_pocket)
		H.equip_to_slot_or_del(new l_pocket(H),SLOT_ID_LEFT_POCKET, ignore_fluff = TRUE, silent = TRUE)
	if(r_pocket)
		H.equip_to_slot_or_del(new r_pocket(H),SLOT_ID_RIGHT_POCKET, ignore_fluff = TRUE, silent = TRUE)
	if(suit_store)
	...
	for(var/path in uniform_accessories)
		...
		for(var/i=0,i<number,i++)
			H.equip_to_slot_or_del(new path(H), /datum/inventory_slot_meta/abstract/attach_as_accessory, ignore_fluff = TRUE, silent = TRUE)
