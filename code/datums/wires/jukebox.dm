...
/datum/wires/jukebox/on_pulse(wire)
	...
	switch(wire)
		if(WIRE_MAIN_POWER1)
			holder.visible_message("[SPAN_NOTICE(icon2html(thing = holder, target = world))] The power light flickers.")
			A.shock(usr, 90)
		if(WIRE_JUKEBOX_HACK)
			holder.visible_message("[SPAN_NOTICE(icon2html(thing = holder, target = world))] The parental guidance light flickers.")
		if(WIRE_REVERSE)
			holder.visible_message("[SPAN_NOTICE(icon2html(thing = holder, target = world))] The data light blinks ominously.")
		if(WIRE_SPEEDUP)
			holder.visible_message("[SPAN_NOTICE(icon2html(thing = holder, target = world))] The speakers squeaks.")
		if(WIRE_SPEEDDOWN)
			holder.visible_message("[SPAN_NOTICE(icon2html(thing = holder, target = world))] The speakers rumble.")
		if(WIRE_START)
			A.StartPlaying()
		if(WIRE_STOP)
			A.StopPlaying()
