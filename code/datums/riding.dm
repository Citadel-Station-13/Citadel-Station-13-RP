...
/datum/riding/boat/handle_ride(mob/user, direction)
	...
	if(istype(next, /turf/simulated/floor/outdoors/water) || istype(current, /turf/simulated/floor/outdoors/water)) //We can move from land to water, or water to land, but not from land to land
		..()
	else
