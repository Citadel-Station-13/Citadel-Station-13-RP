...
#define ADD_STATPANEL_DATA_LINE(INTO_LIST, LINE) INTO_LIST += LINE
/// key, value
#define ADD_STATPANEL_DATA_ENTRY(INTO_LIST, KEY, VALUE) INTO_LIST += list(list(KEY, VALUE))
/// kev, value, target ref; will route to statpanel_click().
#define ADD_STATPANEL_DATA_CLICK(INTO_LIST, KEY, VALUE, REF) . += list(list(KEY, VALUE, REF))
