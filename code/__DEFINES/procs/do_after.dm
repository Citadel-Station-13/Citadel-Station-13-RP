...
#define START_INTERACTING_WITH(M, A, T) do {if(!M.interacting_with){M.interacting_with = list(A = list(T = 1));} else {var/list/_L = M.interacting_with; if(!_L[A]){_L[A] = list(T = 1)} else {++_L[A][T]}}; if(!A.interacting_mobs){A.interacting_mobs = list(M = 1)} else{++A.interacting_mobs[M]};} while(FALSE);
#define STOP_INTERACTING_WITH(M, A, T) do {if(M.interacting_with?[A]?[T]){--M.interacting_with[A][T]; if(M.interacting_with[A][T] <= 0){M.interacting_with[A] -= T}; if(!length(M.interacting_with[A])){M.interacting_with -= A};}; if(A.interacting_mobs){--A.interacting_mobs[M]; if(A.interacting_mobs[M] <= 0){A.interacting_mobs -= M};};} while(FALSE);
