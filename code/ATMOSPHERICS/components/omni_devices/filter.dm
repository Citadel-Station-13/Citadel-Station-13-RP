...
/obj/machinery/atmospherics/omni/atmos_filter/proc/mode_send_switch(var/mode = ATM_NONE)
	switch(mode)
		...
		if(ATM_P)
			return "spicy purple" //*cough* spicy purple *cough*
		if(ATM_N2O)
...
/obj/machinery/atmospherics/omni/atmos_filter/Topic(href, href_list)
	...
	if(configuring && !use_power)
		switch(href_list["command"])
			...
			if("switch_filter")
				var/new_filter = input(usr,"Select filter mode:","Change filter",href_list["mode"]) in list("None", "Oxygen", "Nitrogen", "Carbon Dioxide", "spicy purple", "Nitrous Oxide")
