...
/obj/item/integrated_circuit/input/mmi_tank/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	if(installed_brain)
...
/obj/item/integrated_circuit/input/pAI_connector/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	if(installed_pai)
