...
/obj/item/vehicle_component/equipment/tool/passenger/destroy()
	for(var/atom/movable/AM in src)
		AM.forceMove(get_turf(src))
		to_chat(AM, "<span class='danger'>You tumble out of the destroyed [src.name]!</span>")
	return ..()

/obj/item/vehicle_component/equipment/tool/passenger/Exit(atom/movable/O)
	return 0

/obj/item/vehicle_component/equipment/tool/passenger/proc/move_inside(var/mob/user)
	if (chassis)
...
/mob/proc/verb_eject_mech_passenger()
	...
	var/obj/item/vehicle_component/equipment/tool/passenger/pod = loc
	if(!istype(pod))
...
/obj/item/vehicle_component/equipment/tool/passenger/proc/go_out()
	if(!occupant_legacy)
		return
...
/obj/item/vehicle_component/equipment/tool/passenger/attach()
	..()
	if (chassis)
		add_obj_verb(chassis, /obj/vehicle/sealed/mecha/proc/move_inside_passenger)

/obj/item/vehicle_component/equipment/tool/passenger/detach()
	if(occupant_legacy)
	...
	..()
	if (M && !(locate(/obj/item/vehicle_component/equipment/tool/passenger) in M))
		remove_verb(M, /obj/vehicle/sealed/mecha/proc/move_inside_passenger)

/obj/item/vehicle_component/equipment/tool/passenger/get_equip_info()
	return "[..()] <br />[occupant_legacy? "\[Occupant: [occupant_legacy]\]|" : ""]Exterior Hatch: <a href='?src=\ref[src];toggle_lock=1'>Toggle Lock</a>"

/obj/item/vehicle_component/equipment/tool/passenger/Topic(href,href_list)
	..()
...
/obj/vehicle/sealed/mecha/proc/move_inside_passenger()
	...
	for(var/obj/item/vehicle_component/equipment/tool/passenger/P in src)
