...
/obj/item/vehicle_component/equipment/tool/jetpack/can_attach(obj/vehicle/sealed/mecha/M as obj)
	if(!(locate(src.type) in M.equipment) && !M.proc_res["dyndomove"])
		return ..()

/obj/item/vehicle_component/equipment/tool/jetpack/detach()
	..()
	chassis.proc_res["dyndomove"] = null
	return

/obj/item/vehicle_component/equipment/tool/jetpack/attach(obj/vehicle/sealed/mecha/M as obj)
	..()
	if(!ion_trail)
		ion_trail = new
...
/obj/item/vehicle_component/equipment/tool/jetpack/proc/toggle()
	if(!chassis)
		return
	!equip_ready? turn_off() : turn_on()
	return equip_ready

/obj/item/vehicle_component/equipment/tool/jetpack/proc/turn_on()
	set_ready_state(0)
...
/obj/item/vehicle_component/equipment/tool/jetpack/proc/turn_off()
	set_ready_state(1)
	chassis.proc_res["dyndomove"] = null
	ion_trail.stop()
...
/obj/item/vehicle_component/equipment/tool/jetpack/proc/dyndomove(direction)
	if(!action_checks())
...
/obj/item/vehicle_component/equipment/tool/jetpack/action_checks()
	if(equip_ready || wait)
		return 0
...
/obj/item/vehicle_component/equipment/tool/jetpack/get_equip_info()
	if(!chassis) return
	return "<span style=\"color:[equip_ready?"#0f0":"#f00"];\">*</span>&nbsp;[src.name] \[<a href=\"?src=\ref[src];toggle=1\">Toggle</a>\]"

/obj/item/vehicle_component/equipment/tool/jetpack/Topic(href,href_list)
	..()
	if(href_list["toggle"])
		toggle()

/obj/item/vehicle_component/equipment/tool/jetpack/do_after_cooldown()
