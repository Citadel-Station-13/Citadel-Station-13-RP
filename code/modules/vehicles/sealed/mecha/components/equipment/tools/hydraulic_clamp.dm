...
/obj/item/vehicle_component/equipment/tool/hydraulic_clamp/attach(obj/vehicle/sealed/mecha/M as obj)
	..()
	cargo_holder = M

	return

/obj/item/vehicle_component/equipment/tool/hydraulic_clamp/action(atom/target)
...
/obj/item/vehicle_component/equipment/tool/hydraulic_clamp/safety
	name = "\improper KILL CLAMP"
	equip_cooldown = 15
	energy_drain = 0
	dam_force = 0

/obj/item/vehicle_component/equipment/tool/hydraulic_clamp/safety/action(atom/target)
	if(!action_checks(target)) return
	if(!cargo_holder) return
	if(istype(target,/obj))
		var/obj/O = target
