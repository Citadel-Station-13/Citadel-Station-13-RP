...
/obj/item/vehicle_component/equipment/tool/rcd/Initialize(mapload)
	my_rcd = new(src)
	return ..()

/obj/item/vehicle_component/equipment/tool/rcd/Destroy()
	QDEL_NULL(my_rcd)
	return ..()

/obj/item/vehicle_component/equipment/tool/rcd/action(atom/target)
	if(!action_checks(target) || get_dist(chassis, target) > 3)
		return FALSE

	my_rcd.use_rcd(target, chassis.occupant_legacy)

/obj/item/vehicle_component/equipment/tool/rcd/Topic(href,href_list)
	..()
	if(href_list["mode"])
		my_rcd.mode_index = text2num(href_list["mode"])
		occupant_message("RCD reconfigured to '[my_rcd.modes[my_rcd.mode_index]]'.")
/*
/obj/item/vehicle_component/equipment/tool/rcd/get_equip_info()
