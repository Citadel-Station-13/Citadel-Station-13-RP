...
/obj/item/vehicle_component/module/tool/drill/action(atom/target)
		...
		if(T == chassis.loc && src == chassis.selected)
			if(istype(target, /turf/simulated/wall))
				...
				if(enable_special)
					...
					M1.GetDrilled()
				log_message("Drilled through [target]")
				if(locate(/obj/item/vehicle_component/module/tool/hydraulic_clamp) in chassis.equipment)
...
/obj/item/vehicle_component/module/tool/drill/proc/drill_mob(mob/living/target, mob/user)
...
/obj/item/vehicle_component/module/tool/drill/bore
	name = "depth bore"
...
/obj/item/vehicle_component/module/tool/drill/bore/action(atom/target)
	if(!action_checks(target))
		return
	set_ready_state(0)
	chassis.use_power(energy_drain)
