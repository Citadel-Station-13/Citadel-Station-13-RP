/obj/item/vehicle_module/lazy/legacy/crisis_drone
...
/obj/item/vehicle_module/lazy/legacy/crisis_drone/Initialize(mapload)
	. = ..()
	drone_overlay = new(src.icon, icon_state = droid_state)

/obj/item/vehicle_module/lazy/legacy/crisis_drone/Destroy()
	STOP_PROCESSING(SSobj, src)
	return ..()

/obj/item/vehicle_module/lazy/legacy/crisis_drone/attach(obj/vehicle/sealed/mecha/M as obj)
	. = ..(M)
	if(chassis)
		START_PROCESSING(SSobj, src)

/obj/item/vehicle_module/lazy/legacy/crisis_drone/detach(atom/moveto=null)
	shut_down()
	. = ..(moveto)
	STOP_PROCESSING(SSobj, src)

/obj/item/vehicle_module/lazy/legacy/crisis_drone/critfail()
	. = ..()
	STOP_PROCESSING(SSobj, src)
	shut_down()
...
/obj/item/vehicle_module/lazy/legacy/crisis_drone/process()
...
/obj/item/vehicle_module/lazy/legacy/crisis_drone/proc/valid_target(var/mob/living/L)
	. = TRUE
...
/obj/item/vehicle_module/lazy/legacy/crisis_drone/proc/shut_down()
	if(enabled)
...
/obj/item/vehicle_module/lazy/legacy/crisis_drone/proc/unique_patient_checks(var/mob/living/L)	// Anything special for subtypes. Does it only work on Robots? Fleshies? A species?
	. = TRUE

/obj/item/vehicle_module/lazy/legacy/crisis_drone/proc/heal_target(var/mob/living/L)	// We've done all our special checks, just get to fixing damage.
...
/obj/item/vehicle_module/lazy/legacy/crisis_drone/proc/toggle_drone()
	if(chassis)
		enabled = !enabled
		if(enabled)
			set_ready_state(0)
...
/obj/item/vehicle_module/lazy/legacy/crisis_drone/add_equip_overlay(obj/vehicle/sealed/mecha/M as obj)
	..()
	if(enabled)
		M.add_overlay(drone_overlay)
	return

/obj/item/vehicle_module/lazy/legacy/crisis_drone/Topic(href, href_list)
	..()
	if(href_list["toggle_drone"])
		toggle_drone()
	return

/obj/item/vehicle_module/lazy/legacy/crisis_drone/get_equip_info()
	if(!chassis) return
	return "<span style=\"color:[equip_ready?"#0f0":"#f00"];\">*</span>&nbsp;[src.name] - <a href='?src=\ref[src];toggle_drone=1'>[enabled?"Dea":"A"]ctivate</a>"

/obj/item/vehicle_module/lazy/legacy/crisis_drone/rad
