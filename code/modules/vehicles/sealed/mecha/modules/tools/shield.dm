...
/obj/item/vehicle_module/shield_projector/linear/Initialize(mapload)
	. = ..()
	my_shield = new my_shield_type
	my_shield.shield_regen_delay = equip_cooldown
	my_shield.my_tool = src

/obj/item/vehicle_module/shield_projector/linear/critfail()
	..()
	my_shield.adjust_health(-200)
	return

/obj/item/vehicle_module/shield_projector/linear/Destroy()
	if(!isnull(chassis))
...
/obj/item/vehicle_module/shield_projector/linear/add_equip_overlay(obj/vehicle/sealed/mecha/M)
	..()
	if(!drone_overlay)
...
/obj/item/vehicle_module/shield_projector/linear/Topic(href, href_list)
	..()
	if(href_list["toggle_shield"])
		toggle_shield()
	return

/obj/item/vehicle_module/shield_projector/linear/get_equip_info()
	if(!chassis) return
