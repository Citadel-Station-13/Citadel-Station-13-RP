...
/obj/item/vehicle_module/lazy/legacy/omni_shield/critfail()
	..()
	shields.adjust_health(-200)

/obj/item/vehicle_module/lazy/legacy/omni_shield/Destroy()
	QDEL_NULL(shields)
	..()

/obj/item/vehicle_module/lazy/legacy/omni_shield/attach(obj/vehicle/sealed/mecha/M as obj)
	. = ..()
	if(chassis)
		shields = new shield_type(chassis)

/obj/item/vehicle_module/lazy/legacy/omni_shield/detach()
	if(chassis)
		QDEL_NULL(shields)
	. = ..()

/obj/item/vehicle_module/lazy/legacy/omni_shield/handle_movement_action()
	if(chassis && shields)
		shields.update_shield_positions()

/obj/item/vehicle_module/lazy/legacy/omni_shield/proc/toggle_shield()
	if(shields)
...
/obj/item/vehicle_module/lazy/legacy/omni_shield/Topic(href, href_list)
	..()
	if(href_list["toggle_omnishield"])
		toggle_shield()

/obj/item/vehicle_module/lazy/legacy/omni_shield/get_equip_info()
	if(!chassis) return
