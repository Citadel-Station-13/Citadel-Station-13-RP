...
/obj/item/vehicle_module/lazy/legacy/cloak/Initialize(mapload)
	. = ..()
	cloak_iterator = new /datum/global_iterator/mecha_cloak(list(src),0)
	cloak_iterator.set_delay(equip_cooldown)

/obj/item/vehicle_module/lazy/legacy/cloak/Destroy()
	QDEL_NULL(cloak_iterator)
	return ..()

/obj/item/vehicle_module/lazy/legacy/cloak/detach()
	QDEL_NULL(cloak_iterator)
	if(!equip_ready) //We were running
		stop_cloak()
	return ..()

/obj/item/vehicle_module/lazy/legacy/cloak/get_equip_info()
	if(!chassis)
		return
	return "<span style=\"color:[equip_ready ? "#0f0":"#f00"];\">*</span>&nbsp;[src.name] - <a href='?src=\ref[src];toggle_cloak=1'>[equip_ready ? "A" : "Dea"]ctivate</a>"

/obj/item/vehicle_module/lazy/legacy/cloak/Topic(href, href_list)
	..()
...
/obj/item/vehicle_module/lazy/legacy/cloak/proc/start_cloak()
	if(chassis)
		chassis.cloak()
...
T
/obj/item/vehicle_module/lazy/legacy/cloak/proc/stop_cloak()
	if(chassis)
		chassis.uncloak()
	cloak_iterator.stop()
	set_ready_state(1)
...
/datum/global_iterator/mecha_cloak/process(var/obj/item/vehicle_module/lazy/legacy/cloak/cloak)
