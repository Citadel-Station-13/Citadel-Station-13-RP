...
/obj/vehicle/skateboard/Destroy()
	if(sparks)
		QDEL_NULL(sparks)
	. = ..()

/obj/vehicle/skateboard/load(var/atom/movable/C, var/mob/user as mob)
	var/mob/living/M = C
...
/obj/vehicle/skateboard/attack_hand(var/mob/user as mob)
	if(user == load)
...
/obj/vehicle/skateboard/relaymove(mob/user, direction)
...
/obj/vehicle/skateboard/Move(var/turf/destination, var/mob/living/H)
	if(istype(destination,/turf/space) || istype (destination,/turf/simulated/water) || istype(destination,/turf/simulated/outdoors))
		rough_terrain = TRUE
		return 1
	else
		rough_terrain = FALSE
...
/obj/vehicle/skateboard/post_buckle_mob(mob/living/carbon/M) //allows skateboards to be non-dense but still allows 2 skateboarders to collide with each other
	density = TRUE
	return ..()

/obj/vehicle/skateboard/unbuckle_mob(mob/living/carbon/M)
	. = ..()
	if(!has_buckled_mobs(M))
		density = FALSE
	return ..()

/obj/vehicle/skateboard/Bump(atom/A)
...
/obj/vehicle/skateboard/proc/grind()
	Move(dir)
...
/datum/action/vehicle/skateboard/ollie/Trigger()
	if(world.time > next_ollie)
		var/obj/vehicle/skateboard/V = vehicle_target
		if (V.grinding)
			return
		...
		if(locate(/obj/structure/table) in V.loc.contents)
			...
			addtimer(CALLBACK(V, /obj/vehicle/skateboard/.proc/grind), 2)
...
/obj/vehicle/skateboard/pro/Bump(atom/A)
	if(A.density && has_buckled_mobs())
		var/mob/living/H = buckled_mobs[1]
		playsound(src, 'sound/effects/bang.ogg', 40, TRUE)
...
/obj/vehicle/skateboard/hoverboard
	name = "hoverboard"
...
/obj/vehicle/skateboard/hoverboard/attackby(obj/item/I, mob/user, params)
...
/obj/vehicle/skateboard/hoverboard/Move(var/turf/destination, var/mob/living/H)
	if(istype(destination,/turf/space) || istype (destination,/turf/simulated/water) || istype(destination,/turf/simulated/outdoors))
		rough_terrain = FALSE
		return 0
	else
...
/obj/vehicle/skateboard/hoverboard/Bump(atom/A)
	if(A.density && has_buckled_mobs())
		var/mob/living/H = buckled_mobs[1]
		playsound(src, 'sound/effects/bang.ogg', 40, TRUE)
...
/obj/vehicle/skateboard/hoverboard/admin
...
/obj/vehicle/skateboard/hoverboard/admin/Bump(atom/A)
	if(A.density && has_buckled_mobs())
		var/mob/living/H = buckled_mobs[1]
		playsound(src, 'sound/effects/bang.ogg', 40, TRUE)
...
/obj/vehicle/skateboard/scooter
	name = "scooter"
...
/obj/vehicle/skateboard/scooter/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/tool/wrench))
		...
		var/obj/vehicle/skateboard/S = new(drop_location())
...
/obj/vehicle/skateboard/scooter/Moved()
	. = ..()
