...
/datum/artifact_effect/stun/DoEffectTouch(var/mob/toucher)
	if(toucher && iscarbon(toucher))
		...
		if(prob(susceptibility * 100))
			to_chat(C, "<span class='danger'>A powerful force overwhelms your consciousness.</span>")
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			...
			if(prob(10 * susceptibility))
				to_chat(C, "<span class='danger'>Your body goes numb for a moment.</span>")
				C.Weaken(2)
				C.stuttering += 2
				if(prob(10))
					C.Stun(1)
			else if(prob(10))
				to_chat(C, "<span class='danger'>You feel numb.</span>")
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			...
			if(prob(100 * susceptibility))
				to_chat(C, "<span class='danger'>A wave of energy overwhelms your senses!</span>")
