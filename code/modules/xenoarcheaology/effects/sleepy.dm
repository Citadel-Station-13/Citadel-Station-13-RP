...
/datum/artifact_effect/sleepy/DoEffectTouch(var/mob/toucher)
	if(toucher)
		...
		if(ishuman(toucher) && prob(weakness * 100))
			...
			H << pick("<span class='notice'>You feel like taking a nap.</span>","<span class='notice'> You feel a yawn coming on.</span>","<span class='notice'> You feel a little tired.</span>")
			...
			to_chat(toucher, "<span class='danger'>SYSTEM ALERT: CPU cycles slowing down.</span>")
			return 1
...
/datum/artifact_effect/sleepy/DoEffectAura()
	if(holder)
		...
		for (var/mob/living/silicon/robot/R in range(src.effectrange,holder))
			to_chat(R, "<span class='danger'>SYSTEM ALERT: CPU cycles slowing down.</span>")
		return 1
