...
/obj/machinery/artifact/process(delta_time)
	...
	if( (my_effect.trigger >= TRIGGER_HEAT && my_effect.trigger <= TRIGGER_NITRO) || (my_effect.trigger >= TRIGGER_HEAT && my_effect.trigger <= TRIGGER_NITRO) )
		...
		if(env)
			...
			if(env.gas[/datum/reagent/gas/phoron] >= 2)
				trigger_phoron = 1
			if(env.gas[/datum/reagent/gas/oxygen] >= 2)
				trigger_oxy = 1
			if(env.gas[/datum/reagent/gas/carbon_dioxide] >= 2)
				trigger_co2 = 1
			if(env.gas[/datum/reagent/gas/nitrogen] >= 2)
				trigger_nitro = 1
