...
/obj/machinery/auto_cloner/process(delta_time)
	if(powered(power_channel))
		if(!previous_power_state)
			...
			icon_state = "cellold1"
			visible_message(SPAN_NOTICE("[icon2html(thing = src, target = world)] [src] suddenly comes to life!"))

		//slowly grow a mob
		if(prob(5))
			visible_message(SPAN_NOTICE("[icon2html(thing = src, target = world)] [src] [pick("gloops","glugs","whirrs","whooshes","hisses","purrs","hums","gushes")]."))
		...
		if(previous_power_state)
			...
			icon_state = "cellold0"
			visible_message(SPAN_NOTICE("[icon2html(thing = src, target = world)] [src] suddenly shuts down."))
