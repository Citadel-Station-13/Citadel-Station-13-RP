/client/proc/update_lookup_in_db()
	...
	var/datum/db_query/lookup = SSdbcore.NewQuery(
		"SELECT id FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player_lookup")] WHERE ckey = :ckey",
		list(
			"ckey" = ckey,
		)
	)
	...
	if(sql_id)
		var/datum/db_query/update = SSdbcore.NewQuery(
			"UPDATE [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player_lookup")] SET lastseen = Now(), ip = :ip, computerid = :computerid, lastadminrank = :lastadminrank WHERE id = :id",
			list(
				"ip" = address,
		...
		var/datum/db_query/insert = SSdbcore.NewQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player_lookup")] (id, ckey, firstseen, lastseen, ip, computerid, lastadminrank) VALUES (null, :ckey, Now(), Now(), :ip, :cid, :rank)",
			list(
				"ckey" = ckey,
				"ip" = address,
				"cid" = computer_id,
...
/client/proc/log_connection_to_db_impl()
	...
	SSdbcore.RunQuery(
		"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("connection_log")] (id, datetime, serverip, ckey, ip, computerid) VALUES (null, Now(), :server, :ckey, :ip, :cid)",
		list(
