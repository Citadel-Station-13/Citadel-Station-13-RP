...
/datum/category_item/player_setup_item/general/equipment/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["change_backpack"])
		var/new_backbag = input(user, "Choose your character's style of bag:", CHARACTER_PREFERENCE_INPUT_TITLE, backbaglist[pref.backbag]) as null|anything in backbaglist
		if(!isnull(new_backbag) && CanUseTopic(user))
			pref.backbag = backbaglist.Find(new_backbag)
			return TOPIC_REFRESH_UPDATE_PREVIEW

	else if(href_list["change_pda"])
		var/new_pdachoice = input(user, "Choose your character's style of PDA:", CHARACTER_PREFERENCE_INPUT_TITLE, pdachoicelist[pref.pdachoice]) as null|anything in pdachoicelist
		...
		if(CanUseTopic(user))
			pref.ringtone = sanitize(input(user, "Please enter a new ringtone.", CHARACTER_PREFERENCE_INPUT_TITLE) as null|text, 20)
			return TOPIC_REFRESH
