...
/datum/player_data/proc/_load()
	...
	lookup = SSdbcore.ExecuteQuery(
		"SELECT id, playerid, firstseen FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player_lookup")] WHERE ckey = :ckey",
		list(
			"ckey" = ckey
		)
	)
	...
	if(lookup_pid)
		...
		qdel(lookup)
		lookup = SSdbcore.ExecuteQuery(
			"SELECT id, flags, datediff(Now(), firstseen), firstseen, misc FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player")] WHERE id = :id",
			list(
				"id" = lookup_pid
			)
		)
...
/datum/player_data/proc/register_new_player(migrate_firstseen, lookup_id)
	...
	if(migrate_firstseen)
		insert = SSdbcore.ExecuteQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player")] (flags, firstseen, lastseen, misc) VALUES (:flags, :fs, Now(), :misc)",
			list(
				"flags" = player_flags,
		...
		insert = SSdbcore.ExecuteQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player")] (flags, firstseen, lastseen, misc) VALUES (:flags, Now(), Now(), :misc)",
			list(
				"flags" = player_flags,
				"misc" = safe_json_encode(player_misc),
			)
	...
	// now update lookup
	insert = SSdbcore.ExecuteQuery(
		"UPDATE [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player_lookup")] SET playerid = :pid WHERE id = :id",
		list(
			"id" = lookup_id,
			"pid" = insert_id
		)
...
/datum/player_data/proc/_save()
	qdel(SSdbcore.ExecuteQuery(
		"UPDATE [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player")] SET flags = :flags, misc = :misc WHERE id = :id",
		list(
			"flags" = player_flags,
			"id" = player_id,
			"misc" = safe_json_encode(player_misc, "{}"),
...
/datum/player_data/proc/update_last_seen()
	...
	if(!block_on_available())
		return FALSE
	qdel(SSdbcore.ExecuteQuery(
		"UPDATE [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("player")] SET lastseen = Now() WHERE id = :id",
		list(
			"id" = player_id,
		)
	))
