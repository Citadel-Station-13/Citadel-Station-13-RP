...
/obj/machinery/computer/libraryconsole/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("search")
			if(!prevent_db_spam())
				atom_say("Database cables refreshing. Please wait a moment.")
				return
			INVOKE_ASYNC(src, PROC_REF(update_db_info))
			return TRUE
		if("switch_page")
...
/obj/machinery/computer/libraryconsole/proc/update_page_contents()
	...
	var/datum/db_query/query_library_list_books = SSdbcore.NewQuery({"
		...
		FROM [DB_PREFIX_TABLE_NAME("library")]
...
/obj/machinery/computer/libraryconsole/proc/update_page_count()
	...
	var/datum/db_query/query_library_count_books = SSdbcore.NewQuery({"
		SELECT COUNT(id) FROM [DB_PREFIX_TABLE_NAME("library")]
		WHERE isnull(deleted)
			AND author LIKE CONCAT('%',:author,'%')
			AND title LIKE CONCAT('%',:title,'%')
			AND (:category = 'Any' OR category = :category)
...
/obj/machinery/computer/libraryconsole/bookmanagement/ui_data(mob/user)
	...
	data["display_lore"] = ((obj_flags & OBJ_EMAGGED) && can_spawn_lore)
	...
	switch(screen_state)
		...
		if(LIBRARY_UPLOAD)
			...
			data["active_newscaster_cooldown"] = TRUE
		...
		if(LIBRARY_PRINT)
			data["deity"] = GLOB.deity || SSticker.Bible_deity_name
			var/reigion = GLOB.religion || SSticker.Bible_name
			data["religion"] = reigion
			var/bible_name = GLOB.bible_name || SSticker.Bible_name
			// if(!bible_name)
			// 	bible_name = DEFAULT_BIBLE_REPLACE(reigion)
			data["bible_name"] = bible_name
			data["bible_sprite"] = "display-[GLOB.bible_icon_state || SSticker.Bible_icon_state]"
			data["posters"] = list()
...
/obj/machinery/computer/libraryconsole/bookmanagement/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("print_bible")
			...
			return TRUE
		if("lore_spawn")
			if(obj_flags & OBJ_EMAGGED && can_spawn_lore)
...
/obj/machinery/computer/attackby(obj/item/W as obj, mob/user as mob)
...
/obj/machinery/computer/libraryconsole/bookmanagement/emag_act(remaining_charges, mob/user, obj/item/card/emag/emag_card)
	if(!density || obj_flags & OBJ_EMAGGED)
		return FALSE
...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/print_forbidden_lore(mob/user)
	...
	var/datum/gender/gender = GLOB.gender_datums[user.get_visible_gender()]
	...
	user.visible_message(SPAN_WARNING("[user] stares at the blank screen for a few moments, [gender.his] expression frozen in fear. When [gender.he] finally awaken[gender.s] from it, [gender.he] look[gender.s] a lot older."), range = 2)
	if(ishuman(user))
		var/mob/living/carbon/human/fool = user
		fool.age = clamp(fool.age + 10, 22, 85) //Fuck you
...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/upload_from_scanner(upload_category)
	var/obj/machinery/libraryscanner/scan = get_scanner()
	if(!scan)
		atom_say("No nearby scanner detected.")
		return
	if(!scan.cache)
		atom_say("No cached book found. Aborting upload.")
		return
	if (!SSdbcore.Connect())
		atom_say("Connection to Archive has been severed. Aborting.")
		return
	var/datum/book_info/book = scan.cache
	if(!book.title)
		atom_say("No title detected. Aborting")
		return
	if(!book.author)
		atom_say("No author detected. Aborting")
		return
	if(!book.content)
		atom_say("No content detected. Aborting")
		return
	...
	var/datum/db_query/query_library_upload = SSdbcore.NewQuery({"
		INSERT INTO [DB_PREFIX_TABLE_NAME("library")] (author, title, content, category, ckey, datetime, round_id_created)
	...
	if(!query_library_upload.Execute())
		...
		atom_say("Database error encountered uploading to Archive")
		return
	// usr.log_message(msg, LOG_GAME)
	qdel(query_library_upload)
	library_updated()
	atom_say("Upload Complete. Uploaded title will be available for printing in a moment")
	update_db_info()
...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/attempt_print(datum/callback/call_after)
	if(!COOLDOWN_FINISHED(src, printer_cooldown))
		atom_say("Printer currently unavailable, please wait a moment.")
		return FALSE
...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/print_bible()
	var/obj/item/storage/bible/holy_book = new(loc)
	if(!GLOB.bible_icon_state && !SSticker.Bible_icon_state)
		return
	holy_book.icon_state = GLOB.bible_icon_state || SSticker.Bible_icon_state
	holy_book.name = GLOB.bible_name || SSticker.Bible_name
	holy_book.deity_name = GLOB.deity || SSticker.Bible_deity_name
...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/print_book(id)
	...
	var/datum/db_query/query_library_print = SSdbcore.NewQuery(
		"SELECT * FROM [DB_PREFIX_TABLE_NAME("library")] WHERE id=:id AND isnull(deleted)",
		list("id" = id)
	)
	if(!query_library_print.Execute())
		qdel(query_library_print)
		atom_say("PRINTER ERROR! Failed to print document (0x0000000F)")
		return
	...
	switch(action)
		if("scan")
			if(cache?.compare(held_book.book_data))
				atom_say("This book is already in my internal cache")
				return
...
/obj/machinery/bookbinder/proc/prebind_book(mob/user, obj/item/paper/draw_from)
	...
	if(!scanned_name)
		...
		atom_say("No ID detected. Please scan your ID if you would like to be credited for this book. Otherwise please enter your paper again.")
		return

	if(!user.transfer_item_to_loc(draw_from, src))
		return
