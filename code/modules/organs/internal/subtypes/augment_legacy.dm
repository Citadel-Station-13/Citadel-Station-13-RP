...
/obj/item/organ/internal/augment_legacy
	name = "augment"
...
/obj/item/organ/internal/augment_legacy/Initialize(mapload)
	. = ..()
	setup_radial_icon()
	if(integrated_object_type)
		set_item(integrated_object_type)

/obj/item/organ/internal/augment_legacy/proc/set_item(obj/item/item_or_type)
	if(ispath(item_or_type))
		item_or_type = new item_or_type
	register_item(item_or_type)

/obj/item/organ/internal/augment_legacy/proc/register_item(obj/item/I)
	if(!I)
		return
...
/obj/item/organ/internal/augment_legacy/proc/unregister_item(obj/item/I)
...
/obj/item/organ/internal/augment_legacy/proc/on_item_moved(datum/source, atom/old)
	SIGNAL_HANDLER

	// gives a chance for dropped to fire
	addtimer(CALLBACK(src, PROC_REF(check_item_yank), source), 0)

/obj/item/organ/internal/augment_legacy/proc/on_item_dropped(datum/source)
	SIGNAL_HANDLER
...
/obj/item/organ/internal/augment_legacy/proc/check_item_yank(obj/item/I)
...
/obj/item/organ/internal/augment_legacy/Destroy()
	if(integrated_object)
		QDEL_NULL(integrated_object)
	return ..()

/obj/item/organ/internal/augment_legacy/proc/setup_radial_icon()
	if(!radial_icon)
		radial_icon = icon
	if(!radial_name)
		radial_name = name
...
/obj/item/organ/internal/augment_legacy/handle_organ_mod_special(var/removed = FALSE)
	if(removed && integrated_object && integrated_object.loc != src)
		integrated_object.forceMove(src)
	..(removed)

/obj/item/organ/internal/augment_legacy/proc/augment_use()
	if(!owner)
		return
...
/obj/item/organ/internal/augment_legacy/proc/augment_action()
...
/mob/living/carbon/human/proc/enable_augments(mob/living/L)
	...
	for(var/obj/item/organ/internal/augment_legacy/Aug in internal_organs)
	...
	for(var/augname in present_augs)
		var/obj/item/organ/internal/augment_legacy/iconsource = present_augs[augname]
	...
	if(!isnull(choice) && options[choice])
		var/obj/item/organ/internal/augment_legacy/A = present_augs[choice]
		A.augment_use()
