...
/datum/reagent/ryetalyn/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	...
	if(alien == IS_SLIME && istype(H)) //Shifts them toward white, faster than Rezadone does toward grey.
		...
		if(prob(50))
			if(r_skin)
				r_skin = round((r_skin + 510)/3)
			if(r_hair)
				r_hair = round((r_hair + 510)/3)
			if(r_facial)
				r_facial = round((r_facial + 510)/3)
			H.adjustToxLoss(6 * removed)
		if(prob(50))
			if(g_skin)
				g_skin = round((g_skin + 510)/3)
			if(g_hair)
				g_hair = round((g_hair + 510)/3)
			if(g_facial)
				g_facial = round((g_facial + 510)/3)
			H.adjustToxLoss(6 * removed)
		if(prob(50))
			if(b_skin)
				b_skin = round((b_skin + 510)/3)
			if(b_hair)
				b_hair = round((b_hair + 510)/3)
			if(b_facial)
				b_facial = round((b_facial + 510)/3)
			H.adjustToxLoss(6 * removed)
