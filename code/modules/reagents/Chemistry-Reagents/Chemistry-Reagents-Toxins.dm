...
/datum/reagent/mutagen/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	...
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		var/r_skin = HEX_RED(H.skin_colour)
		var/g_skin = HEX_GREEN(H.skin_colour)
		var/b_skin = HEX_BLUE(H.skin_colour)
		var/r_hair = HEX_RED(H.hair_colour)
		var/g_hair = HEX_GREEN(H.hair_colour)
		var/b_hair = HEX_BLUE(H.hair_colour)
		var/r_facial = HEX_RED(H.facial_hair_colour)
		var/g_facial = HEX_GREEN(H.facial_hair_colour)
		var/b_facial = HEX_BLUE(H.facial_hair_colour)
		if(alien == IS_SLIME && prob(25))
			var/color_shift = rand(-100, 100)
			spawn(1)
				if(prob(33))
					if(r_skin)
						r_skin = max(0, min(255, r_skin + color_shift))
					if(r_hair)
						r_hair = max(0, min(255, r_hair + color_shift))
					if(r_facial)
						r_facial = max(0, min(255, r_facial + color_shift))
				if(prob(33))
					if(g_skin)
						g_skin = max(0, min(255, g_skin + color_shift))
					if(g_hair)
						g_hair = max(0, min(255, g_hair + color_shift))
					if(H.g_facial)
						g_facial = max(0, min(255, g_facial + color_shift))
				if(prob(33))
					if(b_skin)
						b_skin = max(0, min(255, b_skin + color_shift))
					if(b_hair)
						b_hair = max(0, min(255, b_hair + color_shift))
					if(b_facial)
						b_facial = max(0, min(255, b_facial + color_shift))
						return
		if(alien == IS_CHIMERA && prob(33))
			...
			spawn(1)
				...
				if(prob(45))
					...
					if(b_facial)
						b_facial = max(0, min(255, b_facial + color_shift))
		H.skin_colour = rgb(r_skin, g_skin, b_skin)
		H.hair_colour = rgb(r_hair, g_hair, b_hair)
		H.facial_hair_colour = rgb(r_facial, g_facial, b_facial)
