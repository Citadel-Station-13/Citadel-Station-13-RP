...
/datum/reagent/toxin/hydrophoron/touch_turf(var/turf/simulated/T)
	if(!istype(T))
		return
	T.assume_gas(/datum/reagent/gas/phoron, CEILING(volume/2, 1), T20C)
	for(var/turf/simulated/floor/target_tile in range(0,T))
		target_tile.assume_gas(/datum/reagent/gas/phoron, volume/2, 400+T0C)
...
/datum/reagent/toxin/phoron/touch_turf(var/turf/simulated/T, var/amount)
	if(!istype(T))
		return
	T.assume_gas(/datum/reagent/gas/volatile_fuel, amount, T20C)
	remove_self(amount)
