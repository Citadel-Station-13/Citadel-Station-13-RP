		...
		if(SYRINGE_INJECT)
			...
			if(ismob(target))
				while(reagents.total_volume)
					trans += reagents.trans_to_mob(target, amount_per_transfer_from_this, REAGENT_APPLY_INJECT)
					update_icon()
...
/obj/item/reagent_containers/syringe/proc/syringestab(mob/living/carbon/target as mob, mob/living/carbon/user as mob)
	...
	var/trans = reagents.trans_to_mob(target, syringestab_amount_transferred, REAGENT_APPLY_INJECT)
