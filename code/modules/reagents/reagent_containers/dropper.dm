...
/obj/item/reagent_containers/dropper/afterattack(atom/target, mob/user, clickchain_flags, list/params)
	...
	if(reagents.total_volume)
		...
		if(ismob(target))
			...
			trans += reagents.trans_to_mob(target, min(amount_per_transfer_from_this, reagents.total_volume)/2, REAGENT_APPLY_INGEST) //Half injected, half ingested
			trans += reagents.trans_to_mob(target, min(amount_per_transfer_from_this, reagents.total_volume), REAGENT_APPLY_INJECT) //I guess it gets into the bloodstream through the eyes or something
