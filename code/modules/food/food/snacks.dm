...
/obj/item/clothing/mask/candy
	...
	var/type_stick = 0
	var/chem_volume = 0
	var/max_licktime = 0	//Related to sprites
	var/licktime = 0
	var/nutriment_amt = 0
	var/list/nutriment_desc = list("food" = 1)
	blood_sprite_state = null //Can't bloody these

/obj/item/clothing/mask/candy/Initialize(mapload)
	. = ..()
	flags |= NOREACT // so it doesn't react until you open it
	create_reagents(chem_volume) // making the candy a chemical holder with a maximum volume of 15
	if(licktime && !max_licktime)
		max_licktime = licktime
	if(nutriment_amt)
		reagents.add_reagent("nutriment",nutriment_amt,nutriment_desc)

/obj/item/clothing/mask/candy/proc/lick(amount)
	if(licktime > max_licktime)
		licktime = max_licktime
	licktime -= amount
	if(reagents && reagents.total_volume) // check if it has any reagents at all
		if(ishuman(loc))
			var/mob/living/carbon/human/C = loc
			if (src == C.wear_mask && C.check_has_mouth()) // if it's in the human/monkey mouth, transfer reagents to the mob
				reagents.trans_to_mob(C, REM, CHEM_INGEST) // Removed the balance inhibitor put in on cigarettes.
		else // else just remove some of the reagents
			reagents.remove_any(REM)

/obj/item/clothing/mask/candy/process(delta_time)
	lick(1)
	if(lick_percent < 1) //THIS IS WHERE I LEFT OFF BEFORE BED 9/5 Was originally if(licktime < 1)
		dissolve()
		return

/obj/item/clothing/mask/candy/examine(mob/user)
	. = ..()
	var/lick_percent = round((licktime / max_licktime) * 100)
	switch(lick_percent)
		if(90 to INFINITY)
			. += "[src] is still fresh."
		if(60 to 90)
			. += "[src] has a good amount of candy remaining."
		if(30 to 60)
			. += "[src] is about half finished."
		if(10 to 30)
			. += "[src] is starting to look thin."
		else
			. += "[src] is nearly gone!"

/obj/item/clothing/mask/candy/proc/unwrap(var/flavor_text = "[usr] unwraps the [name].")
	if(!src.unwrapped)
		src.unwrapped = 1
		damtype = "brute"
		flags &= ~NOREACT // allowing reagents to react after being unwrapped
...
/obj/item/clothing/mask/candy/proc/dissolve(var/nomessage = 0)
	var/turf/T = get_turf(src)
	STOP_PROCESSING(SSobj, src)
	if (type_stick)
		var/obj/item/trash/lollipop_stick = new type_stick(T)
		transfer_fingerprints_to(lollipop_stick)
		if(ismob(loc))
			var/mob/living/M = loc
			if (!nomessage)
				to_chat(M, "<span class='notice'>Your [name] dissolves.</span>")
...
/obj/item/clothing/mask/candy/proc/wrap()
	unwrapped = 0
	STOP_PROCESSING(SSobj, src)

/obj/item/clothing/mask/candy/attack(mob/living/carbon/human/H, mob/user, def_zone)
	if(unwrapped && H == user && istype(H))
		var/obj/item/blocked = H.check_mouth_coverage()
		if(blocked)
			to_chat(H, "<span class='warning'>\The [blocked] is in the way!</span>")
			return 1
		to_chat(H, "<span class='notice'>You take a bite of your [name].</span>")
		lick(100)
		playsound(H.loc,'sound/items/eatfood.ogg', rand(10,50), 1)
		return 1
	return ..()

/obj/item/clothing/mask/candy/attack_self()
	if(!unwrapped)
		unwrap()
	else
		return

/obj/item/clothing/mask/candy/attackby(obj/item/W as obj, mob/user as mob)
	..()
...
/obj/item/clothing/mask/candy/lollipop
	name = "lollipop"
...
/obj/item/clothing/mask/candy/lollipop/Initialize(mapload)
	. = ..()
...
/obj/item/clothing/mask/candy/lollipop/afterattack(obj/item/reagent_containers/glass/glass, mob/user as mob, proximity)
	..()
	if(!proximity)
		return
	if(istype(glass)) //you can dip lollipops into beakers
		...
		if(transfered)	//if reagents were transfered, show the message
			...
			if(!glass.reagents.total_volume)
				...
				to_chat(user, "<span class='notice'>[src] is fully coated.</span>")

/obj/item/clothing/mask/candy/lollipop/proc/change_head_color(C)
	headcolor = C
	cut_overlay(head)
	head.color = C
	add_overlay(head)
...
//Medicated lollipops.
/obj/item/clothing/mask/candy/lollipop/tramadol
	name = "Tram-pop"
