...
/obj/item/reagent_containers/food/drinks/glass2/proc/can_add_extra(obj/item/glass_extra/GE)
	if(!("[base_icon]_[GE.glass_addition]left" in fast_icon_states(DRINK_ICON_FILE)))
		return 0
	if(!("[base_icon]_[GE.glass_addition]right" in fast_icon_states(DRINK_ICON_FILE)))
		return 0
...
/obj/item/reagent_containers/food/drinks/glass2/update_icon()
	...
	if (reagents.reagent_list.len > 0)
		...
		for(var/S in R.glass_special)
			if("[base_icon]_[S]" in fast_icon_states(DRINK_ICON_FILE))
				under_liquid |= "[base_icon]_[S]"
			else if("[base_icon][amnt]_[S]" in fast_icon_states(DRINK_ICON_FILE))
