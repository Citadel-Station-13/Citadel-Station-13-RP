...
/obj/machinery/smartfridge/drying_rack/attackby(var/obj/item/O as obj, mob/user)
	...
	if(istype(O, /obj/item/stack/wetleather/))
		var/obj/item/stack/wetleather/WL = O
		if(WL.amount > 2)
			to_chat(SPAN_NOTICE("The rack can only fit one sheet at a time!"))
			return 1
		else
			if(!user.attempt_insert_item_for_installation(WL, src))
				return
			stock(WL)
			user.visible_message(SPAN_NOTICE("[user] has added \the [WL] to \the [src]."), SPAN_NOTICE("You add \the [WL] to \the [src]."))
