...
//For debug purposes!
/datum/ai_holder/fsm/proc/ai_log_output(var/msg = "missing message", var/ver = AI_LOG_INFO)
	var/span_type
	switch(ver)
		if(AI_LOG_OFF)
			return
...
/datum/ai_holder/fsm/proc/stance_color()
	var/new_color = null
	switch(stance)
		if(STANCE_SLEEP)
...
/datum/ai_holder/fsm/proc/debug()
	stance_coloring = TRUE
	path_display = TRUE
	last_turf_display = TRUE
	debug_ai = AI_LOG_INFO

/datum/ai_holder/fsm/hostile/debug
	wander = FALSE
	conserve_ammo = FALSE
