/datum/prototype/bodyset_marking/legacy/tatoo
	abstract_type = /datum/prototype/bodyset_marking/legacy/tatoo
	icon = "icons/mob/sprite_accessories/markings/tatoos.dmi"

/datum/prototype/bodyset_marking/legacy/tatoo/tat_rheart
	name = "Tattoo (Heart, R. Arm)"
	id = "marking_tattoo_r_arm_heart"
	icon_state = "tat_rheart"
	body_parts = list(BP_R_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_lheart
	name = "Tattoo (Heart, L. Arm)"
	id = "marking_tattoo_l_arm_heart"
	icon_state = "tat_lheart"
	body_parts = list(BP_L_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_hive
	name = "Tattoo (Hive, Back)"
	id = "marking_tattoo_back_hive"
	icon_state = "tat_hive"
	body_parts = list(BP_TORSO)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_nightling
	name = "Tattoo (Nightling, Back)"
	id = "marking_tattoo_back_nightling"
	icon_state = "tat_nightling"
	body_parts = list(BP_TORSO)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_campbell
	name = "Tattoo (Campbell, R.Arm)"
	id = "marking_tattoo_r_arm_campbell"
	icon_state = "tat_campbell"
	body_parts = list(BP_R_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_campbell/left
	name = "Tattoo (Campbell, L.Arm)"
	id = "marking_tattoo_l_arm_campbell"
	body_parts = list(BP_L_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_campbell/rightleg
	name = "Tattoo (Campbell, R.Leg)"
	id = "marking_tattoo_r_leg_campbell"
	body_parts = list(BP_R_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_campbell/leftleg
	name = "Tattoo (Campbell, L.Leg)"
	id = "marking_tattoo_l_leg_campbell"
	body_parts = list (BP_L_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_silverburgh
	name = "Tattoo (Silverburgh, R.Leg)"
	id = "marking_tattoo_r_leg_silverburgh"
	icon_state = "tat_silverburgh"
	body_parts = list (BP_R_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_silverburgh/left
	name = "Tattoo (Silverburgh, L.Leg)"
	id = "marking_tattoo_l_leg_silverburgh"
	icon_state = "tat_silverburgh"
	body_parts = list (BP_L_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger
	name = "Tattoo (Tiger Stripes, Body)"
	id = "marking_tattoo_body_tiger"
	icon_state = "tat_tiger"
	body_parts = list(BP_L_FOOT,BP_R_FOOT,BP_L_LEG,BP_R_LEG,BP_L_ARM,BP_R_ARM,BP_L_HAND,BP_R_HAND,BP_TORSO,BP_GROIN)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/l_foot
	name = "Tattoo (Tiger Stripes, Left Foot)"
	id = "tat_tiger-l_foot"
	body_parts = list(BP_L_FOOT)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/r_foot
	name = "Tattoo (Tiger Stripes, Right Foot)"
	id = "tat_tiger-r_foot"
	body_parts = list(BP_R_FOOT)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/l_leg
	name = "Tattoo (Tiger Stripes, Left Leg)"
	id = "tat_tiger-l_leg"
	body_parts = list(BP_L_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/r_leg
	name = "Tattoo (Tiger Stripes, Right Leg)"
	id = "tat_tiger-r_leg"
	body_parts = list(BP_R_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/groin
	name = "Tattoo (Tiger Stripes, Groin)"
	id = "tat_tiger-groin"
	body_parts = list(BP_GROIN)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/torso
	name = "Tattoo (Tiger Stripes, Torso)"
	id = "tat_tiger-torso"
	body_parts = list(BP_TORSO)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/l_arm
	name = "Tattoo (Tiger Stripes, Left Arm)"
	id = "tat_tiger-l_arm"
	body_parts = list(BP_L_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/r_arm
	name = "Tattoo (Tiger Stripes, Right Arm)"
	id = "tat_tiger-r_arm"
	body_parts = list(BP_R_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/r_hand
	name = "Tattoo (Tiger Stripes, Right Hand)"
	id = "tat_tiger-r_hand"
	body_parts = list(BP_R_HAND)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_tiger/l_hand
	name = "Tattoo (Tiger Stripes, Left Hand)"
	id = "tat_tiger-l_hand"
	body_parts = list(BP_L_HAND)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_inmon1
	name = "Tattoo (Inmon, Variant 1)"
	id = "marking_tattoo_groin_inmon"
	icon_state = "tat_inmon1"
	body_parts = list(BP_GROIN)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_inmon2
	name = "Tattoo (Inmon, Variant 2)"
	id = "marking_tattoo_groin_inmon2"
	icon_state = "tat_inmon2"
	body_parts = list(BP_GROIN)

/datum/prototype/bodyset_marking/legacy/tatoo/tat_inmon3
	name = "Tattoo (Inmon, Variant 3)"
	id = "marking_tattoo_groin_inmon3"
	icon_state = "tat_inmon3"
	body_parts = list(BP_GROIN)

/datum/prototype/bodyset_marking/legacy/tatoo/spelunker
	name = "Tattoo (Spelunker, Body)"
	id = "marking_tattoo_body_spelunker"
	icon_state = "tat_spelunker"
	body_parts = list(BP_L_LEG,BP_R_LEG,BP_L_ARM,BP_R_ARM,BP_TORSO,BP_GROIN,BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/spelunker/l_leg
	name = "Tattoo (Spelunker, Left Leg)"
	id = "marking_tattoo_body_spelunker-l_leg"
	body_parts = list(BP_L_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/spelunker/r_leg
	name = "Tattoo (Spelunker, Right Leg)"
	id = "marking_tattoo_body_spelunker-r_leg"
	body_parts = list(BP_R_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/spelunker/head
	name = "Tattoo (Spelunker, Head)"
	id = "marking_tattoo_body_spelunker-head"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/spelunker/torso
	name = "Tattoo (Spelunker, Torso)"
	id = "marking_tattoo_body_spelunker-torso"
	body_parts = list(BP_TORSO)

/datum/prototype/bodyset_marking/legacy/tatoo/spelunker/r_arm
	name = "Tattoo (Spelunker, Right Arm)"
	id = "marking_tattoo_body_spelunker-r_arm"
	body_parts = list(BP_R_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/spelunker/l_arm
	name = "Tattoo (Spelunker, Left Arm)"
	id = "marking_tattoo_body_spelunker-l_arm"
	body_parts = list(BP_L_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/spelunker/groin
	name = "Tattoo (Spelunker, Groin)"
	id = "marking_tattoo_body_spelunker-groin"
	body_parts = list(BP_GROIN)

/datum/prototype/bodyset_marking/legacy/tatoo/delver
	name = "Tattoo (Delver, Body)"
	id = "marking_tattoo_body_delver"
	icon_state = "tat_delver"
	body_parts = list(BP_L_LEG,BP_R_LEG,BP_L_ARM,BP_R_ARM,BP_TORSO,BP_GROIN,BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/delver/l_leg
	name = "Tattoo (Delver, Left Leg)"
	id = "marking_tattoo_body_delver-l_leg"
	body_parts = list(BP_L_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/delver/r_leg
	name = "Tattoo (Delver, Right Leg)"
	id = "marking_tattoo_body_delver-r_leg"
	body_parts = list(BP_R_LEG)

/datum/prototype/bodyset_marking/legacy/tatoo/delver/r_arm
	name = "Tattoo (Delver, Right Arm)"
	id = "marking_tattoo_body_delver-r_arm"
	body_parts = list(BP_R_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/delver/l_arm
	name = "Tattoo (Delver, Left Arm)"
	id = "marking_tattoo_body_delver-l_arm"
	body_parts = list(BP_L_ARM)

/datum/prototype/bodyset_marking/legacy/tatoo/delver/groin
	name = "Tattoo (Delver, Groin)"
	id = "marking_tattoo_body_delver-groin"
	body_parts = list(BP_GROIN)

/datum/prototype/bodyset_marking/legacy/tatoo/delver/torso
	name = "Tattoo (Delver, Torso)"
	id = "marking_tattoo_body_delver-torso"
	body_parts = list(BP_TORSO)

/datum/prototype/bodyset_marking/legacy/tatoo/delver/head
	name = "Tattoo (Delver, Head)"
	id = "marking_tattoo_body_delver-head"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/eye_mark
	name = "Tattoo (Mark, Left Eye)"
	id = "marking_tattoo_eye_mark_left"
	icon_state = "tat_eye_tattoo_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/eye_mark/right
	name = "Tattoo (Mark, Right Eye)"
	id = "marking_tattoo_eye_mark_right"
	icon_state = "tat_eye_tattoo_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/jaeger
	name = "Tattoo (Jaeger, Right Head)"
	id = "marking_tattoo_jaeger_r"
	icon_state = "tat_jaeger_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/jaeger/left
	name = "Tattoo (Jaeger, Left Head)"
	id = "marking_tattoo_jaeger_l"
	icon_state = "tat_jaeger_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/lujan
	name = "Tattoo (Lujan, Right Head)"
	id = "marking_tattoo_lujan_r"
	icon_state = "tat_lujan_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/lujan/left
	name = "Tattoo (Lujan, Left Head)"
	id = "marking_tattoo_lujan_l"
	icon_state = "tat_lujan_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/natasha
	name = "Tattoo (Natasha, Left Head)"
	id = "marking_tattoo_natasha_l"
	icon_state = "tat_natasha_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/natasha/right
	name = "Tattoo (Natasha, Right Head)"
	id = "marking_tattoo_natasha_r"
	icon_state = "tat_natasha_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/forrest
	name = "Tattoo (Forrest, Right Head)"
	id = "marking_tattoo_forrest_r"
	icon_state = "tat_forrest_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/forrest/left
	name = "Tattoo (Forrest, Left Head)"
	id = "marking_tattoo_forrest_l"
	icon_state = "tat_forrest_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/toshi
	name = "Tattoo (Toshi, Left Head)"
	id = "marking_tattoo_toshi_l"
	icon_state = "tat_toshi_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/toshi/right
	name = "Tattoo (Toshi, Right Head)"
	id = "marking_tattoo_toshi_r"
	icon_state = "tat_toshi_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/cheshire
	name = "Tattoo (Cheshire, Right Head)"
	id = "marking_tattoo_cheshire_r"
	icon_state = "tat_cheshire_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/cheshire/left
	name = "Tattoo (Cheshire, Left Head)"
	id = "marking_tattoo_cheshire_l"
	icon_state = "tat_cheshire_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/bindi
	name = "Tattoo (Bindi, Head)"
	id = "marking_tattoo_bindi"
	icon_state = "tat_bindi"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/eyestripe
	name = "Tattoo (Eyestripe, Head)"
	id = "marking_tattoo_eyestripe"
	icon_state = "tat_eyestripe"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/kater
	name = "Tattoo (Kater, Right Head)"
	id = "marking_tattoo_kater_r"
	icon_state = "tat_kater_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/kater/left
	name = "Tattoo (Kater, Left Head)"
	id = "marking_tattoo_kater_l"
	icon_state = "tat_kater_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/hunter
	name = "Tattoo (Hunter, Right Head)"
	id = "marking_tattoo_hunter_r"
	icon_state = "tat_hunter_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/hunter/left
	name = "Tattoo (Hunter, Left Head)"
	id = "marking_tattoo_hunter_l"
	icon_state = "tat_hunter_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/nosetripe
	name = "Tattoo (Nosestripe, Head)"
	id = "marking_tattoo_nosetripe"
	icon_state = "tat_nosetripe"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/wingsback
	name = "Tattoo (Wings Back, Torso)"
	id = "marking_tattoo_wings_back"
	icon_state = "tat_wingsback"
	body_parts = list(BP_TORSO)

/datum/prototype/bodyset_marking/legacy/tatoo/heartback
	name = "Tattoo (Heart Back, Torso)"
	id = "marking_tattoo_heart_back"
	icon_state = "tat_heartback"
	body_parts = list(BP_TORSO)

/datum/prototype/bodyset_marking/legacy/tatoo/belly
	name = "Tattoo (Belly, Torso)"
	id = "marking_tattoo_belly"
	icon_state = "tat_belly"
	body_parts = list(BP_TORSO)

/datum/prototype/bodyset_marking/legacy/tatoo/neck
	name = "Tattoo (Thick Neck, Head)"
	id = "marking_tattoo_thickneck"
	icon_state = "tat_neckthickk"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/ear
	name = "Tattoo (Ear Right, Head)"
	id = "marking_tattoo_ear_r"
	icon_state = "tat_ear_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/ear/left
	name = "Tattoo (Ear Left, Head)"
	id = "marking_tattoo_ear_l"
	icon_state = "tat_ear_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/cheekspot
	name = "Tattoo (Cheekspot Left, Head)"
	id = "marking_tattoo_cheekspot_l"
	icon_state = "tat_cheekspot_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/cheekspot/right
	name = "Tattoo (Cheekspot Right, Head)"
	id = "marking_tattoo_cheekspot_r"
	icon_state = "tat_cheekspot_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/crow
	name = "Tattoo (Crow Right, Head)"
	id = "marking_tattoo_crow_r"
	icon_state = "tat_crow_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/crow/left
	name = "Tattoo (Crow Left, Head)"
	id = "marking_tattoo_crow_l"
	icon_state = "tat_crow_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/nosetape
	name = "Tattoo (Nose, Head)"
	id = "marking_tattoo_nose"
	icon_state = "tat_nosetape"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/mouth
	name = "Tattoo (Mouth Left, Head)"
	id = "marking_tattoo_mouth_l"
	icon_state = "tat_lipcorner_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/mouth/right
	name = "Tattoo (Mouth Right, Head)"
	id = "marking_tattoo_mouth_r"
	icon_state = "tat_lipcorner_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/eye
	name = "Tattoo (Eye Corner Right, Head)"
	id = "marking_tattoo_eyecorner_r"
	icon_state = "tat_eyecorner_r"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/eye/left
	name = "Tattoo (Eye Corner Left, Head)"
	id = "marking_tattoo_eyecorner_l"
	icon_state = "tat_eyecorner_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/eyelash
	name = "Tattoo (Eyelash Left, Head)"
	id = "marking_tattoo_eyelash_l"
	icon_state = "tat_eyelash_l"
	body_parts = list(BP_HEAD)

/datum/prototype/bodyset_marking/legacy/tatoo/eyelash/right
	name = "Tattoo (Eyelash Right, Head)"
	id = "marking_tattoo_eyelash_r"
	icon_state = "tat_eyelash_r"
	body_parts = list(BP_HEAD)
