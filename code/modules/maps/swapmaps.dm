...
proc/SwapMaps_CreateFromTemplate(template_id)
	...
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("map_[template_id].txt"))
		...
		to_chat(world.log, "SwapMaps error in SwapMaps_CreateFromTemplate(): map_[template_id] file not found.")
		return
	if(text)
		S=new
...
proc/SwapMaps_LoadChunk(chunk_id,turf/locorner)
	...
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("map_[chunk_id].txt"))
		...
		to_chat(world.log, "SwapMaps error in SwapMaps_LoadChunk(): map_[chunk_id] file not found.")
		return
	if(text)
		S=new
...
proc/SwapMaps_SaveChunk(chunk_id,turf/corner1,turf/corner2)
	if(!corner1 || !corner2)
		to_chat(world.log, "SwapMaps error in SwapMaps_SaveChunk():")
...
proc/SwapMaps_GetSize(id)
	...
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("map_[id].txt"))
		...
		to_chat(world.log, "SwapMaps error in SwapMaps_GetSize(): map_[id] file not found.")
		return
	if(text)
		S=new
		S.ImportText("/",file("map_[id].txt"))
