	...
	if(!used)
		...
		switch(status)
			...
			if(SHELTER_DEPLOY_BAD_AREA)
				visible_message("<span class='warning'>\The [src] will not function in this area.</span>")

			//Anchored objects or no space
			if(SHELTER_DEPLOY_BAD_TURFS, SHELTER_DEPLOY_ANCHORED_OBJECTS)
				var/width = template.width
				var/height = template.height
				visible_message("<span class='warning'>\The [src] doesn't have room to deploy! You need to clear a [width]x[height] area!</span>")
...
/obj/structure/table/survival_pod/Initialize()
