...
/datum/map_reservation/proc/release()
	...
	var/spatial_width = ceil(world.maxx / TURF_ALIGNMENT)
...
/datum/map_reservation/proc/reserve(width, height, border, z_override)
	...
	var/how_many_wide = ceil(real_width / TURF_ALIGNMENT)
	var/how_many_high = ceil(real_height / TURF_ALIGNMENT)
	var/total_many_wide = floor(world.maxx / TURF_ALIGNMENT)
	var/total_many_high = floor(world.maxy / TURF_ALIGNMENT)

	// the dreaded 5 deep for loop
	while((level_index = z_override) || (level_index = pick_n_take(possible_levels)))
		/**
		 * here's the magic
		 * because reservations are aligned to TURF_ALIGNMENT,
		 * we just have to check the start spots, since we always align to them.
		 *
		 * bottom-left turfs on reservations align to
		 * (chunk_x - 1) * TURF_ALIGNMENT + 1
		 * (chunk_y - 1) * TURF_ALIGNMENT + 1
		 */
		for(var/outer_x in 1 to (total_many_wide - how_many_wide + 1))
			for(var/outer_y in 1 to (total_many_high - how_many_high + 1))
				...
				for(var/inner_x in outer_x to outer_x + how_many_wide - 1)
					for(var/inner_y in outer_y to outer_y + how_many_high - 1)
						var/turf/checking = locate(
							1 + (inner_x - 1) * TURF_ALIGNMENT,
							1 + (inner_y - 1) * TURF_ALIGNMENT,
							level_index,
						)
				...
				BL = locate(1 + TURF_ALIGNMENT * (outer_x - 1) + border, 1 + TURF_ALIGNMENT * (outer_y - 1) + border, level_index)
				...
				if(border)
					final_border = list(
							...
							locate(
								1 + TURF_ALIGNMENT * (outer_x - 1),
								1 + TURF_ALIGNMENT * (outer_y - 1),
								level_index,
							),
							locate(
								1 + TURF_ALIGNMENT * (outer_x - 1) + (border - 1),
								BL.y + (real_height - 1),
								level_index,
							),
						) + block( // right pane
							...
							locate(
								1 + TURF_ALIGNMENT * (outer_x - 1) + border,
							...
							locate(
								BL.x + (real_width - 1) - border,
								1 + TURF_ALIGNMENT * (outer_y - 1) + (border - 1),
								level_index,
							),
						)
	...
	src.spatial_bl_x = ceil(bottom_left_coords[1] / TURF_ALIGNMENT)
	src.spatial_bl_y = ceil(bottom_left_coords[2] / TURF_ALIGNMENT)
	src.spatial_tr_x = ceil(top_right_coords[1] / TURF_ALIGNMENT)
	src.spatial_tr_y = ceil(top_right_coords[2] / TURF_ALIGNMENT)
