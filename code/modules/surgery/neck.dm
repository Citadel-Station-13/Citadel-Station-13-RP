...
/datum/surgery_step/brainstem/drill_vertebrae/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	//var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='danger'>[user] almost loses their grip on the [tool]!</span>" , \
	"<span class='danger'>Your hand slips and nearly shreds [target]'s brainstem with \the [tool]!</span>" )
	/*affected.createwound(PIERCE, 10)
