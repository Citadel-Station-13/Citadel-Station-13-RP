...
/obj/effect/overmap/visitable/Initialize(mapload)
	...
	start_x = start_x || rand(OVERMAP_EDGE, using_map_legacy.overmap_size - OVERMAP_EDGE)
	start_y = start_y || rand(OVERMAP_EDGE, using_map_legacy.overmap_size - OVERMAP_EDGE)

	forceMove(locate(start_x, start_y, using_map_legacy.overmap_z))
...
/obj/effect/overmap/visitable/proc/distress(mob/user)
	...
	priority_announcement.Sound('sound/AI/sos.ogg', using_map_legacy.zlevels)//play the sound once
...
/proc/build_overmap()
	...
	using_map_legacy.overmap_z = world.maxz

	testing("Putting overmap on [using_map_legacy.overmap_z]")
	var/area/overmap/A = new
	for (var/square in block(locate(1,1,using_map_legacy.overmap_z), locate(using_map_legacy.overmap_size,using_map_legacy.overmap_size,using_map_legacy.overmap_z)))
		var/turf/T = square
		if(T.x == 1 || T.y == 1 || T.x == using_map_legacy.overmap_size || T.y == using_map_legacy.overmap_size)
			T = T.ChangeTurf(/turf/unsimulated/map/edge)
		else
			T = T.ChangeTurf(/turf/unsimulated/map)
		ChangeArea(T, A)

	using_map_legacy.sealed_levels |= using_map_legacy.overmap_z
