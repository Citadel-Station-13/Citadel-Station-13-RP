...
/decl/overmap_event_handler/proc/create_events(var/z_level, var/overmap_size, var/number_of_events)
	...
	for(var/Trf in overmap_turfs)
		var/turf/T = Trf
		if(!(locate(/atom/movable/overmap_object/entity/visitable) in T))
			candidate_turfs += T
...
/decl/overmap_event_handler/proc/start_hazard(var/atom/movable/overmap_object/entity/visitable/ship/ship, var/atom/movable/overmap_object/event/hazard)	// Make these accept both hazards or events
...
/decl/overmap_event_handler/proc/stop_hazard(var/atom/movable/overmap_object/entity/visitable/ship/ship, var/atom/movable/overmap_object/event/hazard)
	for(var/event_type in hazard.events)
...
/decl/overmap_event_handler/proc/on_turf_entered(var/turf/new_loc, var/atom/movable/overmap_object/entity/visitable/ship/ship, var/old_loc)
	if(!istype(ship))
		return
	if(new_loc == old_loc)
		return

	for(var/atom/movable/overmap_object/event/E in hazard_by_turf[new_loc])
		start_hazard(ship, E)

/decl/overmap_event_handler/proc/on_turf_exited(var/turf/old_loc, var/atom/movable/overmap_object/entity/visitable/ship/ship, var/new_loc)
	if(!istype(ship))
		return
	if(new_loc == old_loc)
		return

	for(var/atom/movable/overmap_object/event/E in hazard_by_turf[old_loc])
...
/decl/overmap_event_handler/proc/update_hazards(var/turf/T)	// Catch all updater
	...
	for(var/atom/movable/overmap_object/event/E in T)
	...
	for(var/atom/movable/overmap_object/entity/visitable/ship/ship in T)
		for(var/datum/event/E in ship_events[ship])
		...
		for(var/atom/movable/overmap_object/event/E in active_hazards)
			start_hazard(ship, E)

/decl/overmap_event_handler/proc/is_event_in_turf(var/datum/event/E, var/turf/T)
	for(var/atom/movable/overmap_object/event/hazard in hazard_by_turf[T])
		if(E in hazard.events && E.severity == hazard.difficulty)
			return TRUE

/decl/overmap_event_handler/proc/is_event_included(var/list/hazards, var/atom/movable/overmap_object/event/E, var/equal_or_better)	// This proc is only used so it can break out of 2 loops cleanly
	for(var/atom/movable/overmap_object/event/A in hazards)
