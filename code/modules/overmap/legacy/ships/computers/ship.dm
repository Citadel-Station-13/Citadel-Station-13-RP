...
/obj/machinery/computer/ship/proc/attempt_hook_up(obj/overmap/entity/visitable/ship/sector)
	if(!istype(sector))
		return
	if(sector == SSovermaps.get_overmap_entity(src))
		linked = sector
		return TRUE

/obj/machinery/computer/ship/proc/sync_linked(var/user = null)
	var/obj/overmap/entity/visitable/ship/sector = SSovermaps.get_overmap_entity(src)
	if(!sector)
		return
...
/obj/machinery/computer/ship/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state, datum/event_args/actor/actor)
	if(..())
		return TRUE
	switch(action)
		if("sync")
