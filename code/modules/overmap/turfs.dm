...
/turf/unsimulated/map/edge/LateInitialize()
	//This could be done by using the using_map_legacy().overmap_size much faster, HOWEVER, doing it programatically to 'find'
...
/turf/unsimulated/map/Initialize(mapload)
	...
	if(x == 1 || x == using_map_legacy().overmap_size)
		numbers += list("[round(y/10)]","[round(y%10)]")
		if(y == 1 || y == using_map_legacy().overmap_size)
			numbers += "-"
	if(y == 1 || y == using_map_legacy().overmap_size)
	...
	for(var/i = 1 to numbers.len)
		...
		if(y == 1)
			I.pixel_y = 3
			I.pixel_x = 5*i + 4
		if(y == using_map_legacy().overmap_size)
			I.pixel_y = world.icon_size - 9
			I.pixel_x = 5*i + 4
		if(x == 1)
			I.pixel_x = 5*i - 2
		if(x == using_map_legacy().overmap_size)
			I.pixel_x = 5*i + 2
		add_overlay(I)
