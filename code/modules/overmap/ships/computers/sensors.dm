...
/obj/machinery/computer/ship/sensors/attempt_hook_up(atom/movable/overmap_object/entity/visitable/ship/sector)
	if(!(. = ..()))
		return
	find_sensors()
...
/obj/machinery/computer/ship/sensors/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("scan")
			var/atom/movable/overmap_object/O = locate(params["scan"])
