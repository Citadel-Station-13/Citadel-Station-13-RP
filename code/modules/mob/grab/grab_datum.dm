// TODO:Fucking fix the admin log procs
/datum/grab
...
/datum/grab/proc/upgrade(var/obj/item/grab/G)
	...
	if (can_upgrade(G))
		upgrade_effect(G)
		//admin_log(G.assailant, G.affecting, "tightens their grip on their victim to [upgrab.state_name]", "was grabbed more tightly to [upgrab.state_name]", "tightens grip to [upgrab.state_name] on")
		return upgrab
	else
