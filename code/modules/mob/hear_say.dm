...
/mob/proc/hear_say(var/message, var/verb = "says", var/datum/language/language = null, var/alt_name = "",var/italics = 0, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol)
	...
	if(speaker && !speaker.client && istype(src,/mob/observer/dead) && get_preference_toggle(/datum/client_preference/ghost_ears) && !(speaker in view(src)))
	...
	if(istype(src, /mob/observer/dead))
		...
		if(get_preference_toggle(/datum/client_preference/ghost_ears) && (speaker in view(src)))
			message = "<b>[message]</b>"
	...
	if(is_deaf())
		...
		if(language)
			...
			if(client && !(language.language_flags & LANGUAGE_HIVEMIND) && say_understands(speaker, language) && language.shorthand && client.get_preference_toggle(/datum/client_preference/language_indicator))
		...
		if(check_mentioned(message) && get_preference_toggle(/datum/client_preference/check_mention))
...
/mob/proc/on_hear_radio(part_a, speaker_name, track, part_b, formatted)
	...
	if(check_mentioned(formatted) && get_preference_toggle(/datum/client_preference/check_mention))
