...
/obj/structure/closet/crate/mimic/open()
	...
	if(mimic_active)
		...
		if(prob(mimic_chance))
			...
			visible_message("<span class='danger'><b>The [new_mimic] suddenly growls as it turns out to be a mimic!</b></span>")
			forceMove(new_mimic)
...
/obj/structure/closet/crate/mimic/damage(var/damage)
	if(contents.len)
		visible_message("<span class='danger'><b>The [src] makes out a crunchy noise as its contents are destroyed!</b></span>")
		for(var/obj/O in src.contents)
			qdel(O)
	..()
