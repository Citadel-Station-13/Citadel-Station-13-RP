...
/mob/living/simple_mob/proc/handle_atmos()
	...
	if(istype(A,/turf))
		...
		if(Environment)
			...
			if(min_oxy)
				if(Environment.gas[/datum/reagent/gas/oxygen] < min_oxy)
					atmos_unsuitable = 1
			if(max_oxy)
				if(Environment.gas[/datum/reagent/gas/oxygen] > max_oxy)
					atmos_unsuitable = 1
			if(min_tox)
				if(Environment.gas[/datum/reagent/gas/phoron] < min_tox)
					atmos_unsuitable = 2
			if(max_tox)
				if(Environment.gas[/datum/reagent/gas/phoron] > max_tox)
					atmos_unsuitable = 2
			if(min_n2)
				if(Environment.gas[/datum/reagent/gas/nitrogen] < min_n2)
					atmos_unsuitable = 1
			if(max_n2)
				if(Environment.gas[/datum/reagent/gas/nitrogen] > max_n2)
					atmos_unsuitable = 1
			if(min_co2)
				if(Environment.gas[/datum/reagent/gas/carbon_dioxide] < min_co2)
					atmos_unsuitable = 1
			if(max_co2)
				if(Environment.gas[/datum/reagent/gas/carbon_dioxide] > max_co2)
					atmos_unsuitable = 1
