...
/mob/living/simple_animal/otie/security/set_target(var/mob/M)
	...
	if(M in ListTargets(view_range))
		...
		if(check_threat(M) >= 4)
			GLOB.global_announcer.autosay("[src] is attempting to detain suspect <b>[target_name(M)]</b> in <b>[get_area(src)]</b>.", "SmartCollar oversight", "Security")
		return M
...
/mob/living/simple_animal/otie/Initialize()
	. = ..()
	if(glowyeyes)
		add_eyes()
