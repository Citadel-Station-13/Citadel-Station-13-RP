...
/mob/living/simple_mob/hostile/malf_drone/Life()
	...
	if(prob(1))
		src.visible_message("<span class='danger'>[bicon(src)] [src] shudders and shakes as some of it's damaged systems come back online.</span>")
	...
	if(prob(disabled ? 0 : 1))
		if(hostile)
			...
			src.visible_message("<span class='danger'>[bicon(src)] [src] suddenly lights up, and additional targetting vanes slide into place.</span>")
			hostile = 1
	...
	if(exploding && prob(20))
		if(prob(50))
			src.visible_message("<span class='danger'>[bicon(src)] [src] begins to spark and shake violenty!</span>")
		else
			src.visible_message("<span class='danger'>[bicon(src)] [src] sparks and shakes like it's about to explode!</span>")
		var/datum/effect_system/spark_spread/s = new /datum/effect_system/spark_spread
		s.set_up(3, 1, src)
		s.start()
