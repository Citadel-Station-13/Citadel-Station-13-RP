...
/mob/living/simple_mob/animal/borer/verb/secrete_chemicals()
	...
	to_chat(src, "<span class='danger'><B>You squirt a measure of [chem] from your reservoirs into [host]'s bloodstream.</B></span>")
...
/mob/living/simple_mob/animal/borer/verb/dominate_victim()
	...
	to_chat(src, "<span class='danger'>You focus your psychic lance on [M] and freeze their limbs with a wave of terrible dread.</span>")
	to_chat(M, "<span class='danger'>You feel a creeping, horrible sense of dread come over you, freezing your limbs and setting your heart racing.</span>")
	M.Weaken(10)
...
/mob/living/simple_mob/animal/borer/verb/bond_brain()
	...
	spawn(100+(host.brainloss*5))
		...
		if(!host || !src || controlling)
			...
			to_chat(src, "<span class='danger'><B>You plunge your probosci deep into the cortex of the host brain, interfacing directly with their nervous system.</B></span>")
			to_chat(host, "<span class='danger'><B>You feel a strange shifting sensation behind your eyes as an alien consciousness displaces yours.</B></span>")
