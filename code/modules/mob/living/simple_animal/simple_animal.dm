...
/mob/living/simple_mob/proc/handle_atmos()
	...
	if(istype(A,/turf))
		...
		if(Environment)
			...
			if(min_oxy)
				if(Environment.gases[/datum/gas/oxygen] < min_oxy)
					atmos_unsuitable = 1
			if(max_oxy)
				if(Environment.gases[/datum/gas/oxygen] > max_oxy)
					atmos_unsuitable = 1
			if(min_tox)
				if(Environment.gases[/datum/gas/phoron] < min_tox)
					atmos_unsuitable = 2
			if(max_tox)
				if(Environment.gases[/datum/gas/phoron] > max_tox)
					atmos_unsuitable = 2
			if(min_n2)
				if(Environment.gases[/datum/gas/nitrogen] < min_n2)
					atmos_unsuitable = 1
			if(max_n2)
				if(Environment.gases[/datum/gas/nitrogen] > max_n2)
					atmos_unsuitable = 1
			if(min_co2)
				if(Environment.gases[/datum/gas/carbon_dioxide] < min_co2)
					atmos_unsuitable = 1
			if(max_co2)
				if(Environment.gases[/datum/gas/carbon_dioxide] > max_co2)
					atmos_unsuitable = 1
