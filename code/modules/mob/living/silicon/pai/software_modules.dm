...
/datum/pai_software/ui_state(mob/user)
	return GLOB.always_state

/datum/pai_software/ui_status(mob/user)
...
/datum/pai_software/directives/ui_data(mob/living/silicon/pai/user, datum/tgui/ui, datum/ui_state/state)
	var/list/data = list()
...
/datum/pai_software/directives/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	if(action == "getdna")
		...
		if(answer == "Yes")
			...
			if(dna.unique_enzymes == P.master_dna)
				to_chat(P, SPAN_BOLD("DNA is a match to stored Master DNA."))
			else
				to_chat(P, SPAN_BOLD("DNA does not match stored Master DNA."))
		else
...
/datum/pai_software/crew_manifest/ui_data(mob/user, datum/tgui/ui, datum/ui_state/state)
	var/list/data = ..()
	if(data_core)
...
/datum/pai_software/med_records/ui_data(mob/living/silicon/pai/user, datum/tgui/ui, datum/ui_state/state)
	var/list/data = ..()
	...
	if(action == "select")
		...
		if(record)
			...
			if (!( data_core.general.Find(R) ))
				P.medical_cannotfind = TRUE
			else
				P.medical_cannotfind = FALSE
...
/datum/pai_software/sec_records/ui_data(mob/living/silicon/pai/user, datum/tgui/ui, datum/ui_state/state)
	var/list/data = ..()
	...
	if(action == "select")
		...
		if(record)
			...
			if (!( data_core.general.Find(R) ))
				...
				P.security_cannotfind = TRUE
			else
				P.security_cannotfind = FALSE
...
/datum/pai_software/door_jack/ui_data(mob/living/silicon/pai/user, datum/tgui/ui, datum/ui_state/state)
	var/list/data = ..()
...
/datum/pai_software/door_jack/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	var/mob/living/silicon/pai/P = usr
	if(!istype(P) || ..())
		return TRUE
	...
	switch(action)
		...
		if("cable")
			...
			for(var/mob/M in viewers(T))
				M.show_message(\
					SPAN_WARNING("\A port on [P] opens to reveal [P.cable], which promptly falls to the floor."), 3,
...
/datum/pai_software/atmosphere_sensor/ui_data(mob/user, datum/tgui/ui, datum/ui_state/state)
	var/list/data = ..()
...
/datum/pai_software/signaller/ui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Signaler", name, parent_ui)
		ui.open()

/datum/pai_software/signaller/ui_data(mob/living/silicon/pai/user, datum/tgui/ui, datum/ui_state/state)
	var/list/data = ..()

	var/obj/item/integated_radio/signal/R = user.sradio
...
/datum/pai_software/signaller/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	if(..())
		return TRUE
