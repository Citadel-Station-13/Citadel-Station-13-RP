...
/mob/living/silicon/pai/proc/pai_network_change()
	...
	for (var/obj/machinery/camera/C in Cameras)
		if(!C.status)
			...
			if(C.network != "CREED" && C.network != "thunder" && C.network != "RD" && C.network != "spicy purple" && C.network != "Prison") COMPILE ERROR! This will have to be updated as camera.network is no longer a string, but a list instead
