...
/obj/item/robotanalyzer/proc/do_scan(mob/living/M as mob, mob/living/user as mob)
	...
	switch(scan_type)
		if("robot")
			...
			user.show_message("\t Key: <font color='#FFA500'>Electronics</font>/<span class='danger'>Brute</span>", 1)
			user.show_message("\t Damage Specifics: <font color='#FFA500'>[BU]</font> - <span class='danger'>[BR]</span>")
			...
			if(length(damaged)>0)
				for(var/datum/robot_component/org in damaged)
					...
					capitalize(org.name),					\
					(org.installed == -1)	?	"<span class='danger'><b>DESTROYED</b></span> "							:"",\
					(org.electronics_damage > 0)	?	"<font color='#FFA500'>[org.electronics_damage]</font>"	:0,	\
					(org.brute_damage > 0)	?	"<span class='danger'>[org.brute_damage]</span>"							:0,		\
					(org.toggled)	?	"Toggled ON"	:	"<span class='danger'>Toggled OFF</span>",\
					(org.powered)	?	"Power ON"		:	"<span class='danger'>Power OFF</span>"),1)
			else
		...
		if("prosthetics")
			...
			to_chat(user, "Key: <font color='#FFA500'>Electronics</font>/<span class='danger'>Brute</span>")
			...
			if(H.internal_organs.len)
				for(var/obj/item/organ/external/E in H.organs)
					if(!(E.robotic >= ORGAN_ROBOT))
						continue
					organ_found = 1
					to_chat(user, "[E.name]: <span class='danger'>[E.brute_dam]</span> <font color='#FFA500'>[E.burn_dam]</font>")
			if(!organ_found)
			...
			if(H.internal_organs.len)
				for(var/obj/item/organ/O in H.internal_organs)
					if(!(O.robotic >= ORGAN_ROBOT))
						continue
					organ_found = 1
					to_chat(user, "[O.name]: <span class='danger'>[O.damage]</span>")
			if(!organ_found)
				to_chat(user, "No prosthetics located.")
