...
/mob/living/silicon/ai/show_system_integrity()
	. = list()
	if(!src.stat)
		INJECT_STATPANEL_DATA_LINE(., "Hardware integrity: [hardware_integrity()]%")
		INJECT_STATPANEL_DATA_LINE(., "Internal capacitor: [backup_capacitor()]%")
	else
...
/mob/living/silicon/ai/show_malf_ai()
	...
	if(src.is_malf())
		if(src.hacked_apcs)
			INJECT_STATPANEL_DATA_LINE(., "Hacked APCs: [src.hacked_apcs.len]")
		INJECT_STATPANEL_DATA_LINE(., "System Status: [src.hacking ? "Busy" : "Stand-By"]")
		if(src.research)
			INJECT_STATPANEL_DATA_LINE(., "Available CPU: [src.research.stored_cpu] TFlops")
			INJECT_STATPANEL_DATA_LINE(., "Maximal CPU: [src.research.max_cpu] TFlops")
			INJECT_STATPANEL_DATA_LINE(., "CPU generation rate: [src.research.cpu_increase_per_tick * 10] TFlops/s")
			INJECT_STATPANEL_DATA_LINE(., "Current research focus: [src.research.focus ? src.research.focus.name : "None"]")
			if(src.research.focus)
				INJECT_STATPANEL_DATA_LINE(., "Research completed: [round(src.research.focus.invested, 0.1)]/[round(src.research.focus.price)]")
			if(system_override == 1)
				INJECT_STATPANEL_DATA_LINE(., "SYSTEM OVERRIDE INITIATED")
			else if(system_override == 2)
				INJECT_STATPANEL_DATA_LINE(., "SYSTEM OVERRIDE COMPLETED")
