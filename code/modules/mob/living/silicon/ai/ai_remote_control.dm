...
/mob/living/silicon/ai/proc/deploy_to_shell(var/mob/living/silicon/robot/target)
	...
	for(var/borgie in GLOB.available_ai_shells)
		...
		if(R.shell && !R.deployed && (R.stat != DEAD) && (!R.connected_ai || (R.connected_ai == src) )  && !(SSmapping.legacy_map_config.ai_shell_restricted && !(T.z in SSmapping.legacy_map_config.ai_shell_allowed_levels)) )	//VOREStation Edit: shell restrictions
			possible += R
