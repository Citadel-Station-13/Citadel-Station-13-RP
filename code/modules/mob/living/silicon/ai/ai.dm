...
/mob/living/silicon/ai/Initialize(mapload, datum/ai_laws/L, obj/item/mmi/B, safety = TRUE)
	...
	if(L)
		...
		laws = new SSmapping.legacy_map_config.default_law_type
...
/mob/living/silicon/ai/proc/ai_emergency_message()
	...
	if(emergency_message_cooldown)
		...
		return
	var/input = sanitize(input(usr, "Please choose a message to transmit to [SSmapping.legacy_map_config.boss_short] via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination.  Transmission does not guarantee a response. There is a 30 second delay before you may send another message, be clear, full and concise.", "To abort, send an empty message.", ""))
	if(!input)
		return
