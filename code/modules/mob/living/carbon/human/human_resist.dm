...
/mob/living/carbon/human/proc/escape_straight_jacket()
	...
	if(do_after(src, breakouttime, mobility_flags = MOBILITY_RESIST))
		if(!wear_suit)
			return
		visible_message(
...
/mob/living/carbon/human/proc/break_straight_jacket()
	...
	if(do_after(src, 20 SECONDS, mobility_flags = MOBILITY_RESIST))	// Same scaling as breaking cuffs, 5 seconds to 120 seconds, 20 seconds to 480 seconds.
