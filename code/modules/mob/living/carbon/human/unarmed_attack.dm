...
/datum/unarmed_attack/proc/apply_effects(var/mob/living/carbon/human/user,var/mob/living/carbon/human/target,var/armour,var/attack_damage,var/zone)
		...
		switch(zone) // strong punches can have effects depending on where they hit
			...
			if(BP_GROIN)
				if(!target.isSynthetic())
					target.visible_message(SPAN_WARNING("[target] looks like [TT.he] [TT.is] in pain!"), SPAN_WARNING("[(target.gender=="female") ? "Oh god that hurt!" : "Oh no, not your[pick("testicles", "crown jewels", "clockweights", "family jewels", "marbles", "bean bags", "teabags", "sweetmeats", "goolies")]!"]")) // """""""I see no easy way to fix this for non-organic or neuter characters.""""""" - original coder
	...
	if(user.species.infect_wounds)		//Creates a pre-damaged, pre-infected wound. As nasty as this code.
		if(prob(infected_wound_probability))
			...
			target.visible_message(SPAN_DANGER("<i>[user] [attack_message]</i>"))
