...
/mob/proc/update_mobility(blocked, forced)
	...
	if(HAS_TRAIT(src, TRAIT_MOBILITY_CAN_HOLD_BLOCKED))
		. &= ~MOBILITY_CAN_HOLD
	if(HAS_TRAIT(src, TRAIT_MOBILITY_CAN_MOVE_BLOCKED))
		. &= ~MOBILITY_CAN_MOVE
	if(HAS_TRAIT(src, TRAIT_MOBILITY_CAN_PICKUP_BLOCKED))
		. &= ~MOBILITY_CAN_PICKUP
	if(HAS_TRAIT(src, TRAIT_MOBILITY_CAN_PULL_BLOCKED))
		. &= ~MOBILITY_CAN_PULL
	if(HAS_TRAIT(src, TRAIT_MOBILITY_CAN_RESIST_BLOCKED))
		. &= ~MOBILITY_CAN_RESIST
	if(HAS_TRAIT(src, TRAIT_MOBILITY_CAN_STAND_BLOCKED))
		. &= ~MOBILITY_CAN_STAND
	if(HAS_TRAIT(src, TRAIT_MOBILITY_CAN_UI_BLOCKED))
		. &= ~MOBILITY_CAN_UI
	if(HAS_TRAIT(src, TRAIT_MOBILITY_CAN_STORAGE_BLOCKED))
