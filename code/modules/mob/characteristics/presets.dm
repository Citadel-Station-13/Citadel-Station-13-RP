GLOBAL_LIST_EMPTY(characteristic_presets)
 ...
 */
/proc/resolve_characteristic_preset(datum/characteristic_preset/typepath_or_instance)
	RETURN_TYPE(/datum/characteristic_preset)
	if(istype(typepath_or_instance))
		return typepath_or_instance
	. = GLOB.characteristic_presets[typepath_or_instance]
	if(!.)
		return (GLOB.characteristic_presets[typepath_or_instance] = (new typepath_or_instance))
