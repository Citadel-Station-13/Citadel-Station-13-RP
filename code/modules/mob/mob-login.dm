	...
	if(Configuration.get_entry(/datum/toml_config_entry/backend/logging/toggles/access))
		for(var/mob/M in GLOB.player_list)
			...
			if( M.key && (M.key != key) )
				...
				if(matches)
					if(M.client)
						message_admins((SPAN_BOLDANNOUNCE("Notice: ") + SPAN_PURPLE("<A href='?src=\ref[usr];priv_msg=[ckey]'>[key_name_admin(src)]</A> has the same [matches] as <A href='?src=\ref[usr];priv_msg=\ref[M]'>[key_name_admin(M)]</A>.")), 1)
						log_adminwarn("Notice: [key_name(src)] has the same [matches] as [key_name(M)].")
					else
						message_admins((SPAN_BOLDANNOUNCE("Notice: ") + SPAN_PURPLE("<A href='?src=\ref[usr];priv_msg=[ckey]'>[key_name_admin(src)]</A> has the same [matches] as [key_name_admin(M)] (no longer logged in).")), 1)
