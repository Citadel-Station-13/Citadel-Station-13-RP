...
/mob/new_player/proc/handle_player_polling()
	...
	if(SSdbcore.Connect())
		...
		var/datum/db_query/select_query = SSdbcore.RunQuery(
			"SELECT id, question FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_question")] WHERE [(isadmin? "" : "adminonly = false AND")] Now() BETWEEN starttime AND endtime",
			list()
		)
...
/mob/new_player/proc/poll_player(var/pollid = -1)
	...
	if(SSdbcore.Connect())
		...
		var/datum/db_query/select_query = SSdbcore.RunQuery(
			"SELECT starttime, endtime, question, pollytype, multiplechoiceoptions FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_question")] WHERE id = :id",
			list(
				"id" = "[pollid]"
			)
		)
		...
		switch(polltype)
			...
			if("OPTION")
				var/datum/db_query/voted_query = SSdbcore.RunQuery(
					"SELECT optionid FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_vote")] WHERE pollid = :id AND ckey = :ckey",
					list(
						"id" = "[pollid]",
						"ckey" = usr.ckey
					)
				...
				var/datum/db_query/options_query = SSdbcore.RunQuery(
					"SELECT id, text FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_option")] WHERE pollid = :id",
					list(
						"id" = pollid
					)
				)
			...
			if("TEXT")
				var/datum/db_query/voted_query = SSdbcore.RunQuery(
					"SELECT replytext FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_textreply")] WHERE pollid = :id AND ckey = :ckey",
					list(
						"id" = pollid,
						"ckey" = usr.ckey
					)
			...
			if("NUMVAL")
				var/datum/db_query/voted_query = SSdbcore.RunQuery(
					"SELECT o.text, v.rating FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_option")] o, [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_vote")] v WHERE o.pollid = :pid AND v.ckey = :ckey AND o.id = v.optionid",
					list(
						"pid" = pollid,
						"ckey" = usr.ckey
					)
				...
				if(!voted)	//Only make this a form if we have not voted yet
					...
					var/datum/db_query/option_query = SSdbcore.RunQuery(
						"SELECT id, text, minval, maxval, descmin, descmid, descmax FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_option")] WHERE pollid = :id",
						list(
							"id" = pollid
						)
					)
...
/mob/new_player/proc/vote_on_poll(var/pollid = -1, var/optionid = -1, var/multichoice = 0)
	...
	if(SSdbcore.Connect())
		...
		var/datum/db_query/select_query = SSdbcore.RunQuery(
			"SELECT starttime, endtime, question, polltype, multiplechoiceoptions FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_question")] WHERE id = :id AND Now() BETWEEN starttime AND endtime",
			list(
				"id" = pollid
			)
		)
		...
		var/datum/db_query/select_query2 = SSdbcore.RunQuery(
			"SELECT id FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_option")] WHERE id = :id AND pollid = :pollid",
			list(
				"id" = optionid,
				"pollid" = pollid
			)
		...
		var/datum/db_query/voted_query = SSdbcore.RunQuery(
			"SELECT id FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_vote")] WHERE pollid = :id AND ckey = :ckey",
			list(
				"id" = pollid,
				"ckey" = usr.ckey
			)
		...
		SSdbcore.RunQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_vote")] (id, datetime, pollid, optionid, ckey, ip, adminrank) VALUES (null, Now(), :poll, :option, :ckey, :addr, :rank)",
			list(
				"poll" = pollid,
				"option" = optionid,
				"ckey" = usr.ckey,
...
/mob/new_player/proc/log_text_poll_reply(var/pollid = -1, var/replytext = "")
	...
	if(SSdbcore.Connect())
		...
		var/datum/db_query/select_query = SSdbcore.RunQuery(
			"SELECT starttime, endtime, question, polltype FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_question")] WHERE id = :id AND Now() BETWEEN starttime AND endtime",
			list(
				"id" = pollid
			)
		)
		...
		var/datum/db_query/voted_query = SSdbcore.RunQuery(
			"SELECT id FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_textreply")] WHERE pollid = :id AND ckey = :ckey",
			list(
				"id" = pollid,
				"ckey" = usr.ckey
			)
		...
		SSdbcore.RunQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_textreply")] (id, datetime, pollid, ckey, ip, replytext, adminrank) VALUES (null, Now(), :pollid, :ckey, :addr, :reply, :rank)",
			list(
				"pollid" = pollid,
				"ckey" = usr.ckey,
				"addr" = usr.client.address,
...
/mob/new_player/proc/vote_on_numval_poll(var/pollid = -1, var/optionid = -1, var/rating = null)
	...
	if(SSdbcore.Connect())
		var/datum/db_query/select_query = SSdbcore.NewQuery(
			"SELECT starttime, endtime, question, polltype FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_question")] WHERE id = :pollid AND Now() BETWEEN starttime AND endtime",
			list("pollid" = pollid)
		)
		...
		var/datum/db_query/select_query2 = SSdbcore.RunQuery(
			"SELECT id FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_option")] WHERE id = :optionid AND pollid = :pollid",
			list(
				"optionid" = optionid,
				"pollid" = pollid
			)
		...
		var/datum/db_query/voted_query = SSdbcore.RunQuery(
			"SELECT id FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_vote")] WHERE optionid = :optionid AND ckey = :ckey",
			list(
		...
		SSdbcore.RunQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("poll_vote")] (id, datetime, pollid, optionid, ckey, ip, adminrank, rating) VALUES (null, Now(), :pollid, :optionid, :ckey, :address, :rank, :rating)",
			list(
