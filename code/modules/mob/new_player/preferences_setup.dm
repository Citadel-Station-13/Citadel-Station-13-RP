...
/datum/preferences/update_preview_icon() // Lines up and un-overlaps character edit previews. Also un-splits taurs.
	var/mob/living/carbon/human/dummy/mannequin/mannequin = get_mannequin(client_ckey)
	mannequin.delete_inventory(TRUE)
	if(regen_limbs) //if limbs are changed in a way that won't be regenerated by species changes (ie, prosthetics), regenerate them first
		var/datum/species/current_species = GLOB.all_species[species]
		current_species.create_organs(mannequin)
		regen_limbs = 0
	dress_preview_mob(mannequin)
	...
	var/icon/preview_icon = icon('icons/effects/128x72_vr.dmi', bgstate)
