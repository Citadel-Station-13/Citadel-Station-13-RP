		...
		if((H.species.flags & IS_PLANT) && (M.nutrition < 500))
			if(prob(15))
				...
				for (var/mob/V in viewers(src))
					V.show_message("<span class='danger'>[M] writhes in pain as [TM.his] vacuoles boil.</span>", 3, "<span class='danger'>You hear the crunching of leaves.</span>", 2)
			if(prob(35))
			//	for (var/mob/V in viewers(src)) //Public messages commented out to prevent possible metaish genetics experimentation and stuff. - Cheridan
			//		V.show_message("<span class='danger'>[M] is mutated by the radiation beam.</span>", 3, "<span class='danger'> You hear the snapping of twigs.</span>", 2)
				if(prob(80))
					randmutb(M)
					domutcheck(M,null)
				else
			...
			//	for (var/mob/V in viewers(src))
			//		V.show_message("<span class='danger'>[M] is singed by the radiation beam.</span>", 3, "<span class='danger'> You hear the crackle of burning leaves.</span>", 2)
