...
/obj/item/syringe_cartridge/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	if(syringe)
...
/obj/item/gun/launcher/syringe/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	if(next)
