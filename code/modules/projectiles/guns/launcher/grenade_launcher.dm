...
/obj/item/gun/launcher/grenade/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	pump(user)
...
/obj/item/gun/launcher/grenade/underslung/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	return
