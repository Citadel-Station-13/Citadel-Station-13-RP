...
/obj/item/gun/launcher/grenade/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	pump(user)
...
/obj/item/gun/launcher/grenade/underslung/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	return
