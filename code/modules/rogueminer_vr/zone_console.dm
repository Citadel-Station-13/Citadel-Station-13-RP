...
/obj/machinery/computer/roguezones/nano_ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	else if(shuttle_control.z in SSmapping.legacy_map_config.belter_docked_z)
		data["shuttle_location"] = "Landed"
		data["shuttle_at_station"] = 1
	else if(shuttle_control.z == SSmapping.legacy_map_config.belter_transit_z)
		data["shuttle_location"] = "In-transit"
		data["shuttle_at_station"] = 0
	else if(shuttle_control.z == SSmapping.legacy_map_config.belter_belt_z)
		data["shuttle_location"] = "Belt"
		data["shuttle_at_station"] = 0

	var/can_scan = 0
	if(chargePercent >= 100)	// Keep having weird problems with these in one 'if' statement
		if(shuttle_control && (shuttle_control.z in SSmapping.legacy_map_config.belter_docked_z))	// Even though I put them all in parens to avoid OoO problems...
	...
	data["can_recall_shuttle"] = (shuttle_control && (shuttle_control.z in SSmapping.legacy_map_config.belter_belt_z) && !curZoneOccupied)
...
/obj/machinery/computer/roguezones/proc/failsafe_shuttle_recall()
	...
	if (!(shuttle_control.z in SSmapping.legacy_map_config.belter_belt_z))
