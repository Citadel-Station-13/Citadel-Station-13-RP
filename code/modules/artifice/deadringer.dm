...
/obj/item/deadringer/proc/makeacorpse(var/mob/living/carbon/human/H)
	...
	if(H.get_equipped_item(SLOT_ID_GLOVES))
		...
		temp.disguise(c_type.type)
		ADD_TRAIT(temp, TRAIT_NODROP, HOLOGRAM_TRAIT)
	if(H.get_equipped_item(SLOT_ID_LEFT_EAR))
		temp = H.get_equipped_item(SLOT_ID_LEFT_EAR)
		corpse.equip_to_slot_or_del(new temp.type(corpse), SLOT_ID_LEFT_EAR)
		temp = corpse.get_equipped_item(SLOT_ID_LEFT_EAR)
		ADD_TRAIT(temp, TRAIT_NODROP, HOLOGRAM_TRAIT)
	if(H.get_equipped_item(SLOT_ID_GLASSES))
		...
		temp = corpse.get_equipped_item(SLOT_ID_GLASSES)
		var/obj/item/clothing/c_type = H.get_equipped_item(SLOT_ID_GLASSES)
		temp.disguise(c_type.type)
		ADD_TRAIT(temp, TRAIT_NODROP, HOLOGRAM_TRAIT)
