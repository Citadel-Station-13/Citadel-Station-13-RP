...
/turf/simulated/icerock //wall piece
	name = "icerock"
	icon = 'icons/turf/walls.dmi'
	icon_state = "icerock"
	oxygen = 0
...
/turf/simulated/icerock/ignore_mapgen
	ignore_mapgen = 1
...
/turf/simulated/icerock/floor/ignore_mapgen
	ignore_mapgen = 1

/turf/simulated/icerock/proc/make_floor()
	if(!density && !opacity)
		return
	density = 0
	opacity = 0
	...
	if(density)
		...
		for(var/direction in cardinal)
			...
			if(istype(T) && !T.density)
				add_overlay(get_cached_border("icerock_side",direction,icon,"icerock_side"))
	...
	//We are an ice floor
		...
		if(update_neighbors)
			for(var/direction in alldirs)
				if(istype(get_step(src, direction), /turf/simulated/icerock))
					var/turf/simulated/icerock/M = get_step(src, direction)
					M.update_icon()

/turf/simulated/icerock/ex_act(severity)
...
/turf/simulated/icerock/attackby(obj/item/weapon/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/weapon/pickaxe))
			...
			if(finds && finds.len)
				...
				if(newDepth > F.excavation_required) // Digging too deep can break the item. At least you won't summon a Balrog (probably)
					fail_message = ". <b>[pick("There is a crunching noise","[W] collides with some different rock","Part of the ice face crumbles away","Something breaks under [W]")]</b>"
...
/turf/simulated/icerock/proc/clear_ore_effects()
	for(var/obj/effect/mineral/M in contents)
		qdel(M)

/turf/simulated/icerock/proc/DropMineral()
	if(!mineral)
		return
	clear_ore_effects()
