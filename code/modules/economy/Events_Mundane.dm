...
/datum/event/mundane_news/announce()
	...
	var/author = "The "+using_map_legacy().starsys_name+" Times"
	var/channel = author
	...
	if(!body)
		...
		switch(event_type)
			if(RESEARCH_BREAKTHROUGH)
				...
				[using_map_legacy().company_name] declined to comment as to whether this could impinge on profits."
			...
			if(RESIGNATION)
				body = "[using_map_legacy().company_name] regretfully announces the resignation of [pick("Sector Admiral","Division Admiral","Ship Admiral","Vice Admiral")] [random_name(pick(MALE,FEMALE))]."
				if(prob(25))
			...
			if(GOSSIP)
				...
				if(prob(33))
					...
					"the decision to be dropped by a major sponsor","a disasterous interview on [using_map_legacy().starsys_name] Tonight")]."
			if(TOURISM)
				...
				The [using_map_legacy().starsys_name] Times is offering discount tickets for two to see [random_name(pick(MALE,FEMALE))] live in return for eyewitness reports and up to the minute coverage."
...
/datum/event/trivial_news/announce()
	...
	"[pick(SPECIES_UNATHI,"Spacer")] gets tattoo of "+using_map_legacy().starsys_name+" on chest '[pick("[using_map_legacy().boss_short]","star","starship","asteroid")] tickles most'.",\
	"Skrell marries computer; wedding attended by 100 modems.",\
	"Chef reports successfully using harmonica as cheese grater.",\
	"[using_map_legacy().company_name] invents handkerchief that says 'Bless you' after sneeze.",\
	...
	"Survey: 'Cheese Louise' Voted Best Pizza Restaurant In [using_map_legacy().starsys_name]",\
