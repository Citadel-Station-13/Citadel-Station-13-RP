...
/obj/machinery/computer/transhuman/resleeving/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	for(var/obj/machinery/transhuman/synthprinter/spod in spods)
		spods_list_ui[++spods_list_ui.len] = list("spod" = spod, "steel" = spod.stored_material[MATERIAL_ID_STEEL], "glass" = spod.stored_material[MATERIAL_ID_GLASS])
		...
		if(istype(active_br))
			...
			//We have the machines. We can rebuild them. Probably.
				...
				if(active_br.synthetic)
					...
					else if(spod.stored_material[MATERIAL_ID_STEEL] < spod.body_cost)
						temp = "Error: Not enough [SSmaterials.material_display_name_by_id(MATERIAL_ID_STEEL)] in SynthFab."
					else if(spod.stored_material[MATERIAL_ID_GLASS] < spod.body_cost)
