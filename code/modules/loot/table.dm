 ...
 */
/datum/prototype/simple/loot_table
	anonymous = TRUE
	anonymous_namespace = "LootTable"

	/// paths of either /atom/movable's or /datum/prototype/simple/loot_pack's to chance
 ...
 */
/datum/prototype/simple/loot_table/proc/draw(amount)
	if(amount == 1)
 ...
 */
/datum/prototype/simple/loot_table/proc/draw_and_resolve(amount)
	. = list()
	var/list/drawn = draw(amount)
	for(var/thing in drawn)
		if(ispath(thing, /datum/prototype/simple/loot_pack) || istext(thing))
			var/datum/prototype/simple/loot_pack/resolved = SSrepository.fetch(thing)
...
/datum/prototype/simple/loot_table/proc/cache_tally()
	. = 0
	for(var/thing in contents)
		. += contents[thing] || 1
	cached_tally = .

/datum/prototype/simple/loot_table/proc/draw_single()
...
/datum/prototype/simple/loot_table/proc/draw_multi(amt)
	if(amt <= 5)
 ...
 * spawn contents at
 */
/datum/prototype/simple/loot_table/proc/instantiate(atom/location, amt)
	...
	for(var/thing in got)
		...
		else if(ispath(thing, /datum/prototype/simple/loot_pack) || istext(thing))
			var/datum/prototype/simple/loot_pack/pack = SSrepository.fetch(thing)
			if(!pack)
