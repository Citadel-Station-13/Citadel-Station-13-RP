...
/datum/pipeline/proc/temperature_interact(turf/target, share_volume, thermal_conductivity)
	...
	if(istype(target, /turf/simulated))
		...
		if(modeled_location.special_temperature)//First do special interactions then the usuall stuff
			CACHE_VSC_PROP(atmos_vsc, /atmos/hepipes/thickness, thickness)
			CACHE_VSC_PROP(atmos_vsc, /atmos/hepipes/thermal_conductivity, thermal_conductivity_setting)
			var/delta_temp = modeled_location.special_temperature - air.temperature
			//assuming aluminium with thermal conductivity 235 W / (m * K), Copper (400), Silver (430), steel (50), gold (320)
			var/heat_gain = thermal_conductivity_setting / (thickness * delta_temp)//assuming 1 cm wall thickness
