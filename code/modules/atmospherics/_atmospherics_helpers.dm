...
/proc/scrub_gas(var/obj/machinery/M, var/list/filtering, var/datum/gas_mixture/source, var/datum/gas_mixture/sink, var/total_transfer_moles = null, var/available_power = null)
	...
	filtering = filtering & source.gases	//only filter gasses that are actually there. DO NOT USE &=
	...
	for (var/g in filtering)
		var/ratio = source.gases[g]/total_filterable_moles //this converts the specific power per mole of pure gas to specific power per mole of scrubbed gas
	...
	var/power_draw = 0
	for (var/g in filtering)
		var/transfer_moles = source.gases[g]
		//filter gas in proportion to the mole ratio
		transfer_moles = min(transfer_moles, total_transfer_moles*(source.gases[g]/total_filterable_moles))
...
/proc/filter_gas(var/obj/machinery/M, var/list/filtering, var/datum/gas_mixture/source, var/datum/gas_mixture/sink_filtered, var/datum/gas_mixture/sink_clean, var/total_transfer_moles = null, var/available_power = null)
	...
	for (var/g in source.gases)
		...
		if (g in filtering)
			...
			total_unfilterable_moles += source.gases[g]

		var/ratio = source.gases[g]/source.total_moles //converts the specific power per mole of pure gas to specific power per mole of input gas mix
		total_specific_power += specific_power_gas[g]*ratio
	...
	for (var/g in removed.gases)
		var/power_used = specific_power_gas[g]*removed.gases[g]

		if (g in filtering)
			//use update=0. All the filtered gasses are supposed to be added simultaneously, so we update after the for loop.
			sink_filtered.adjust_gas_temp(g, removed.gases[g], removed.temperature, update=0)
			removed.adjust_gas(g, -removed.gases[g], update=0)
...
/proc/filter_gas_multi(var/obj/machinery/M, var/list/filtering, var/datum/gas_mixture/source, var/datum/gas_mixture/sink_clean, var/total_transfer_moles = null, var/available_power = null)
	...
	for (var/g in removed.gases)
		...
		if (g in filtering)
			var/datum/gas_mixture/sink_filtered = filtering[g]
			//use update=0. All the filtered gasses are supposed to be added simultaneously, so we update after the for loop.
			sink_filtered.adjust_gas_temp(g, removed.gases[g], removed.temperature, update=1)
			removed.adjust_gas(g, -removed.gases[g], update=0)
			if (power_used)
