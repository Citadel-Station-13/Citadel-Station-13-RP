...
/datum/data/pda/app/status_display/ui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	if(..())
		return TRUE
	switch(action)
		if("Status")
...
/datum/data/pda/app/signaller/ui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	if(..())
		return TRUE
	if(pda.cartridge && istype(pda.cartridge.radio, /obj/item/radio/integrated/signal))
		var/obj/item/radio/integrated/signal/R = pda.cartridge.radio
...
/datum/data/pda/app/power/update_ui(mob/user as mob, list/data)
	data.Add(power_monitor.ui_data(user))

/datum/data/pda/app/power/ui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	if(..())
		return TRUE
	if(power_monitor.ui_act(action, params, ui, state))
		return TRUE
	switch(action)
		if("Back")
...
/datum/data/pda/app/crew_records/ui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	if(..())
		return TRUE
	switch(action)
		if("Records")
