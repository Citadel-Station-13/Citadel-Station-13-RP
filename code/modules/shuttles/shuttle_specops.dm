...
/datum/shuttle/autodock/ferry/specops/launch(var/user)
	...
	if (istype(user, /obj/machinery/computer))
		...
		if(world.time <= reset_time)
			C.visible_message("<span class='notice'>[using_map_legacy().boss_name] will not allow the Special Operations shuttle to launch yet.</span>")
	...
	spawn(2 SECONDS)
		if (!location)
			...
			for(var/turf/T in get_area_turfs(shuttle_area))
				...
				to_chat(M, "<span class='danger'>You have arrived at [using_map_legacy().boss_name]. Operation has ended!</span>")
		else
