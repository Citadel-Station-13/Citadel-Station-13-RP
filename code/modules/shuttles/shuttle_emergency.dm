	...
	if (current_location == landmark_station)	// Leaving the station
		spawn(0)
			...
			if (SSemergencyshuttle.evac)
				priority_announcement.Announce(replacetext(replacetext(SSmapping.legacy_map_config.emergency_shuttle_leaving_dock, "%dock_name%", "[SSmapping.legacy_map_config.dock_name]"),  "%ETA%", "[estimated_time] minute\s"))
			else
				priority_announcement.Announce(replacetext(replacetext(SSmapping.legacy_map_config.shuttle_leaving_dock, "%dock_name%", "[SSmapping.legacy_map_config.dock_name]"),  "%ETA%", "[estimated_time] minute\s"))
	..()
