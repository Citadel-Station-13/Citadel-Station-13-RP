...
/datum/tgui_module/admin_shuttle_controller/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("adminobserve")
			...
			else if(istype(S, /atom/movable/overmap_object/entity/visitable))
				var/atom/movable/overmap_object/entity/visitable/V = S
				var/client/C = usr.client
				if(!isobserver(usr))
					C.admin_ghost()
				spawn(2)
		...
		if("classicmove")
			...
			return TRUE
		if("overmap_control")
			var/atom/movable/overmap_object/entity/visitable/ship/V = locate(params["ref"])
			if(istype(V))
