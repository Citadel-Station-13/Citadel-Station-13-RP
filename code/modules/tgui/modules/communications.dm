...
/datum/tgui_module/communications/ui_interact(mob/user, datum/tgui/ui)
	if(SSmapping.legacy_map_config && !(get_z(user) in SSmapping.legacy_map_config.contact_levels))
...
/datum/tgui_module/communications/ui_data(mob/user)
	...
	data["boss_short"] = SSmapping.legacy_map_config.boss_short
...
/datum/tgui_module/communications/ui_act(action, params)
	if(..())
		return TRUE
	if(SSmapping.legacy_map_config && !(get_z(usr) in SSmapping.legacy_map_config.contact_levels))
		...
		if("MessageCentCom")
			if(is_authenticated(usr) == COMM_AUTHENTICATION_MAX)
				if(centcomm_message_cooldown > world.time)
					...
					return
				var/input = sanitize(input("Please choose a message to transmit to [SSmapping.legacy_map_config.boss_short] via quantum entanglement. \
				...
				log_game("[key_name(usr)] has made an IA [SSmapping.legacy_map_config.boss_short] announcement: [input]")
...
/proc/call_shuttle_proc(var/mob/user)
	...
	if(deathsquad.deployed)
		to_chat(user, "[SSmapping.legacy_map_config.boss_short] will not allow the shuttle to be called. Consider all contracts terminated.")
		return
	...
	if(emergency_shuttle.going_to_centcom())
		to_chat(user, "The emergency shuttle may not be called while returning to [SSmapping.legacy_map_config.boss_short].")
		return
...
/proc/init_shift_change(var/mob/user, var/force = 0)
	...
	if(emergency_shuttle.going_to_centcom())
		to_chat(user, "The shuttle may not be called while returning to [SSmapping.legacy_map_config.boss_short].")
		return
	...
	if(!force)
		if(emergency_shuttle.deny_shuttle)
			to_chat(user, "[SSmapping.legacy_map_config.boss_short] does not currently have a shuttle available in your sector. Please try again later.")
			return
