...
/datum/tgui_module/communications/ui_interact(mob/user, datum/tgui/ui)
	if(using_map_legacy() && !(get_z(user) in using_map_legacy().contact_levels))
...
/datum/tgui_module/communications/ui_data(mob/user)
	...
	data["boss_short"] = using_map_legacy().boss_short
		...
		if("MessageCentCom")
			if(is_authenticated(usr) == COMM_AUTHENTICATION_MAX)
				if(centcomm_message_cooldown > world.time)
					...
					return
				var/input = sanitize(input(usr, "Please choose a message to transmit to [using_map_legacy().boss_short] via quantum entanglement. \
