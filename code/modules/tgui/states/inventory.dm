...
/datum/ui_state/glasses_state/can_use_topic(src_object, mob/user)
	if(ishuman(user))
...
/datum/ui_state/nif_state/can_use_topic(src_object, mob/user)
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
...
/datum/ui_state/nif_main_state/can_use_topic(src_object, mob/user)
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		...
		if(H.nif.stat == NIF_WORKING)
			...
			return min(user.shared_ui_interaction(src_object), UI_UPDATE)

	return UI_CLOSE

GLOBAL_DATUM_INIT(ui_commlink_state, /datum/ui_state/commlink_state, new)
/datum/ui_state/commlink_state/can_use_topic(src_object, mob/user)
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
