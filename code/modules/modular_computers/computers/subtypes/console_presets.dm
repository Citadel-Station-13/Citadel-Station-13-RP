...
/obj/machinery/modular_computer/console/preset/Initialize(mapload)
	...
	if(_has_printer)
		cpu.install_component(new /obj/item/computer_hardware/nano_printer)
	if(_has_battery)
		cpu.install_component(new /obj/item/computer_hardware/battery(cpu, /obj/item/cell/computer/super))
	// if(_has_ai)
...
// ===== SYSMIN CONSOLE =====
/obj/machinery/modular_computer/console/preset/research_sysmin
	console_department = "Research" // r&d holds ntnet controls
	name = "research network administrator console"
	desc = "A stationary computer. This one comes preloaded with ntnet administrator programs."

/obj/machinery/modular_computer/console/preset/research_sysmin/install_programs()
	var/obj/item/computer_hardware/hard_drive/hard_drive = cpu.all_components[MC_HDD]
	hard_drive.store_file(new/datum/computer_file/program/ntnetmonitor())
...
/obj/machinery/modular_computer/console/preset/cargochat/cargo/LateInitialize()
	. = ..()
	var/datum/ntnet_conversation/cargochat = ntnet_global.get_chat_channel_by_id(chatprogram.active_channel)
