...
/obj/item/modular_computer
	...
	//! old vars
	/// Emag check instead of modern bflag check on emags
...
/obj/item/modular_computer/MouseDrop(obj/over_object, src_location, over_location)
	var/mob/M = usr
	if((!istype(over_object, /atom/movable/screen)) && usr.canUseTopic(src, TRUE))
...
/obj/item/modular_computer/emag_act(remaining_charges, mob/user)
	...
	if(!enabled)
		...
		return FALSE
	computer_emagged = TRUE //Mostly for consistancy purposes; the programs will do their own emag handling
	var/newemag = FALSE
...
/obj/item/modular_computer/examine(mob/user)
	. = ..()
	if(integrity <= integrity_failure * integrity_max)
		. += SPAN_DANGER("It is heavily damaged!")
	else if(integrity < integrity_max)
...
/obj/item/modular_computer/proc/turn_on(mob/user)
	...
	var/obj/item/computer_hardware/recharger/tesla_link/recharger = all_components[MC_CHARGE]
	if(recharger)
		recharger.enabled = 1
...
/obj/item/modular_computer/proc/get_header_data()
	...
	if(recharger && recharger.enabled && recharger.check_functionality())
		//  && recharger.use_power(0) tesla thing magically pulls power FOR NOW
...
/obj/item/modular_computer/proc/set_flashlight_color(color)
	...
	set_light(comp_light_luminosity, 1, comp_light_color) // eugh, no set light color
	update_light()
	return TRUE
...
/obj/item/modular_computer/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(W.tool_behaviour == TOOL_WELDER)
		...
		// if(!W.tool_check(user, amount=1))
		// 	return

		to_chat(user, SPAN_NOTICE("You begin repairing damage to \the [src]..."))
		if(W.use_tool(src, user, 20, volume=50, cost=1))
