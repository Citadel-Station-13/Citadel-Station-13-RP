...
/datum/extension/interactive/ntos/extension_act(href, href_list, user)
	...
	if(href_list["PC_enable_component"] )
		var/obj/item/computer_hardware/H = locate(href_list["PC_enable_component"]) in holder
		if(H && istype(H) && !H.enabled)
			H.enabled = 1
			H.on_enable(src)
		return TOPIC_REFRESH
	if(href_list["PC_disable_component"] )
		var/obj/item/computer_hardware/H = locate(href_list["PC_disable_component"]) in holder
		if(H && istype(H) && H.enabled)
			H.enabled = 0
			H.on_disable()
		return TOPIC_REFRESH
...
/datum/extension/interactive/ntos/proc/get_header_data()
	...
	var/obj/item/computer_hardware/tesla_link/tesla_link = get_component(PART_TESLA)
