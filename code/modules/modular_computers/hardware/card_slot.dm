...
/obj/item/computer_hardware/card_slot/diagnostics()
	. = ..()
...
/obj/item/computer_hardware/card_slot/proc/verb_eject_id()
	set name = "Remove ID"
	set category = "Object"
	set src in view(1)
	...
	var/obj/item/computer_hardware/card_slot/device = src
	if (!istype(device))
...
/obj/item/computer_hardware/card_slot/proc/insert_id(var/obj/item/card/id/I, mob/user)
	if(!istype(I))
		return FALSE
...
/obj/item/computer_hardware/card_slot/attackby(obj/item/card/id/I, mob/living/user)
	if(!istype(I))
		return ..()
	insert_id(I, user)
	return TRUE
