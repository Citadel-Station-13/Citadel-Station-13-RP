...
/obj/item/computer_hardware/network_card/proc/get_signal(var/specific_action = 0)
	...
	if(holder2)
		...
		if(!istype(T)) //no reception in nullspace
			return 0
		if(T.z in SSmapping.legacy_map_config.station_levels)
			// Computer is on station. Low/High signal depending on what type of network card you have
			if(long_range)
				return 2
			else
				return 1
		if(T.z in SSmapping.legacy_map_config.contact_levels) //not on station, but close enough for radio signal to travel
