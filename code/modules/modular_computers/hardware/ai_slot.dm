...
/obj/item/computer_hardware/ai_slot/update_power_usage()
...
/obj/item/computer_hardware/ai_slot/verb/eject_ai(mob/user)
	set name = "Eject AI"
	set category = "Object"
	set src in view(1)
	...
	var/obj/item/computer_hardware/ai_slot/device = src
	if (!istype(device))
...
/obj/item/computer_hardware/ai_slot/proc/do_eject_ai(mob/user)
	if (stored_card)
		stored_card.dropInto(get_turf(loc))
	stored_card = null

	loc.verbs -= /obj/item/computer_hardware/ai_slot/verb/eject_ai

	update_power_usage()

/obj/item/computer_hardware/ai_slot/proc/do_insert_ai(mob/user, obj/item/aicard/card)
	if (stored_card)
		do_eject_ai(user)
	stored_card = card
