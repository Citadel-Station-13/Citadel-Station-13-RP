...
/obj/machinery/lapvend/proc/fabricate_and_recalc_price(fabricate = FALSE)
	...
	if(devtype == 1) // Laptop, generally cheaper to make it accessible for most station roles
		...
		switch(dev_battery)
			if(1) // Basic(750C)
				if(fabricate)
					battery_module.try_insert(new /obj/item/cell/computer)
			if(2) // Upgraded(1100C)
				if(fabricate)
					battery_module.try_insert(new /obj/item/cell/computer/advanced)
				total_price += 199
			if(3) // Advanced(1500C)
				if(fabricate)
					battery_module.try_insert(new /obj/item/cell/computer/super)
				total_price += 499
		switch(dev_disk)
			if(1) // Basic(128GQ)
				if(fabricate)
		...
		if(dev_apc_recharger)
			total_price += 399
			if(fabricate)
				fabricated_laptop.install_component(new /obj/item/computer_hardware/recharger/tesla_link)
		if(dev_printer)
			total_price += 99
			if(fabricate)
				fabricated_laptop.install_component(new /obj/item/computer_hardware/nano_printer)
		if(dev_card)
			total_price += 199
			if(fabricate)
		...
		switch(dev_battery)
			if(1) // Basic(300C)
				if(fabricate)
					battery_module.try_insert(new /obj/item/cell/computer/nano)
			if(2) // Upgraded(500C)
				if(fabricate)
					battery_module.try_insert(new /obj/item/cell/computer/micro)
				total_price += 199
			if(3) // Advanced(750C)
				if(fabricate)
					battery_module.try_insert(new /obj/item/cell/computer)
		...
		if(dev_printer)
			total_price += 99
			if(fabricate)
				fabricated_tablet.install_component(new/obj/item/computer_hardware/nano_printer)
		if(dev_card)
			total_price += 199
			if(fabricate)
...
/obj/machinery/lapvend/attackby(obj/item/I, mob/user)
	...
	if(istype(I, /obj/item/card/id))
		...
		if(account.security_level != 0) //If card requires pin authentication (ie seclevel 1 or 2)
			...
			account = attempt_account_access(ID.associated_account_number, attempt_pin, 2)
			if(!account)
				ping("Unable to access account: incorrect credentials.")
				return
		var/target_credits = total_price - credits
		if(!total_price > account.money)
			ping("Insufficient credits on card to purchase!")
			return
		account.money -= target_credits
		...
		ping("[target_credits] cr have been withdrawn from your account.")
		return
	return ..()
