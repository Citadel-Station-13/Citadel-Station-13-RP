...
/datum/computer_file/program/card_mod
	...
	///list of jobs (str) this current user can modify
...
/datum/computer_file/program/card_mod/proc/authenticate(mob/user, obj/item/card/id/id_card)
	...
	region_access = list()
	if(!target_dept && (ACCESS_COMMAND_CARDMOD in id_card.access))
		minor = FALSE
		authenticated = TRUE
	...
	if(length(head_types))
		for(var/j in SSjob.occupations)
			...
			for(var/head in head_types)
				if(SSjob.is_job_in_department(job, manager_lookup[head]))
	...
	switch(action)
		...
		if("PRG_eject")
			if(!computer || !card_slot2)
				return
			if(target_id_card)
				data_core.manifest_modify(target_id_card.registered_name, target_id_card.assignment, target_id_card.rank)
		...
		if("PRG_assign")
			...
			if(target == "Custom")
				...
				if(is_centcom)
					...
					var/datum/role/job/job
					for(var/jobtype in subtypesof(/datum/role/job))
...
/datum/computer_file/program/card_mod/ui_static_data(mob/user)
	...
	if(is_centcom)
		...
		departments = list(
			CARDCON_DEPARTMENT_COMMAND = SSjob.get_job_titles_in_department(DEPARTMENT_COMMAND),
			CARDCON_DEPARTMENT_ENGINEERING = SSjob.get_job_titles_in_department(DEPARTMENT_ENGINEERING), //GLOB.engineering_positions,
			CARDCON_DEPARTMENT_MEDICAL = SSjob.get_job_titles_in_department(DEPARTMENT_MEDICAL), //GLOB.medical_positions,
			CARDCON_DEPARTMENT_SCIENCE = SSjob.get_job_titles_in_department(DEPARTMENT_RESEARCH), //GLOB.science_positions,
			CARDCON_DEPARTMENT_SECURITY = SSjob.get_job_titles_in_department(DEPARTMENT_SECURITY), //GLOB.security_positions,
			CARDCON_DEPARTMENT_SUPPLY = SSjob.get_job_titles_in_department(DEPARTMENT_CARGO), //GLOB.supply_positions,
			CARDCON_DEPARTMENT_SERVICE = SSjob.get_job_titles_in_department(DEPARTMENT_CIVILIAN), //GLOB.civilian_positions
		)
	data["jobs"] = list()
