...
/datum/computer_file/program/signaler/proc/signal()
	...
	if(usr && T)
		...
		lastsignalers.Add(logging_data)
...
/datum/computer_file/program/signaler/proc/set_frequency(new_frequency)
	radio_controller.remove_object(src, signal_frequency)
	signal_frequency = new_frequency
	radio_connection = radio_controller.add_object(src, signal_frequency, RADIO_SIGNALER)
	return
