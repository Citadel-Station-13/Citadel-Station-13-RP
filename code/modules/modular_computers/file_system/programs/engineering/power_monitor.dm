...
/datum/computer_file/program/power_monitor
	...
	extended_desc = "This program localy connects to sensors to provide information about electrical systems"
...
/datum/computer_file/program/power_monitor/ui_data()
	...
	if(connected_powernet)
		data["supply"] = render_power(connected_powernet.viewavail)
		data["demand"] = render_power(connected_powernet.viewload)
		...
		for(var/obj/machinery/power/terminal/term in connected_powernet.nodes)
			...
			if(istype(A))
				data["areas"] += list(list(
					...
					"load" = render_power(A.lastused_total),
					"charging" = A.charging,
					"eqp" = A.equipment,
					"lgt" = A.lighting,
					"env" = A.environ
