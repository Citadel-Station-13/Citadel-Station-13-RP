...
/datum/nano_module/supermatter_monitor/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = default_state)
	...
	if(istype(active))
		...
		//data["SM_EPR"] = active.get_epr()
		if(air.total_moles)
			data["SM_gas_O2"] = round(100*air.gases[/datum/gas/oxygen]/air.total_moles,0.01)
			data["SM_gas_CO2"] = round(100*air.gases[/datum/gas/carbon_dioxide]/air.total_moles,0.01)
			data["SM_gas_N2"] = round(100*air.gases[/datum/gas/nitrogen]/air.total_moles,0.01)
			data["SM_gas_PH"] = round(100*air.gases[/datum/gas/phoron]/air.total_moles,0.01)
			data["SM_gas_N2O"] = round(100*air.gases[/datum/gas/nitrous_oxide]/air.total_moles,0.01)
		else
			data["SM_gas_O2"] = 0
			data["SM_gas_CO2"] = 0
			data["SM_gas_N2"] = 0
