...
/datum/ntnet/proc/add_log(var/log_string, var/obj/item/computer_hardware/network_card/source = null)
	...
	for(var/obj/machinery/ntnet_relay/R in ntnet_global.relays)
		var/obj/item/computer_hardware/hard_drive/portable/P = R.get_component_of_type(/obj/item/computer_hardware/hard_drive/portable)
		if(P)
...
/datum/ntnet/proc/rename_email(mob/user, old_login, desired_name, domain)
	...
	if(user.mind)
		...
		user.store_memory("Your email account address has been changed to [new_login].", /decl/memory_options/system)
	if(issilicon(user))
...
/datum/ntnet/proc/create_email(mob/user, desired_name, domain, assignment, desired_password)
	...
	if(find_email_by_name(login))
		...
		user.store_memory("You were not assigned an email address.", /decl/memory_options/system)
	else
		...
		if(user.mind)
			...
			user.store_memory("Your email account address is [EA.login] and the password is [EA.password].", /decl/memory_options/system)
		if(issilicon(user))
			var/mob/living/silicon/S = user
			var/datum/nano_module/email_client/my_client = S.get_subsystem_from_path(/datum/nano_module/email_client)
			if(my_client)
