...
/datum/disease2/effect/monkey/activate(var/mob/living/carbon/mob,var/multiplier)
	if(istype(mob,/mob/living/carbon/human))
		var/mob/living/carbon/human/h = mob
		if(prob(25))
			to_chat(mob, SPAN_WARNING(" You feel as if your body is trying to rapidly regress..."))
		if(prob(3))
			h.monkeyize()
	else
		to_chat(mob, SPAN_WARNING("You feel an odd attraction to consume banana's..."))
	...
	if(istype(mob, /mob/living/carbon/human))
		...
		if (O.robotic != ORGAN_ROBOT)
			if(prob(15))
				...
				to_chat(H, SPAN_WARNING("You feel like doom is coming.. you should head to medical!"))
...
/datum/disease2/effect/organ_repair/activate(var/mob/living/carbon/mob,var/multiplier)
	if (mob.reagents.get_reagent_amount(data) < 8)
		to_chat(mob, SPAN_NOTICE("You feel like your insides relaxed and you feel healthier after."))
