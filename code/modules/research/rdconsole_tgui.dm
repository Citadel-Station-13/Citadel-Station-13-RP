...
/obj/machinery/computer/rdconsole/ui_static_data(mob/user)
	...
	data["tech"] = ui_GetResearchLevelsInfo()
	data["designs"] = ui_GetDesignInfo(design_page)

	data["lathe_designs"] = ui_GetProtolatheDesigns(linked_lathe, builder_page)
	data["imprinter_designs"] = ui_GetImprinterDesigns(linked_imprinter, builder_page)
	...
	if(!locked && !busy_msg)
		...
		if(linked_destroy)
			data["info"]["linked_destroy"] = list(
				...
				"origin_tech" = ui_GetOriginTechForItem(linked_destroy.loaded_item),
			)
...
/obj/machinery/computer/rdconsole/proc/ui_GetResearchLevelsInfo()
	var/list/data = list()
...
/obj/machinery/computer/rdconsole/proc/ui_GetOriginTechForItem(obj/item/I)
	if(!istype(I))
		return list()
...
/obj/machinery/computer/rdconsole/proc/ui_GetProtolatheDesigns(obj/machinery/r_n_d/protolathe/P, page)
	if(!istype(P))
		return list()
...
/obj/machinery/computer/rdconsole/proc/ui_GetImprinterDesigns(obj/machinery/r_n_d/circuit_imprinter/P, page)
	if(!istype(P))
		return list()
...
/obj/machinery/computer/rdconsole/proc/ui_GetDesignInfo(page)
	var/list/data = list()
	// For some reason, this is faster than direct access.
	var/list/known_designs = files.known_designs
	for(var/datum/design/D in known_designs)
