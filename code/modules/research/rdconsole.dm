...
/obj/machinery/computer/rdconsole/proc/SyncRDevices() //Makes sure it is properly sync'ed up with the devices attached to it (if any).
	for(var/obj/machinery/rnd/D in range(5, src)) //Originally 3, buffed to 5 - Werewolf
...
/obj/machinery/computer/rdconsole/Initialize(mapload)
	...
	if(!id)
		for(var/obj/machinery/rnd/server/centcom/S in machines)
				...
				if(src)
					for(var/obj/machinery/rnd/server/S in machines)
						...
						if((id in S.id_with_upload) || istype(S, /obj/machinery/rnd/server/centcom))
							...
							server_processed = 1
						if((id in S.id_with_download) && !istype(S, /obj/machinery/rnd/server/centcom))
