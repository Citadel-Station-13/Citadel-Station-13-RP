	...
	if (..())
		...
		if(W.is_crowbar())
			...
			if (do_after(usr, (100 * cur_coils) * W.toolspeed)) // More coils = takes longer to disassemble. It's complex so largest one with 5 coils will take 50s with a normal crowbar
				...
				to_chat(user, "<span class='danger'>You have disassembled the SMES cell!</span>")
				dismantle()
				return
		...
		// Superconducting Magnetic Coil - Upgrade the SMES
			...
			if (cur_coils < max_coils)
				...
				to_chat(user, "<span class='danger'>You can't insert more coils into this SMES unit!</span>")
