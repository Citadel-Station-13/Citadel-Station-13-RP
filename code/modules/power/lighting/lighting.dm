...
/obj/machinery/light_construct/attack_hand(mob/user, datum/event_args/clickchain/e_args)
	. = ..()
	if(.)
...
/obj/machinery/light/attack_hand(mob/user, datum/event_args/clickchain/e_args)
...
/obj/machinery/light/flamp/attack_hand(mob/user, datum/event_args/clickchain/e_args)
	if(lamp_shade)
		if(status == LIGHT_EMPTY)
			to_chat(user, "There is no [get_fitting_name()] in this light.")
			return
