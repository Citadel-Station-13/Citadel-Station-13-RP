...
/obj/effect/fusion_em_field/process()
	...
	if(uptake_gas && uptake_gas.total_moles)
		for(var/gasname in uptake_gas.gases)
			if(uptake_gas.gases[gasname]*10 > dormant_reactant_quantities[gasname])
				AddParticles(gasname, uptake_gas.gases[gasname]*10)
				uptake_gas.adjust_gas(gasname, -(uptake_gas.gases[gasname]), update=FALSE)
