...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if (W.is_crowbar() && opened)
				...
				if (has_electronics==1)
					...
					if ((machine_stat & BROKEN))
						user.visible_message(\
							SPAN_WARNING("[user.name] has broken the charred power control board inside [src.name]!"),\
		...
		cell = W
		user.visible_message(\
			SPAN_WARNING("[user.name] has inserted a power cell into [src.name]!"),\
		...
		if ((machine_stat & BROKEN) \
			...
			to_chat(user,SPAN_NOTICE("The [src.name] looks too sturdy to bash open with \the [W.name]."))
...
//		return
	...
	if(istype(user,/mob/living/carbon/human))
		...
		if(H.species.can_shred(H))
			...
			user.visible_message("<span call='warning'>[user.name] slashes at the [src.name]!</span>", SPAN_NOTICE("You slash at the [src.name]!"))
...
/obj/machinery/power/apc/proc/set_broken()
	...
	spawn(rand(2,5))
		src.visible_message(SPAN_WARNING("[src]'s screen flickers suddenly, then explodes in a rain of sparks and small debris!"))
		machine_stat |= BROKEN
		operating = 0
		update_icon()
		update()
