...
/obj/item/cell/slime_core/using_item_on(obj/item/using, datum/event_args/actor/clickchain/clickchain, clickchain_flags)
	...
	if(istype(using, join_type) && join_result)
		// TODO: sound
		// TODO: log
		var/obj/item/joined = new join_result
...
/obj/item/cell/slime_core/on_attack_hand(datum/event_args/actor/clickchain/clickchain, clickchain_flags)
	...
	if(split_result && clickchain.performer.is_holding_inactive(src))
		// TODO: sound
		// TODO: log
		var/obj/item/split1 = new split_result
		var/obj/item/split2 = new split_result
		clickchain.chat_feedback(SPAN_NOTICE("You squeeze through [src]'s middle, splitting it into two [split]\s."), target = src)
		qdel(src)
		// TODO: check telekinesis
...
/obj/item/cell/slime_core/eighth
	...
	self_recharge_amount = POWER_CELL_CAPACITY_SMALL * (5 / 6) / ((5 MINUTES) / (1 SECONDS))
...
/obj/item/cell/slime_core/fourth
	...
	self_recharge_amount = POWER_CELL_CAPACITY_WEAPON * (5 / 6) / ((5 MINUTES) / (1 SECONDS))
...
/obj/item/cell/slime_core/half
	...
	self_recharge_amount = POWER_CELL_CAPACITY_MEDIUM * (5 / 6) / ((5 MINUTES) / (1 SECONDS))
...
/obj/item/cell/slime_core/whole
	...
	self_recharge_amount = POWER_CELL_CAPACITY_LARGE * (5 / 6) / ((5 MINUTES) / (1 SECONDS))
