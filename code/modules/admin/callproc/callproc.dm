...
/client/proc/callproc()
	...
	set desc = "Call a proc on any datum in the server."
	set waitfor = FALSE
	callproc_blocking()
...
/proc/WrapAdminProcCall(datum/target, procname, list/arguments)
	...
	if(!is_remote_handler && current_caller && current_caller != user_identifier)
		if(!GLOB.AdminProcCallSpamPrevention[user_identifier])
			...
			GLOB.AdminProcCallSpamPrevention[user_identifier] = TRUE
			UNTIL(!GLOB.AdminProcCaller)
			to_chat(usr, SPAN_ADMINNOTICE("Running your proc"))
			GLOB.AdminProcCallSpamPrevention -= user_identifier
		else
			UNTIL(!GLOB.AdminProcCaller)
...
/client/proc/get_callproc_returnval(returnval,procname)
	...
	if(islist(returnval))
		...
		. += "<font color='blue'>[procname] returned an icon: <a href='?_src_=vars;[HrefToken()];Vars=[REF(returnval)]'>[REF(returnval)]</a> - [icon2html(I, src)] [I] ([I.type])</font>"
	else if(istype(returnval, /datum))
		var/datum/D = returnval
		. += "<font color='blue'>[procname] returned a datum: <a href='?_src_=vars;[HrefToken()];Vars=[REF(returnval)]'>[REF(returnval)]</a> - [D] ([D.type])</font>"
	else
