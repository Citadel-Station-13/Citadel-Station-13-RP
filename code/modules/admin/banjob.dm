...
/proc/jobban_loadbanfile()
	if(config_legacy.ban_legacy_system)
		...
		var/datum/db_query/query = SSdbcore.RunQuery(
			"SELECT ckey, job FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("ban")] WHERE bantype = 'JOB_PERMABAN' AND isnull(unbanned)",
			list()
		)
		...
		var/datum/db_query/query1 = SSdbcore.RunQuery(
			"SELECT ckey, job FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("ban")] WHERE bantype = 'JOB_TEMPBAN' AND isnull(unbanned) AND expiration_time > Now()",
			list()
		)
