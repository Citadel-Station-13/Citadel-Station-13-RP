...
/datum/tgs_chat_command/irccheck/Run(datum/tgs_chat_user/sender, params)
	...
	return "[clients.len] players on [using_map.name], Mode: [master_mode]; round [current_state] -- [server ? server : "[world.internet_address]:[world.port]"]"
...
/datum/tgs_chat_command/sdql/Run(datum/tgs_chat_user/sender, params)
	...
	AdminProcCaller = "CHAT_[sender.friendly_name]"	//_ won't show up in ckeys so it'll never match with a real admin
	var/list/results = world.SDQL2_query(params, AdminProcCaller, AdminProcCaller)
	AdminProcCaller = null
	if(!results)
