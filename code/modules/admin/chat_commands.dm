...
/datum/tgs_chat_command/irccheck/Run(datum/tgs_chat_user/sender, params)
	...
	return "[GLOB.clients.len] players on [using_map_legacy().name], Mode: [master_mode]; round [current_state] -- Duration [roundduration2text()] -- [server ? server : "[world.internet_address]:[world.port]"]"
