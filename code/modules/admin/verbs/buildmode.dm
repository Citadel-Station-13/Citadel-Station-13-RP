...
/obj/effect/bmode/buildhelp/Click()
	switch(master.cl.buildmode)
		if(1) // Basic Build
			...
			to_chat(usr, SPAN_NOTICE("Left Mouse Button        = Construct / Upgrade"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button       = Deconstruct / Delete / Downgrade"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button + ctrl = R-Window"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button + alt  = Airlock"))
			to_chat(usr, "")
			to_chat(usr, SPAN_NOTICE("Use the button in the upper left corner to"))
			to_chat(usr, SPAN_NOTICE("change the direction of built objects."))
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
		if(2) // Adv. Build
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on buildmode button = Set object type"))
			to_chat(usr, SPAN_NOTICE("Middle Mouse Button on buildmode button= On/Off object type saying"))
			to_chat(usr, SPAN_NOTICE("Middle Mouse Button on turf/obj        = Capture object type"))
			...
			to_chat(usr, SPAN_NOTICE("Mouse Button + ctrl                    = Copy object type"))
			to_chat(usr, "")
			to_chat(usr, SPAN_NOTICE("Use the button in the upper left corner to"))
			to_chat(usr, SPAN_NOTICE("change the direction of built objects."))
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
		...
		if(5) // Room Build
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button on turf              = Select as point A"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on turf             = Select as point B"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on buildmode button = Change floor/wall type"))
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
		if(6) // Make Ladders
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button on turf              = Set as upper ladder loc"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on turf             = Set as lower ladder loc"))
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
		if(7) // Move Into Contents
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button on turf/obj/mob      = Select"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on turf/obj/mob     = Move into selection"))
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
		if(8) // Make Lights
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button on turf/obj/mob      = Make it glow"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on turf/obj/mob     = Reset glowing"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on buildmode button = Change glow properties"))
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
		if(9) // Control mobs with ai_holders.
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button on AI mob            = Select/Deselect mob"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button + alt on AI mob      = Toggle hostility on mob"))
			to_chat(usr, SPAN_NOTICE("Left Mouse Button + ctrl on AI mob     = Reset target/following/movement"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on enemy mob        = Command selected mobs to attack mob"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on allied mob       = Command selected mobs to follow mob"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button + shift on any mob  = Command selected mobs to follow mob regardless of faction"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button on tile             = Command selected mobs to move to tile (will cancel if enemies are seen)"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button + shift on tile     = Command selected mobs to reposition to tile (will not be inturrupted by enemies)"))
			to_chat(usr, SPAN_NOTICE("Right Mouse Button + alt on obj/turfs  = Command selected mobs to attack obj/turf"))
			to_chat(usr, SPAN_NOTICE("***********************************************************"))
	return 1
