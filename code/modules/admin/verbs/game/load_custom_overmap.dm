...
ADMIN_VERB_DEF(load_custom_overmap, R_ADMIN, "Load Custom Overmap", "Load a custom overmap.", VERB_CATEGORY_GAME)
	...
	var/map = input(caller, "Select overmap .dmm", "Instantiate Overmap") as file|null
	if(!map)
		return
	...
	if(!parsed_map.parsed)
		tgui_alert(caller, "Failed to parse map.", "Parse Error")
		return
	...
	if(parsed_map.width >= max_x || parsed_map.height >= max_y)
		tgui_alert(caller, "Your map is too big for the current world size. Yours was [parsed_map.width]x[parsed_map.height], but maximum is: [max_x]x[max_y]", "Improper Dimensions")
		return
