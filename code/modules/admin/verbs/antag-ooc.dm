/client/proc/aooc(msg as text)
	...
	for(var/mob/M in GLOB.mob_list)
		if(check_rights(R_ADMIN|R_MOD, 0, M)) // Staff can see AOOC unconditionally, and with more details.
			to_chat(M, SPAN_OOC("<span class='aooc'>Antag-OOC: <EM>[get_options_bar(src, 0, 1, 1)]([admin_jump_link(usr, M.client.holder)]):</EM> <span class='message'><span class='linkify'>[msg]</span></span></span>"))
			...
			if((M.mind && M.mind.special_role && A && A.can_hear_aooc) || isobserver(M)) // Antags must have their type be allowed to AOOC to see AOOC.  This prevents, say, ERT from seeing AOOC.
				to_chat(M, SPAN_OOC("<span class='aooc'>Antag-OOC:<EM>[player_display]: </EM> <span class='message'><span class='linkify'>[msg]</sppan></span></span>"))
