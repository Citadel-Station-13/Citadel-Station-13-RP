...
/datum/admin_help/proc/MessageNoRecipient(msg, urgent = FALSE)
	...
	var/admin_msg = SPAN_ADMINNOTICE(SPAN_ADMINHELP("Ticket [TicketHref(SPAN_TOOLTIP("Open the ticket in a new window.", "#[id]"), ref_src)]</span><b>: [LinkedReplyName(ref_src)] [FullMonty(ref_src)]:</b> [SPAN_LINKIFY(keywords_lookup(msg))]"))
...
/datum/admin_help_ui_handler/proc/perform_adminhelp(client/user_client, message, urgent)
	...
	if(urgent)
		if(!TG_COOLDOWN_FINISHED(src, ahelp_cooldowns?[user_client.ckey]))
...
/proc/send2tgs_adminless_only(source, msg, requiredflags = R_BAN)
	...
	if(. <= 0)
		...
		if(!afkmins.len && !stealthmins.len && !powerlessmins.len)
			message = "[msg] - No admins online"
		else
			message = "[msg] - All admins stealthed\[[english_list(stealthmins)]\], AFK\[[english_list(afkmins)]\], or lacks +BAN\[[english_list(powerlessmins)]\]! Total: [allmins.len] "
		send2adminchat(source, message)
