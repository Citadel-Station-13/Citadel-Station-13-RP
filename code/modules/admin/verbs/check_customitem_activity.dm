...
/proc/populate_inactive_customitems_list(var/client/C)
	...
	if(ckeys_with_customitems.len)
		var/datum/db_query/query_inactive = dbcon.NewQuery("SELECT ckey, lastseen FROM [format_table_name("player")] WHERE datediff(Now(), lastseen) > 60")
	...
	if(ckeys_with_customitems.len)
		for(var/cur_ckey in ckeys_with_customitems)
			var/datum/db_query/query_inactive = dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE ckey = '[cur_ckey]'")
			query_inactive.Execute()
