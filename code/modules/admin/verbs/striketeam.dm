...
/client/proc/strike_team()
	...
	if(world.time < 6000)
		to_chat(usr, "<span class='danger'>There are [(6000-world.time)/10] seconds remaining before it may be called.</span>")
		return
	...
	if(team.deployed)
		to_chat(usr, "<span class='danger'>Someone is already sending a team.</span>")
		return
