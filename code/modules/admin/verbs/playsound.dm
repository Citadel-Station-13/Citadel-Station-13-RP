/client/proc/play_sound(S as sound)
	...
	for(var/mob/M in GLOB.player_list)
		if(M.get_preference_toggle(/datum/client_preference/play_admin_midis)) //if(M.client.prefs.toggles & SOUND_MIDI)
	...
	if(istext(web_sound_input))
		...
		if(web_sound_url || stop_web_sounds)
			for(var/m in GLOB.player_list)
				...
				if(M.get_preference_toggle(/datum/client_preference/play_admin_midis)) //if(C.prefs.toggles & SOUND_MIDI)
					if(!stop_web_sounds)
