...
/obj/item/clothing/on_inv_unequipped(datum/inventory/inventory, slot_id_or_index, inv_op_flags, datum/event_args/actor/actor)
	. = ..()
	...
	if(!(inv_op_flags & INV_OP_IS_ACCESSORY) && length(accessories))
		for(var/obj/item/acc as anything in accessories)
			acc.on_inv_unequipped(inventory, slot_id_or_index, inv_op_flags | INV_OP_IS_ACCESSORY, actor)
