...
/obj/item/clothing/accessory/proc/get_mob_overlay()
	...
	else if(wearer && sprite_sheets[wearer.species.get_bodytype_legacy(wearer)]) //Teshari can finally into webbing, too!
		mob_overlay = image("icon" = sprite_sheets[wearer.species.get_bodytype_legacy(wearer)], "icon_state" = "[tmp_icon_state]")
	else
...
/obj/item/clothing/accessory/collar/proc/update_collartag(mob/user, obj/item/I, var/erasemethod, var/erasing, var/writemethod)
	if(!(istype(user.get_active_held_item(),I)) || !(istype(user.get_inactive_hand(),src)) || (user.stat))
		return
