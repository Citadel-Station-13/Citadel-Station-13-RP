...
/obj/item/clothing/head/reindeer/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
...
/obj/item/clothing/head/cowl/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	if(src.icon_state == initial(icon_state))
