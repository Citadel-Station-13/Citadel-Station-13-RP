...
/obj/item/clothing/glasses/omnihud/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	if(!ishuman(user))
...
/obj/item/clothing/glasses/omnihud/eng/meson/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	if(!active)
