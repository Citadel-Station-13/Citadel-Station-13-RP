...
/obj/item/clothing/glasses/omnihud/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	if(!ishuman(user))
...
/obj/item/clothing/glasses/omnihud/eng/meson/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	if(!active)
