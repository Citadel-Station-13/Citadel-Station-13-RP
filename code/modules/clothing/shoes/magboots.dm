...
/obj/item/clothing/shoes/magboots/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	if(magpulse)
...
/obj/item/clothing/shoes/magboots/vox/attack_self(datum/event_args/actor/actor, mob/user)
	if(src.magpulse)
		clothing_flags &= ~NOSLIP
		magpulse = 0
