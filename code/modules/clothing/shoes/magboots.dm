...
/obj/item/clothing/shoes/magboots/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	if(magpulse)
...
/obj/item/clothing/shoes/magboots/vox/attack_self(mob/user, datum/event_args/actor/e_args)
	if(src.magpulse)
		clothing_flags &= ~NOSLIP
		magpulse = 0
