...
/obj/item/clothing/head/helmet/medieval/paladin/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
...
/obj/item/clothing/head/medievalfake/paladin/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	if(src.icon_state == initial(icon_state))
