...
/obj/item/clothing/suit/equipped(mob/user, slot, flags)
	...
	if(!taurized && slot == SLOT_ID_SUIT && ishuman(user))
		...
		if(isTaurTail(H.tail_style))
			...
			var/list/resolved = resolve_worn_assets(user, resolve_inventory_slot_meta(/datum/inventory_slot_meta/inventory/suit), FALSE, H.species.get_effective_bodytype(src, SLOT_ID_SUIT))
