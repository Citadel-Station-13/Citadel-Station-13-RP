...
/obj/item/clothing/mask/gas/half/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	adjust_mask(user)
...
/obj/item/clothing/mask/gas/swat/vox/attack_self(mob/user, datum/event_args/actor/e_args)
	. = ..()
	if(.)
		return
	feeding_port(user)
