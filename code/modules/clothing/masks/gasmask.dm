...
/obj/item/clothing/mask/gas/half/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	adjust_mask(user)
...
/obj/item/clothing/mask/gas/swat/vox/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	feeding_port(user)
