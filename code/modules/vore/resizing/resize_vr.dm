...
/mob/living/proc/inform_someone_you_just_stepped_over_them(mob/living/micro)
	...
	tail = ishuman(src)? ((H = src) && H.legacy_has_taur_tail() && H.tail_style) : null
...
/mob/lving/Crossed(atom/movable/AM, oldloc)
	...
	if(isliving(AM))
		...
		switch(fetish_content_check)
			...
			if(WE_RAN_BETWEEN_THEIR_LEGS)
				tail = ishuman(AM) && (H = AM) && H.legacy_has_taur_tail() && H.tail_style
			...
			if(THEY_RAN_BETWEEN_OUR_LEGS)
				tail = ishuman(src) && (H = src) && H.legacy_has_taur_tail() && H.tail_style
	...
	switch(a_intent)
		...
		if(INTENT_GRAB)
			...
			if((get_effective_size() - tmob.get_effective_size()) >= 0.50)
				...
				if(!H || (!H.legacy_has_taur_tail() && H.shoes))
				...
				else if(H && (!H.legacy_has_taur_tail() && !H.shoes))
