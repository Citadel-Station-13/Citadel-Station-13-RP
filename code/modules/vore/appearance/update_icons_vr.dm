...
/mob/living/carbon/human/proc/get_tail_image()
	...
	if(tail_style && !(wear_suit && wear_suit.flags_inv & HIDETAIL && !isTaurTail(tail_style)))
		var/icon/tail_s = new/icon("icon" = tail_style.icon, "icon_state" = (GLOB.taur_loafs[tail_style?.icon_state] && resting) ? "[tail_style]_loaf" : (wagging && tail_style.ani_state ? tail_style.ani_state : tail_style.icon_state))
