...
/obj/item/flag/process_mob_melee(mob/M, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
...
/obj/item/clothing/accessory/badge/holo/detective/ruda/process_mob_melee(mob/M, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
	if(user.a_intent == INTENT_HARM)
		return ..()
	. = CLICKCHAIN_DO_NOT_PROPAGATE
	if(isliving(user))
...
/obj/item/fluff/injector/monkey/process_mob_melee(mob/M, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
	if(user.a_intent == INTENT_HARM)
		return ..()
	. = CLICKCHAIN_DO_NOT_PROPAGATE
...
/obj/item/fluff/injector/numb_bite/process_mob_melee(mob/M, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
	if(user.a_intent == INTENT_HARM)
		return ..()
	. = CLICKCHAIN_DO_NOT_PROPAGATE
	if(usr == M) //Is the person using it on theirself?
