...
/obj/belly/proc/set_messages(var/raw_text, var/type, var/delim = "\n\n")
	...
	if(raw_list.len > 10)
		raw_list.Cut(11)
		log_debug(span_debug_warning("[owner] tried to set [lowertext(name)] with 11+ messages"))

	for(var/i = 1, i <= raw_list.len, i++)
		if(length(raw_list[i]) > 160 || length(raw_list[i]) < 10) //160 is fudged value due to htmlencoding increasing the size
			raw_list.Cut(i,i)
			log_debug(span_debug_warning("[owner] tried to set [lowertext(name)] with >121 or <10 char message"))
		else
