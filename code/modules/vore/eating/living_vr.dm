...
/mob/living
	...
	/// This creature subsists on a diet of pure adminium.
	var/adminbus_eat_minerals = FALSE
	/// Have they been bitten by a vamp?
	var/bitten = FALSE
	/// Do we get pain messages?
	var/painmsg = TRUE
...
/mob/living/proc/handle_eat_minerals(obj/item/snack, mob/living/user)
	...
	if(!I)
		var/sadnotice = pick("You look longingly at your empty hands, imagining if they held something edible...", "Why is the ore gone?")
		to_chat(feeder, SPAN_NOTICE("[sadnotice]")) // I just want to eat my ore :(
		return
	...
	if(istype(I, /obj/item/ore)) //Raw unrefined ore. Some things are just better untempered!
		...
		var/list/rock_munch = list(
			MAT_CARBON        = list("nutrition" = 15, "remark" = "Utterly bitter, crunching down on [O] only makes you long for better things. But a snack's a snack...", "WTF" = FALSE),
			...
			MAT_HEMATITE      = list("nutrition" = 15, "remark" = "The familiar texture and taste of [O] does the job but leaves little to the imagination and hardly sates your appetite.", "WTF" = FALSE),
			...
			MAT_PLATINUM      = list("nutrition" = 40, "remark" = "A bit tangy but elegantly balanced with a long faintly sour finish. Delectible.", "WTF" = FALSE),
			MAT_RUTILE        = list("nutrition" = 50, "remark" = "A little... angular, you savour the light but chewy [O], finishing it quickly.", "WTF" = FALSE),
			MAT_SAND          = list("nutrition" = 0,  "remark" = "You crunch on [O] but its texture is almost gag-inducing. Stifling a cough, you somehow manage to swallow both [O] and your regrets.", "WTF" = FALSE),
		...
		var/list/refined_taste = list(
			...
			MAT_OSMIUM            = list("nutrition" = 45,  "remark" = "Successive bites serve to almost chill your palate, a rush of rich and mildly sour flavour unlocked with the grinding of your powerful jaws. Delectible.", "WTF" = FALSE),
			...
			MAT_PHORONGLASS       = list("nutrition" = 10,  "remark" = "With a satisfying crunch, you grind [O] down with ease and find it somewhat palatable due to a subtle but familiar rush of phoronic warmth.", "WTF" = FALSE),
			MAT_PHORONRGLASS      = list("nutrition" = 15,  "remark" = "With a satisfying crunch, you grind [O] down. It is quite palatable due to a subtle metallic tang and familiar rush of phoronic warmth.", "WTF" = FALSE),
	...
	if(nom) //Ravenous 1-4, snackage confirmed. Clear for chowdown, over.
		...
		if(do_after(feeder, T, ignore_movement = TRUE)) //Eat on the move, but not multiple things at once.
			if(feeder != src)
		 ...
		 //If it ain't ore or the type of sheets we can eat, bugger off!
		to_chat(src, SPAN_NOTICE("You pause for a moment to examine [I] and realize it's not even worth the energy to chew."))
