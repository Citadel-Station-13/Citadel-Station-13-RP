...
/hook/roundend/proc/persist_locations()
	for(var/mob/Player in human_mob_list)
		if(!Player.mind || isnewplayer(Player))
			...
			persist_interround_data(Player, SSmapping.legacy_map_config.spawnpoint_died)
		else
			var/turf/playerTurf = get_turf(Player)
			if(isAdminLevel(playerTurf.z))
				// Evac'd - Next round they arrive on the shuttle.
				persist_interround_data(Player, SSmapping.legacy_map_config.spawnpoint_left)
			else
				// Stayed on station, go to dorms
				persist_interround_data(Player, SSmapping.legacy_map_config.spawnpoint_stayed)
	return 1
