...
/datum/lore/atc_controller/proc/msg(message, sender)
	ASSERT(message)
	GLOB.global_announcer.autosay("[message]", sender ? sender : "[using_map_legacy().dock_name] Control")

/datum/lore/atc_controller/proc/reroute_traffic(yes = TRUE)
	if(yes)
		if(!squelched)
			msg("Re-routing traffic and fleet patterns around [using_map_legacy().station_name].")
		squelched = TRUE
...
/datum/lore/atc_controller/proc/shift_ending(evac = FALSE)
	msg("[using_map_legacy().shuttle_name], this is [using_map_legacy().dock_name] Control, you are cleared to complete routine transfer from [using_map_legacy().station_name] to [using_map_legacy().dock_name].")
	sleep(5 SECONDS)
	if(QDELETED(src))
		return
	msg("[using_map_legacy().shuttle_name] departing [using_map_legacy().dock_name] for [using_map_legacy().station_name] on routine transfer route. Estimated time to arrival: ten minutes.","[using_map_legacy().shuttle_name]")
...
/datum/lore/atc_controller/proc/random_convo()
	...
	var/alt_atc_names = list("[using_map_legacy().dock_name] Flight Control","[using_map_legacy().dock_name] FliCon","[using_map_legacy().dock_name] System Control","[using_map_legacy().dock_name] Star Control","[using_map_legacy().dock_name] SysCon","[using_map_legacy().dock_name] Control","[using_map_legacy().dock_name] STC","[using_map_legacy().dock_name] StarCon")
	...
	*/
	...
	switch(using_map_legacy().dock_type)
	...
	switch(chatter_type)
		...
		if("fleettraffic")
			if(prob(90)) // Good & Neutral Fleet reports
				var/report = pick("All systems are clear and we're maintaining near sector patrol with the [using_map_legacy().station_name]",
					...
					"We have confirmed visuals of [rand(3,15)] Diona clinging onto the [using_map_legacy().dock_name] hull. We'll have reports of first contact with them in a moment",
					...
					"Our vessel is undergoining early transfer procedures for the crew and should be at the [using_map_legacy().dock_name] in roughly [pick("5","10","15")] minutes",
				...
				msg("This is the [combined_first_name] reporting in to the [using_map_legacy().dock_name] with fleet updates. [report].","[source_prefix] [source_shipname]")
				sleep(6 SECONDS)
				// To try and give a more organic response on Traffic Control
				if(prob(50))
					msg("This is [using_map_legacy().dock_name]. Report received [source_prefix] [source_shipname]. Continue operations as normal.")
				else
					msg("[source_prefix] [source_shipname], thank you for your report. Please proceed operations as normal. [using_map_legacy().dock_name], out.")
				...
				var/bad_report = pick("Recent cultist activity on the nearby planet we scouted has deemed the area unsafe for exploration and mining operations",
					...
					"A recent fight with pirates has left [pick("one","two")] of our decks de-pressurized. We're attempting to reach the [using_map_legacy().dock_name] for fleet repairs",
				...
				msg("Attention [using_map_legacy().dock_name], this is the [combined_first_name] with an urgent report for the fleet. [bad_report].","[source_prefix] [source_shipname]")
				sleep(5 SECONDS)
				if(prob(50))
					msg("Thank you for the update [source_prefix] [source_shipname]. We'll relay this to the needed Captains. [using_map_legacy().dock_name], out.")
				else
					msg("Understood [source_prefix] [source_shipname]. Keep us updated on any changes or developments.")
					sleep(5 SECONDS)
					msg("Will do [using_map_legacy().dock_name]. [source_shipname], over and out.","[source_prefix] [source_shipname]")
		...
		if("emerg")
			...
			sleep(5 SECONDS)
			msg("[combined_first_name], this is [using_map_legacy().dock_name] Control, copy. Switch to emergency responder channel [ertchannel].")
			sleep(5 SECONDS)
			msg("Understood [using_map_legacy().dock_name] Control, switching now!","[source_prefix] [source_shipname]")
		...
		if("policescan")
			...
			msg("[combined_first_name], this is [using_map_legacy().dock_name] Control, your [pick("ship","vessel","starship")] has been flagged for routine inspection. Hold position and prepare to be scanned.")
			sleep(5 SECONDS)
			msg("[confirm] [using_map_legacy().dock_name] Control, holding position.","[source_prefix] [source_shipname]")
			sleep(5 SECONDS)
			msg("Your compliance is appreciated, [combined_first_name]. Scan commencing.")
			sleep(10 SECONDS)
			msg(complain,"[source_prefix] [source_shipname]")
			sleep(15 SECONDS)
			msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Scan complete. [completed]")
		...
		if("policeflee")
			...
			msg("Unknown [pick("ship","vessel","starship")], this is [using_map_legacy().dock_name] Control, identify yourself and submit to a full inspection. Flying without an active transponder is a violation of system regulations.")
			sleep(5 SECONDS)
			msg("[uhoh]","[source_shipname]")
			sleep(5 SECONDS)
			msg("This is [using_map_legacy().starsys_name] Defense Control to all local assets: vector to interdict and detain [combined_first_name]. Control out.","[using_map_legacy().starsys_name] Defense Control")
		...
		if("policeshipflee")
			...
			msg("[uhoh]","[source_shipname]")
			sleep(5 SECONDS)
			msg("[using_map_legacy().starsys_name] Defense Control, this is [combined_second_name], we have a situation here, please advise.","[secondary_prefix] [secondary_shipname]")
			sleep(5 SECONDS)
			msg("Defense Control copies, [combined_second_name], reinforcements are en route. Switch further communications to encrypted band [sdfchannel].","[using_map_legacy().starsys_name] Defense Control")
		...
		if("policeshipcombat")
			...
			msg("[using_map_legacy().starsys_name] Defense Control, this is [combined_second_name], engaging [combined_first_name] [pick("near route","in sector")] [rand(1,100)], [battlestatus]","[secondary_prefix] [secondary_shipname]")
			sleep(5 SECONDS)
			msg("[using_map_legacy().starsys_name] Defense Control copies, [combined_second_name]. Keep us updated.","[using_map_legacy().starsys_name] Defense Control")
		...
		if("sdfpatrolupdate")
			...
			msg("[using_map_legacy().starsys_name] Defense Control, this is [combined_first_name] reporting in, [statusupdate], over.","[source_prefix] [source_shipname]")
			sleep(5 SECONDS)
			msg("[using_map_legacy().starsys_name] Defense Control copies, [combined_first_name]. Keep us updated, out.","[using_map_legacy().starsys_name] Defense Control")
		...
		if("hostiledetected")
			...
			msg("This is [using_map_legacy().starsys_name] Defense Control to all SDF assets. Priority update follows.","[using_map_legacy().starsys_name] Defense Control")
			sleep(5 SECONDS)
			msg("Be on the lookout for [combined_first_name], last sighted near route [rand(1,100)]. [orders]. DefCon, out.","[using_map_legacy().starsys_name] Defense Control")
		...
		if("distress")
			...
			sleep(5 SECONDS)
			msg("[combined_first_name], this is [using_map_legacy().starsys_name] Defense Control, copy. SDF is en route, contact on [sdfchannel].")
			sleep(5 SECONDS)
			msg("Understood [using_map_legacy().starsys_name] Defense Control, switching now!","[source_prefix] [source_shipname]")
		...
		if("traveladvisory")
			...
			msg("This is [using_map_legacy().dock_name] Control to all vessels in the [using_map_legacy().starsys_name] system. Priority travel advisory follows.")
			sleep(5 SECONDS)
		...
		if("pathwarning")
			...
			msg("[combined_first_name], this is [using_map_legacy().dock_name] Control, your [pick("ship","vessel","starship")] is approaching [navhazard], observe minimum safe distance and adjust your heading appropriately.")
			sleep(5 SECONDS)
			msg("[confirm] [using_map_legacy().dock_name] Control, adjusting course.","[source_prefix] [source_shipname]")
			sleep(5 SECONDS)
		...
		if("docking_request_chain")
			...
			if(path == "generic")
				...
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
				sleep(5 SECONDS)
				msg("[confirm_positive], [using_map_legacy().dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
			if(path == "sdf patrol")
				...
				msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
				sleep(5 SECONDS)
				msg("[confirm_special], [using_map_legacy().dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
				...
				if(subpath == "repair")
					...
					sleep(5 SECONDS)
					msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Repair crews are standing by, contact them on channel [engchannel].")
					sleep(5 SECONDS)
					msg("[confirm_special], [using_map_legacy().dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
					...
					sleep(5 SECONDS)
					msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Medtechs are standing by, contact them on channel [medchannel].")
					sleep(5 SECONDS)
					msg("[confirm_special], [using_map_legacy().dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
					...
					sleep(5 SECONDS)
					msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Security teams are standing by, contact them on channel [secchannel].")
					sleep(5 SECONDS)
					msg("[confirm_special], [using_map_legacy().dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
			else if(path == "denied")
				...
				msg("[callname], this is [combined_first_name], [pick("stopping by","passing through")] on our way to [source_destname], requesting permission to [landing_short].","[source_prefix] [source_shipname]")
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Request denied, [reason].")
				sleep(5 SECONDS)
				msg("[confirm_negative], [using_map_legacy().dock_name] Control. [disappointed]","[source_prefix] [source_shipname]")
				...
				msg("[callname], this is [combined_first_name], [pick("stopping by","passing through")] on our way to [source_destname], requesting permission to [landing_short].","[source_prefix] [source_shipname]")
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Request denied, [reason] and resubmit your request.")
				sleep(5 SECONDS)
				msg("[confirm_negative], [using_map_legacy().dock_name] Control.","[source_prefix] [source_shipname]")
				sleep(delay MINUTES)
				msg("[callname], this is [combined_first_name], resubmitting [landing_move].","[source_prefix] [source_shipname]")
				sleep (5 SECONDS)
				msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Everything appears to be in order now, permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
				sleep(5 SECONDS)
				msg("[confirm_positive], [using_map_legacy().dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
		...
		if("undocking_request_chain")
			...
			if(path == "standard")
				...
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Permission granted. Docking clamps released. [safetravels].")
				sleep(5 SECONDS)
				msg("[thanks], [using_map_legacy().dock_name] Control. This is [combined_first_name] setting course for [source_destname], out.","[source_prefix] [source_shipname]")
				...
				msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Permission granted. Docking clamps released. [safetravels].")
				sleep(5 SECONDS)
				msg("[thanks], [using_map_legacy().dock_name] Control. This is [combined_first_name] beginning system patrol, out.","[source_prefix] [source_shipname]")
			else if(path == "denied")
				...
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [using_map_legacy().dock_name] Control. Everything appears to be in order now, permission granted. Docking clamps released. [safetravels].")
				sleep(5 SECONDS)
				msg("[thanks], [using_map_legacy().dock_name] Control. This is [combined_first_name] setting course for [source_destname], out.","[source_prefix] [source_shipname]")
			...
			sleep(5 SECONDS)
			msg("[combined_first_name], this is [using_map_legacy().dock_name] Control, [response].")
			sleep(5 SECONDS)
			msg("[using_map_legacy().dock_name] Control, [yes ? "thank you" : "understood"], out.","[source_prefix] [source_shipname]")
