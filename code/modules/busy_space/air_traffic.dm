...
/datum/lore/atc_controller/proc/msg(message, sender)
	ASSERT(message)
	GLOB.global_announcer.autosay("[message]", sender ? sender : "[SSmapping.legacy_map_config.dock_name] Control")

/datum/lore/atc_controller/proc/reroute_traffic(yes = TRUE)
	if(yes)
		if(!squelched)
			msg("Re-routing traffic and fleet patterns around [SSmapping.legacy_map_config.station_name].")
		squelched = TRUE
...
/datum/lore/atc_controller/proc/shift_ending(evac = FALSE)
	msg("[SSmapping.legacy_map_config.shuttle_name], this is [SSmapping.legacy_map_config.dock_name] Control, you are cleared to complete routine transfer from [SSmapping.legacy_map_config.station_name] to [SSmapping.legacy_map_config.dock_name].")
	sleep(5 SECONDS)
	if(QDELETED(src))
		return
	msg("[SSmapping.legacy_map_config.shuttle_name] departing [SSmapping.legacy_map_config.dock_name] for [SSmapping.legacy_map_config.station_name] on routine transfer route. Estimated time to arrival: ten minutes.","[SSmapping.legacy_map_config.shuttle_name]")
...
/datum/lore/atc_controller/proc/random_convo()
	...
	var/alt_atc_names = list("[SSmapping.legacy_map_config.dock_name] Flight Control","[SSmapping.legacy_map_config.dock_name] FliCon","[SSmapping.legacy_map_config.dock_name] System Control","[SSmapping.legacy_map_config.dock_name] Star Control","[SSmapping.legacy_map_config.dock_name] SysCon","[SSmapping.legacy_map_config.dock_name] Control","[SSmapping.legacy_map_config.dock_name] STC","[SSmapping.legacy_map_config.dock_name] StarCon")
	...
	*/
	...
	switch(SSmapping.legacy_map_config.dock_type)
	...
	switch(chatter_type)
		...
		if("fleettraffic")
			if(prob(90)) // Good & Neutral Fleet reports
				var/report = pick("All systems are clear and we're maintaining near sector patrol with the [SSmapping.legacy_map_config.station_name]",
					...
					"We have confirmed visuals of [rand(3,15)] Diona clinging onto the [SSmapping.legacy_map_config.dock_name] hull. We'll have reports of first contact with them in a moment",
					...
					"Our vessel is undergoining early transfer procedures for the crew and should be at the [SSmapping.legacy_map_config.dock_name] in roughly [pick("5","10","15")] minutes",
				...
				msg("This is the [combined_first_name] reporting in to the [SSmapping.legacy_map_config.dock_name] with fleet updates. [report].","[source_prefix] [source_shipname]")
				sleep(6 SECONDS)
				// To try and give a more organic response on Traffic Control
				if(prob(50))
					msg("This is [SSmapping.legacy_map_config.dock_name]. Report received [source_prefix] [source_shipname]. Continue operations as normal.")
				else
					msg("[source_prefix] [source_shipname], thank you for your report. Please proceed operations as normal. [SSmapping.legacy_map_config.dock_name], out.")
				...
				var/bad_report = pick("Recent cultist activity on the nearby planet we scouted has deemed the area unsafe for exploration and mining operations",
					...
					"A recent fight with pirates has left [pick("one","two")] of our decks de-pressurized. We're attempting to reach the [SSmapping.legacy_map_config.dock_name] for fleet repairs",
				...
				msg("Attention [SSmapping.legacy_map_config.dock_name], this is the [combined_first_name] with an urgent report for the fleet. [bad_report].","[source_prefix] [source_shipname]")
				sleep(5 SECONDS)
				if(prob(50))
					msg("Thank you for the update [source_prefix] [source_shipname]. We'll relay this to the needed Captains. [SSmapping.legacy_map_config.dock_name], out.")
				else
					msg("Understood [source_prefix] [source_shipname]. Keep us updated on any changes or developments.")
					sleep(5 SECONDS)
					msg("Will do [SSmapping.legacy_map_config.dock_name]. [source_shipname], over and out.","[source_prefix] [source_shipname]")
		...
		if("emerg")
			...
			sleep(5 SECONDS)
			msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control, copy. Switch to emergency responder channel [ertchannel].")
			sleep(5 SECONDS)
			msg("Understood [SSmapping.legacy_map_config.dock_name] Control, switching now!","[source_prefix] [source_shipname]")
		...
		if("policescan")
			...
			msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control, your [pick("ship","vessel","starship")] has been flagged for routine inspection. Hold position and prepare to be scanned.")
			sleep(5 SECONDS)
			msg("[confirm] [SSmapping.legacy_map_config.dock_name] Control, holding position.","[source_prefix] [source_shipname]")
			sleep(5 SECONDS)
			msg("Your compliance is appreciated, [combined_first_name]. Scan commencing.")
			sleep(10 SECONDS)
			msg(complain,"[source_prefix] [source_shipname]")
			sleep(15 SECONDS)
			msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Scan complete. [completed]")
		...
		if("policeflee")
			...
			msg("Unknown [pick("ship","vessel","starship")], this is [SSmapping.legacy_map_config.dock_name] Control, identify yourself and submit to a full inspection. Flying without an active transponder is a violation of system regulations.")
			sleep(5 SECONDS)
			msg("[uhoh]","[source_shipname]")
			sleep(5 SECONDS)
			msg("This is [SSmapping.legacy_map_config.starsys_name] Defense Control to all local assets: vector to interdict and detain [combined_first_name]. Control out.","[SSmapping.legacy_map_config.starsys_name] Defense Control")
		...
		if("policeshipflee")
			...
			msg("[uhoh]","[source_shipname]")
			sleep(5 SECONDS)
			msg("[SSmapping.legacy_map_config.starsys_name] Defense Control, this is [combined_second_name], we have a situation here, please advise.","[secondary_prefix] [secondary_shipname]")
			sleep(5 SECONDS)
			msg("Defense Control copies, [combined_second_name], reinforcements are en route. Switch further communications to encrypted band [sdfchannel].","[SSmapping.legacy_map_config.starsys_name] Defense Control")
		...
		if("policeshipcombat")
			...
			msg("[SSmapping.legacy_map_config.starsys_name] Defense Control, this is [combined_second_name], engaging [combined_first_name] [pick("near route","in sector")] [rand(1,100)], [battlestatus]","[secondary_prefix] [secondary_shipname]")
			sleep(5 SECONDS)
			msg("[SSmapping.legacy_map_config.starsys_name] Defense Control copies, [combined_second_name]. Keep us updated.","[SSmapping.legacy_map_config.starsys_name] Defense Control")
		...
		if("sdfpatrolupdate")
			...
			msg("[SSmapping.legacy_map_config.starsys_name] Defense Control, this is [combined_first_name] reporting in, [statusupdate], over.","[source_prefix] [source_shipname]")
			sleep(5 SECONDS)
			msg("[SSmapping.legacy_map_config.starsys_name] Defense Control copies, [combined_first_name]. Keep us updated, out.","[SSmapping.legacy_map_config.starsys_name] Defense Control")
		...
		if("hostiledetected")
			...
			msg("This is [SSmapping.legacy_map_config.starsys_name] Defense Control to all SDF assets. Priority update follows.","[SSmapping.legacy_map_config.starsys_name] Defense Control")
			sleep(5 SECONDS)
			msg("Be on the lookout for [combined_first_name], last sighted near route [rand(1,100)]. [orders]. DefCon, out.","[SSmapping.legacy_map_config.starsys_name] Defense Control")
		...
		if("distress")
			...
			sleep(5 SECONDS)
			msg("[combined_first_name], this is [SSmapping.legacy_map_config.starsys_name] Defense Control, copy. SDF is en route, contact on [sdfchannel].")
			sleep(5 SECONDS)
			msg("Understood [SSmapping.legacy_map_config.starsys_name] Defense Control, switching now!","[source_prefix] [source_shipname]")
		...
		if("traveladvisory")
			...
			msg("This is [SSmapping.legacy_map_config.dock_name] Control to all vessels in the [SSmapping.legacy_map_config.starsys_name] system. Priority travel advisory follows.")
			sleep(5 SECONDS)
		...
		if("pathwarning")
			...
			msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control, your [pick("ship","vessel","starship")] is approaching [navhazard], observe minimum safe distance and adjust your heading appropriately.")
			sleep(5 SECONDS)
			msg("[confirm] [SSmapping.legacy_map_config.dock_name] Control, adjusting course.","[source_prefix] [source_shipname]")
			sleep(5 SECONDS)
		...
		if("docking_request_chain")
			...
			if(path == "generic")
				...
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
				sleep(5 SECONDS)
				msg("[confirm_positive], [SSmapping.legacy_map_config.dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
			if(path == "sdf patrol")
				...
				msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
				sleep(5 SECONDS)
				msg("[confirm_special], [SSmapping.legacy_map_config.dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
				...
				if(subpath == "repair")
					...
					sleep(5 SECONDS)
					msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Repair crews are standing by, contact them on channel [engchannel].")
					sleep(5 SECONDS)
					msg("[confirm_special], [SSmapping.legacy_map_config.dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
					...
					sleep(5 SECONDS)
					msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Medtechs are standing by, contact them on channel [medchannel].")
					sleep(5 SECONDS)
					msg("[confirm_special], [SSmapping.legacy_map_config.dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
					...
					sleep(5 SECONDS)
					msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Permission granted, proceed to [landing_zone]. Follow the green lights on your way in. Security teams are standing by, contact them on channel [secchannel].")
					sleep(5 SECONDS)
					msg("[confirm_special], [SSmapping.legacy_map_config.dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
			else if(path == "denied")
				...
				msg("[callname], this is [combined_first_name], [pick("stopping by","passing through")] on our way to [source_destname], requesting permission to [landing_short].","[source_prefix] [source_shipname]")
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Request denied, [reason].")
				sleep(5 SECONDS)
				msg("[confirm_negative], [SSmapping.legacy_map_config.dock_name] Control. [disappointed]","[source_prefix] [source_shipname]")
				...
				msg("[callname], this is [combined_first_name], [pick("stopping by","passing through")] on our way to [source_destname], requesting permission to [landing_short].","[source_prefix] [source_shipname]")
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Request denied, [reason] and resubmit your request.")
				sleep(5 SECONDS)
				msg("[confirm_negative], [SSmapping.legacy_map_config.dock_name] Control.","[source_prefix] [source_shipname]")
				sleep(delay MINUTES)
				msg("[callname], this is [combined_first_name], resubmitting [landing_move].","[source_prefix] [source_shipname]")
				sleep (5 SECONDS)
				msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Everything appears to be in order now, permission granted, proceed to [landing_zone]. Follow the green lights on your way in.")
				sleep(5 SECONDS)
				msg("[confirm_positive], [SSmapping.legacy_map_config.dock_name] Control. [dockingplan]","[source_prefix] [source_shipname]")
		...
		if("undocking_request_chain")
			...
			if(path == "standard")
				...
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Permission granted. Docking clamps released. [safetravels].")
				sleep(5 SECONDS)
				msg("[thanks], [SSmapping.legacy_map_config.dock_name] Control. This is [combined_first_name] setting course for [source_destname], out.","[source_prefix] [source_shipname]")
				...
				msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Permission granted. Docking clamps released. [safetravels].")
				sleep(5 SECONDS)
				msg("[thanks], [SSmapping.legacy_map_config.dock_name] Control. This is [combined_first_name] beginning system patrol, out.","[source_prefix] [source_shipname]")
			else if(path == "denied")
				...
				sleep(5 SECONDS)
				msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control. Everything appears to be in order now, permission granted. Docking clamps released. [safetravels].")
				sleep(5 SECONDS)
				msg("[thanks], [SSmapping.legacy_map_config.dock_name] Control. This is [combined_first_name] setting course for [source_destname], out.","[source_prefix] [source_shipname]")
			...
			sleep(5 SECONDS)
			msg("[combined_first_name], this is [SSmapping.legacy_map_config.dock_name] Control, [response].")
			sleep(5 SECONDS)
			msg("[SSmapping.legacy_map_config.dock_name] Control, [yes ? "thank you" : "understood"], out.","[source_prefix] [source_shipname]")
