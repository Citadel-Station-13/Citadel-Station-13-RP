...
/obj/item/shield_projector/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	if(active)
		if(always_on)
...
/obj/item/shield_projector/line/exosuit/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	if(active)
		if(always_on)
			to_chat(user, "<span class='warning'>You can't seem to deactivate \the [src].</span>")
			return
