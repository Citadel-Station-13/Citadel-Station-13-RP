...
/obj/machinery/shieldwallgen/attack_hand(mob/user as mob)
	if(state != 1)
		to_chat(user, "<span class='danger'>The shield generator needs to be firmly secured to the floor first.</span>")
		return 1
	if(src.locked && !istype(user, /mob/living/silicon))
		to_chat(user, "<span class='danger'>The controls are locked!</span>")
		return 1
	if(power != 1)
		to_chat(user, "<span class='danger'>The shield generator needs to be powered by wire underneath.</span>")
		return 1
...
/obj/machinery/shieldwallgen/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/card/id)||istype(W, /obj/item/pda))
		...
		src.add_fingerprint(user)
		visible_message("<span class='danger'>The [src.name] has been hit with \the [W.name] by [user.name]!</span>")
