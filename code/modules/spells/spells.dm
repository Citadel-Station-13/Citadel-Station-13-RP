...
/mob/statpanel_data(client/C)
	...
	if(!length(spell_list))
		if(C.statpanel.spell_last)
			// dispose
			for(var/tab in C.statpanel.spell_last)
				C.statpanel_tab(tab, FALSE)
			C.statpanel.spell_last = null
		return
	LAZYINITLIST(C.statpanel.spell_last)
	...
	// process tabs
	var/list/removing = C.statpanel.spell_last - collected
	var/list/adding = collected - C.statpanel.spell_last
