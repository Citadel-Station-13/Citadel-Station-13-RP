...
/datum/ghostrole_menu/ui_static_data(mob/user, datum/tgui/ui, datum/tgui_embed_context/embed_context)
	. = ..()
...
/datum/ghostrole_menu/ui_act(action, list/params, datum/tgui/ui, datum/tgui_embed_context/embed_context)
	if(..())
		return
	if(!isobserver(usr))
		return
