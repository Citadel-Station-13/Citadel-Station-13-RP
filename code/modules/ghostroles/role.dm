...
/proc/init_ghostroles()
	. = list()
	for(var/path in subtypesof(/datum/prototype/role/ghostrole))
		var/datum/prototype/role/ghostrole/G = path
...
/proc/get_ghostrole_datum(path)
	...
	if(ispath(is_this_a_path, /datum/prototype/role/ghostrole))
...
/datum/prototype/role/ghostrole
	/// Abstract type.
	abstract_type = /datum/prototype/role/ghostrole
...
/datum/prototype/role/ghostrole/New(_id)
...
/datum/prototype/role/ghostrole/proc/Instantiate(client/C, atom/loc, list/params)
	var/mob/living/L = instantiator.Run(C, loc, params)
	. = istype(L) && L
	if(.)
		L.mind?.assigned_role = assigned_role || name

/datum/prototype/role/ghostrole/proc/Transfer(client/C, mob/created)
 ...
 */
/datum/prototype/role/ghostrole/proc/PreInstantiate(client/C)
	return TRUE

/**
 * Checks if the client is a valid user mob and if we can allow a spawn from them
 */
/datum/prototype/role/ghostrole/proc/AllowSpawn(client/C, list/params)
...
/datum/prototype/role/ghostrole/proc/SpawnsLeft(client/C)
	if(spawnerless)
 ...
 * Ban check.
 */
/datum/prototype/role/ghostrole/proc/BanCheck(client/C)
	if(!jobban_role)
		return FALSE
	return jobban_isbanned(C.mob, jobban_role)

/datum/prototype/role/ghostrole/proc/GiveCustomObjective(mob/created, objective)
	created.GhostroleGiveCustomObjective(src, objective)

/mob/proc/GhostroleGiveCustomObjective(datum/prototype/role/ghostrole/R, objective)
	if(!mind)
		mind_initialize()
	if(!mind)
		CRASH("No mind.")
...
/datum/prototype/role/ghostrole/existing
	name = "Playable Mob"
...
/datum/prototype/role/ghostrole/existing/Instantiate(client/C, atom/loc, list/params)
