...
/datum/event/carp_migration/proc/spawn_carp(var/num_groups, var/group_size_min, var/group_size_max, var/dir, var/speed)
	...
	while(I < n)
		...
		if(istype(T,/turf/space))
			...
			spawned_carp ++
			M.throw_at_old(get_random_edge_turf(GLOB.reverse_dir[dir],TRANSITIONEDGE + 2, Z), 5, speed)//, callback = CALLBACK(src,TYPE_PROC_REF(/datum/event/carp_migration, check_gib),M))
		if(no_show)
			break
