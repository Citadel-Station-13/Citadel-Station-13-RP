...
//
// This section lets us configure which events we want in the rotation without completely removing them.
// Specifically this is done by moving the events we don't want down into the disabled_events list
// When put into disabled_events, it allows us to manually enable them ingame without vv or something wacky.
//
