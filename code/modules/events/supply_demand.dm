...
/datum/supply_demand_order/thing/New(var/qty, var/atom/type_path)
	...
	if(!name)
		log_debug(span_debug_warning("supply_demand event: Order for thing [type_path] has no name."))
...
/datum/supply_demand_order/reagent/match_item(var/atom/I)
	...
	if(amount_to_take >= 1)
		...
		log_debug(span_debug_warning("supply_demand event: not taking reagent '[reagent_id]': [amount_to_take]"))
	return
...
/datum/supply_demand_order/gas/match_item(var/obj/machinery/portable_atmospherics/canister)
	...
	if(canmix.return_pressure() < mixture.return_pressure())
		log_debug(span_debug_warning("supply_demand event: canister fails to match [canmix.return_pressure()] kPa < [mixture.return_pressure()] kPa"))
		return
	...
	for(var/gas in mixture.gas)
		...
		if(abs(targetPercent-canPercent) > 1)
			log_debug(span_debug_warning("supply_demand event: canister fails to match because '[gas]': [canPercent] != [targetPercent]"))
			return // Fail!
