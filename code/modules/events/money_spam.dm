	...
	if(useMS)
		if(prob(5))
			...
			switch(pick(1,2,3,4,5,6,7))
				...
				if(3)
					sender = pick("Galactic Payments Association","Better Business Bureau","[(LEGACY_MAP_DATUM).starsys_name] E-Payments","NAnoTrasen Finance Deparmtent","Luxury Replicas")
				...
				if(6)
					sender = pick("[(LEGACY_MAP_DATUM).company_name] Morale Divison","Feeling Lonely?","Bored?","www.wetskrell.nt")
					message = pick("The [(LEGACY_MAP_DATUM).company_name] Morale Division wishes to provide you with quality entertainment sites.",\
					...
					"Wetskrell.nt only provides the higest quality of male entertaiment to [(LEGACY_MAP_DATUM).company_name] Employees.",\
					"Simply enter your [(LEGACY_MAP_DATUM).company_name] Bank account system number and pin. With three easy steps this service could be yours!")
				if(7)
