...
/datum/event/canister_leak/start()
	...
	for(var/i in 1 to 10)
		...
		if(severity <= EVENT_LEVEL_MUNDANE && area_is_occupied(get_area(C)))
			log_debug(span_debug_warning("canister_leak event: Rejecting canister [C] ([C.x],[C.y],[C.z]) because area is occupied"))
			continue
		// Okay lets break it
		break_canister(C)
		return
	...
	log_debug(span_debug_warning("canister_leak event: Giving up after too many failures to pick target canister"))
	kill()
	return

/datum/event/canister_leak/proc/break_canister(var/obj/machinery/portable_atmospherics/canister/C)
	log_debug(span_debug_warning("canister_leak event: Canister [C] ([C.x],[C.y],[C.z]) destroyed."))
	C.atom_break()
