...
/datum/event/cult/overmap/announce()
	if(istype(victim, /atom/movable/overmap/entity/visitable/ship/talon))
...
/datum/event/cult/overmap/start()		// override - cancel if not main ship since it doesn't properly target the actual triggering ship
	if(istype(victim, /atom/movable/overmap/entity/visitable/ship/landable))
		kill()
		return
	if(istype(victim, /atom/movable/overmap/entity/visitable/ship/talon)) //Forces the location to be the Talon.
		isTalon = 1
		location = 5
	return ..()
