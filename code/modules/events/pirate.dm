...
/datum/event/pirate/overmap/announce()
	if(istype(victim, /atom/movable/overmap_object/entity/visitable/ship/talon))
		if(piratestring == "human")
...
/datum/event/pirate/overmap/start()		// override - cancel if not main ship since it doesn't properly target the actual triggering ship
	if(istype(victim, /atom/movable/overmap_object/entity/visitable/ship/landable))
		kill()
	if(istype(victim, /atom/movable/overmap_object/entity/visitable/ship/talon)) //Forces the location to be the Talon.
		isTalon = 1
		location = 5
	return ..()
