...
/obj/item/hardsuit/Initialize(mapload)
	...
	if(glove_type)
		...
		add_obj_verb(src, TYPE_PROC_REF(/obj/item/hardsuit, toggle_gauntlets))
	if(helm_type)
		helmet = new helm_type(src)
		add_obj_verb(src, TYPE_PROC_REF(/obj/item/hardsuit, toggle_helmet))
	if(boot_type)
		boots = new boot_type(src)
		add_obj_verb(src, TYPE_PROC_REF(/obj/item/hardsuit, toggle_boots))
	if(chest_type)
		chest = new chest_type(src)
		if(allowed)
			chest.allowed = allowed
		add_obj_verb(src, TYPE_PROC_REF(/obj/item/hardsuit, toggle_chest))
