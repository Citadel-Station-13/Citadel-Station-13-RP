		...
		if(useMS)
			if(prob(5))
				...
				switch(pick(1,2,3,4,5,6,7))
					...
					if(3)
						sender = pick("Galactic Payments Association","Better Business Bureau","[SSmapping.legacy_map_config.starsys_name] E-Payments","NAnoTrasen Finance Deparmtent","Luxury Replicas")
					...
					if(4)
						...
						"Dr. Maxman was created by George Acuilar, M.D, a [SSmapping.legacy_map_config.boss_short] Certified Urologist who has treated over 70,000 patients sector wide with 'male problems'.",\
					...
					if(6)
						sender = pick("[SSmapping.legacy_map_config.company_name] Morale Divison","Feeling Lonely?","Bored?","www.wetskrell.nt")
						message = pick("The [SSmapping.legacy_map_config.company_name] Morale Division wishes to provide you with quality entertainment sites.",\
						...
						"Wetskrell.nt only provides the higest quality of male entertaiment to [SSmapping.legacy_map_config.company_name] Employees.",\
						"Simply enter your [SSmapping.legacy_map_config.company_name] Bank account system number and pin. With three easy steps this service could be yours!")
					if(7)
