...
/datum/gm_action/blob/set_up()
	...
	for(var/i in 1 to 10)
		...
		if(is_area_occupied(A))
			log_debug(span_debug_warning("Blob infestation event: Rejected [A] because it is occupied."))
			continue
		var/list/turfs = list()
		for(var/turf/simulated/floor/F in A)
			if(turf_clear(F))
				turfs += F
		if(turfs.len == 0)
			log_debug(span_debug_warning("Blob infestation event: Rejected [A] because it has no clear turfs."))
			continue
		target_area = A
		target_turf = pick(turfs)

	if(!target_area)
		log_debug(span_debug_warning("Blob infestation event: Giving up after too many failures to pick target area"))
