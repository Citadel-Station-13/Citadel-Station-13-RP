...
var/list/admin_departments = list("[using_map_legacy().boss_name]", "Hadii's Folly Governmental Authority", "Supply")
...
/obj/machinery/photocopier/faxmachine/Initialize(mapload)
	...
	allfaxes += src
	if(!destination)
		destination = "[using_map_legacy().boss_name]"
...
/obj/machinery/photocopier/faxmachine/nano_ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	if(scan)
		...
		data["scanName"] = null
	data["bossName"] = using_map_legacy().boss_name
...
/obj/machinery/photocopier/faxmachine/proc/send_admin_fax(mob/sender, destination)
	...
	if (destination == using_map_legacy().boss_name)
		message_admins(sender, "[uppertext(using_map_legacy().boss_short)] FAX", rcvdcopy, "CentComFaxReply", "#006100")
		...
		message_admins(sender, "[uppertext(using_map_legacy().boss_short)] SUPPLY FAX", rcvdcopy, "CentComFaxReply", "#5F4519")
	else
