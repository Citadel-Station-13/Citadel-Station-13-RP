/datum/prototype/sprite_accessory/wing/feathered
	name = "feathered wings (colorable)"
	id = "wing_feather"
	icon = 'icons/mob/sprite_accessory/wings/feathered-generic.dmi'
	icon_state = "feathered"
	do_colouration = 1
	legacy_use_additive_color_matrix = FALSE

/datum/prototype/sprite_accessory/wing/feathered_medium
	name = "medium feathered wings (colorable)" // Keekenox made these feathery things with a little bit more shape to them than the other wings. They are medium sized wing boys.
	id = "wing_feather_medium"
	icon = 'icons/mob/sprite_accessory/wings/feathered-generic.dmi'
	icon_state = "feathered2"
	variations = list(
		SPRITE_ACCESSORY_VARIATION_SPREAD = "feathered2-spr"
	)
	do_colouration = 1
	legacy_use_additive_color_matrix = FALSE

/datum/prototype/sprite_accessory/wing/feathered_large //Made by Natje!
	name = "large feathered wings (colorable)"
	id = "wing_feather_large"
	icon = 'icons/mob/sprite_accessory/wings/feathered-generic.dmi'
	icon_state = "feathered3"
	do_colouration = 1
	legacy_use_additive_color_matrix = FALSE
