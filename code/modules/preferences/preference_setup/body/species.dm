/datum/category_item/player_setup_item/body/char_species
...
/datum/category_item/player_setup_item/body/char_species/content(datum/preferences/prefs, mob/user, data)
	. = list()
...
/datum/category_item/player_setup_item/body/char_species/spawn_checks(datum/preferences/prefs, data, flags, list/errors)
...
/datum/category_item/player_setup_item/body/char_species/filter_data(datum/preferences/prefs, data, list/errors)
	// resolve
	var/datum/character_species/CS = SScharacters.resolve_character_species(data)
	if(CS)
		// cool we found it
...
/datum/category_item/player_setup_item/body/char_species/informed_default_value(datum/preferences/prefs, randomizing)
...
/datum/category_item/player_setup_item/body/char_species/default_value(randomizing)
	return SScharacters.default_species_id()

/datum/category_item/player_setup_item/body/char_species/act(datum/preferences/prefs, mob/user, action, list/params)
	switch(action)
		if("change")
...
// todo: proper view-only section support
/datum/category_item/player_setup_item/body/real_species
	name = "(Virtual) Real Species"
	sort_order = 2
	save_key = CHARACTER_DATA_REAL_SPECIES
	load_order = PREFERENCE_LOAD_ORDER_REAL_SPECIES

/datum/category_item/player_setup_item/body/real_species/filter_data(datum/preferences/prefs, data, list/errors)
...
/datum/category_item/player_setup_item/body/real_species/default_value(randomizing)
	return SScharacters.default_species_id()

/datum/category_item/player_setup_item/body/real_species/copy_to_mob(datum/preferences/prefs, mob/M, data, flags)
	if(!ishuman(M))
		return TRUE
