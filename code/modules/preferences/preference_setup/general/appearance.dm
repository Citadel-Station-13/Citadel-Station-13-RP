/datum/category_item/player_setup_item/general/appearance
	name = "Religion"
	save_key = CHARACTER_DATA_RELIGION
	sort_order = 6


#warn impl all this shit

/datum/category_item/player_setup_item/general/appearance/content(datum/preferences/prefs, mob/user, data)
	. = list()
...
/datum/category_item/player_setup_item/general/appearance/act(datum/preferences/prefs, mob/user, action, list/params)
	switch(action)
		if("pick")
			var/id = params["pick"]
...
/datum/category_item/player_setup_item/general/appearance/filter_data(datum/preferences/prefs, data, list/errors)
	var/datum/lore/character_background/religion/current = SScharacters.resolve_religion(data)
...
/datum/category_item/player_setup_item/general/appearance/copy_to_mob(datum/preferences/prefs, mob/M, data, flags)
...
/datum/category_item/player_setup_item/general/appearance/spawn_checks(datum/preferences/prefs, data, flags, list/errors, list/warnings)
	var/datum/lore/character_background/religion/current = SScharacters.resolve_religion(data)
	if(!current?.check_species_id(prefs.character_species_id()))
...
/datum/category_item/player_setup_item/general/appearance/default_value(randomizing)
	return SScharacters.resolve_religion(/datum/lore/character_background/religion/custom).id

/datum/category_item/player_setup_item/general/appearance/informed_default_value(datum/preferences/prefs, randomizing)
