	...
	if(href_list["skin_color"])
		if(can_change_skin_color())
			...
			if(new_skin && can_still_topic(state))
				...
				if(owner.change_skin_color(rgb(r_skin, g_skin, b_skin)))
					update_dna()
					return 1
	if(href_list["hair"])
	...
	if(href_list["hair_color"])
		if(can_change(APPEARANCE_HAIR_COLOR))
			...
			if(new_hair && can_still_topic(state))
				...
				if(owner.change_hair_color(rgb(r_hair, g_hair, b_hair)))
					update_dna()
					return 1
	...
	if(href_list["facial_hair_color"])
		if(can_change(APPEARANCE_FACIAL_HAIR_COLOR))
			...
			if(new_facial && can_still_topic(state))
				...
				if(owner.change_facial_hair_color(rgb(r_facial, g_facial, b_facial)))
					update_dna()
					return 1
	if(href_list["eye_color"])
		if(can_change(APPEARANCE_EYE_COLOR))
			...
			if(new_eyes && can_still_topic(state))
				...
				if(owner.change_eye_color(rgb(r_eyes, g_eyes, b_eyes)))
					update_dna()
					return 1
