...
/mob/living/carbon/human/UnarmedAttack(atom/A, proximity_flag, list/modifiers)
	...
	if(istype(G) && G.Touch(A,1)) //proximity_flag &&
		return
...
/mob/living/carbon/human/RangedAttack(atom/A, mouseparams)
	...
	if(isturf(A) && get_dist(src,A) <= 1)
		...
		return TRUE
	//Only used by xenos right now, can be expanded. Do not continue as this is an attack
	if(spitting)
		Spit(A)
...
/mob/living/UnarmedAttack(atom/A, proximity_flag, list/modifiers)
	if(hands_are_full()) //HAS_TRAIT(src, TRAIT_HANDS_BLOCKED))
		return
