...
*/
/turf/proc/AdjacentQuick(atom/neighbor, atom/target)
	var/turf/T0 = get_turf(neighbor)
	if(T0 == src) //same turf
		return TRUE
...
/turf/proc/ClickCross(target_dir, border_only, target_atom = null, atom/movable/mover = null)
	for(var/obj/O in src)
		if((mover && O.CanPass(mover,get_step(src,target_dir))) || (!mover && !O.density))
			continue
		if(O == target_atom || O == mover || O.throwpass) // (O.pass_flags_self & LETPASSTHROW)) //check if there's a dense object present on the turf
