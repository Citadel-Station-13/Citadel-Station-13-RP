...
/obj/structure/safe/legacy_ex_act(severity)
	if((severity >= 2) && explosion_count < BROKEN_THRESHOLD)
		explosion_count++
...
/obj/structure/safe/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	var/canhear = FALSE
	if(user.is_holding(/obj/item/clothing/accessory/stethoscope))
		canhear = TRUE
...
/obj/structure/safe/proc/check_broken()
	return (atom_flags & ATOM_BROKEN) || explosion_count >= BROKEN_THRESHOLD
