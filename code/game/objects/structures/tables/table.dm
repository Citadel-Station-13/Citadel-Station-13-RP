...
/obj/structure/table/wrench_act(obj/item/I, datum/event_args/actor/clickchain/e_args, flags, hint)
	. = ..()
	if(.)
		return
	if(!isnull(material_reinforcing))
...
/obj/structure/table/screwdriver_act(obj/item/I, datum/event_args/actor/clickchain/e_args, flags, hint)
	. = ..()
	if(.)
		return
	if(isnull(material_reinforcing))
...
/obj/structure/table/update_icon()
	...
	if(flipped != 1)
		...
		if(material_base)
			for(var/i = 1 to 4)
				var/image/I = get_table_image(icon, "[material_base.table_icon_base]_[legacy_connections[i]]", 1<<(i-1), material_base.icon_colour, 255 * material_base.opacity)
				overlays_to_add += I

		// Reinforcements
		if(material_reinforcing)
			for(var/i = 1 to 4)
				var/image/I = get_table_image(icon, "[material_reinforcing.table_reinf_icon_base]_[legacy_connections[i]]", 1<<(i-1), material_reinforcing.icon_colour, 255 * material_reinforcing.opacity)
				overlays_to_add += I
