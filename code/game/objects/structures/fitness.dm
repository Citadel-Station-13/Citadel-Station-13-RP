...
/obj/structure/fitness/punchingbag/attack_hand(mob/user, datum/event_args/clickchain/e_args)
	if(!istype(user))
		..()
		return
...
/obj/structure/fitness/weightlifter/attack_hand(mob/user, datum/event_args/clickchain/e_args)
	if(!istype(user))
		return
	if(user.loc != src.loc)
