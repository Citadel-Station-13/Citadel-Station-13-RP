...
/obj/structure/undies_wardrobe/interact(var/mob/living/carbon/human/H)
	...
	for(var/datum/category_group/underwear/UWC in GLOB.global_underwear.categories)
		...
		if(UWI)
			for(var/datum/loadout_entry_tweak/gt in UWI.tweaks)
...
/obj/structure/undies_wardrobe/proc/get_metadata(var/mob/living/carbon/human/H, var/underwear_category, var/datum/loadout_entry_tweak/gt)
...
/obj/structure/undies_wardrobe/proc/set_metadata(var/mob/living/carbon/human/H, var/underwear_category, var/datum/loadout_entry_tweak/gt, var/new_metadata)
...
/obj/structure/undies_wardrobe/Topic(href, href_list, state)
	...
	if(href_list["remove_underwear"])
		...
		if(!(underwear in H.all_underwear))
			return
		var/datum/loadout_entry_tweak/gt = locate(href_list["tweak"])
		if(!gt)
			return
