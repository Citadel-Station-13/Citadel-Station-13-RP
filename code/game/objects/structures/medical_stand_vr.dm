	...
	switch (action_type)
		if ("Remove tank")
			if (!tank)
				...
				user.visible_message(SPAN_NOTICE("\The [user] removes \the [tank] from \the [src]."), "<span class='warning'>You remove \the [tank] from \the [src].</span</span>>")
				user.put_in_hands(tank)
				tank = null
				update_icon()
				return
			else if (!is_loosen)
				user.visible_message(SPAN_NOTICE("\The [user] tries to removes \the [tank] from \the [src] but it won't budge."), "<span class='warning'>You try to removes \the [tank] from \the [src] but it won't budge.</span</span>>")
				return
		if ("Toggle valve")
			if (!tank)
				to_chat(user, SPAN_WARNING("There is no tank in \the [src]!"))
