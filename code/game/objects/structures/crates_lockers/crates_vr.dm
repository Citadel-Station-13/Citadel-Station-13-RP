...
/obj/structure/closet/crate/secure/bullet_act(var/obj/item/projectile/Proj)
	...
	if(locked && tamper_proof && health <= Proj.damage)
		if(tamper_proof == 2) // Mainly used for events to prevent any chance of opening the box improperly.
			visible_message("<span class='danger'><b>The anti-tamper mechanism of [src] triggers an explosion!</b></span>")
		...
		switch(open_chance)
			if(1)
				visible_message("<span class='danger'><b>The anti-tamper mechanism of [src] causes an explosion!</b></span>")
				var/turf/T = get_turf(src.loc)
				explosion(T, 0, 0, 0, 1) // Non-damaging, but it'll alert security.
				qdel(src)
			if(2 to 4)
				visible_message("<span class='danger'><b>The anti-tamper mechanism of [src] causes a small fire!</b></span>")
