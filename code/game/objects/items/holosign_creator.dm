...
/obj/item/holosign_creator/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	if(signs.len)
...
/obj/item/holosign_creator/cyborg/attack_self(datum/event_args/actor/actor, mob/user)
	. = ..()
	if(.)
		return
	if(iscyborg(user))
