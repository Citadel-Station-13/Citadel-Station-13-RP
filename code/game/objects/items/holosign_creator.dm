...
/obj/item/holosign_creator/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	. = ..()
	if(.)
		return
	if(signs.len)
...
/obj/item/holosign_creator/cyborg/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	. = ..()
	if(.)
		return
	if(iscyborg(user))
