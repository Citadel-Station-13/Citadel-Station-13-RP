...
/obj/item/tank/proc/check_status()
	...
	if(pressure > TANK_FRAGMENT_PRESSURE)
		...
		if(integrity <= 0)
			...
			visible_message("[icon2html(thing = src, target = world)] <span class='danger'>\The [src] flies apart!</span>", SPAN_WARNING("You hear a bang!"))
		...
		if((integrity <= 17 || src.leaking) && !valve_welded)
			...
			if(!leaking)
				...
				leaking = 1
				#ifdef FIREDBG
				log_debug(SPAN_DEBUG(SPAN_WARNING("[x],[y] tank is leaking: [pressure] kPa, integrity [integrity]")))
				#endif
		else
			integrity-= 1
	else
