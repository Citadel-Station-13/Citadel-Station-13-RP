...
/obj/item/material/hammer_head/examine(var/mob/user)
	...
	if(.)
		switch(construction_stage)
			if(1)
				to_chat(user, "<span class='notice'>It has a slot in the base for a metal rod.</span>")
			if(2)
				to_chat(user, "<span class='notice'>It has a bunch of rods sticking out of the hole. Consider welding it.</span>")
			if(3)
				to_chat(user, "<span class='notice'>It has a short rough metal shaft sticking to it, quite short, needs more rods.</span>")
			if(4)
				to_chat(user, "<span class='notice'>It has a bunch of unwelded rods jammed into the shaft.</span>")
			if(5)
				to_chat(user, "<span class='notice'>It has a pretty long rough metal shaft sticking out of it, it look hard to grab. Could make a handle with some steel...</span>")
			if(6)
				to_chat(user, "<span class='notice'>It has a few unwelded sheets bent in half across the handle.</span>")
			if(7)
				to_chat(user, "<span class='notice'>It has red hot, pliable looking metal sheets spread over the handle. What a sloppy job. Finish the job with a wrench.</span>")
