...
/obj/item/clothing/mask/smokable/cigarette/attack_self(datum/event_args/actor/actor, mob/user)
	if(lit == 1)
...
/obj/item/clothing/mask/smokable/pipe/attack_self(datum/event_args/actor/actor, mob/user)
	if(lit == 1)
		if(user.a_intent == INTENT_HARM)
...
/obj/item/flame/lighter/attack_self(datum/event_args/actor/actor, mob/user)
	if(!base_state)
		base_state = icon_state
	if(!lit)
