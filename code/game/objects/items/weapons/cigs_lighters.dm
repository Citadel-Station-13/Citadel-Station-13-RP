...
/obj/item/clothing/mask/smokable/attack_mob(mob/M, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
...
/obj/item/flame/lighter/attack_mob(mob/M, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
	var/mob/living/carbon/human/H = M
	if(!istype(H))
		return ..()
	if(lit == 1)
