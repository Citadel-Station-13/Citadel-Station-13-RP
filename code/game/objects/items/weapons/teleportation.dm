...
/obj/item/hand_tele/attack_self(mob/user as mob)
	...
	if(!current_location || (current_location.z in using_map_legacy().admin_levels) || current_location.block_tele)//If turf was not found or they're on z level 2 or >7 which does not currently exist.
