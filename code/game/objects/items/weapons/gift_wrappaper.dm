...
/obj/item/a_gift/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	. = ..()
	if(.)
		return
	var/gift_type = pick(
...
/obj/item/gift/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	. = ..()
	if(.)
		return
	user.temporarily_remove_from_inventory(src, INV_OP_FORCE | INV_OP_SHOULD_NOT_INTERCEPT | INV_OP_SILENT)
...
/obj/item/b_gift/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	. = ..()
	if(.)
		return
	var/gift_type = pick(
