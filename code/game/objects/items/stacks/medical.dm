...
/obj/item/stack/medical/crude_pack/checked_application(mob/M, mob/user)
	...
	if (istype(M, /mob/living/carbon/human))
		...
		if(affecting.is_bandaged())
			...
			user.visible_message(SPAN_NOTICE("\The [user] starts bandaging [M]'s [affecting.name]."), \
			...
			for (var/datum/wound/W in affecting.wounds)
				...
				if (W.current_stage <= W.max_bleeding_stage)
					...
					user.visible_message(SPAN_NOTICE("\The [user] places a bandage over \a [W.desc] on [M]'s [affecting.name]."), \
...
/obj/item/stack/medical/bruise_pack/checked_application(mob/M, mob/user)
	...
	if (istype(M, /mob/living/carbon/human))
		...
		if(affecting.is_bandaged())
			...
			user.visible_message(SPAN_NOTICE("\The [user] starts treating [H]'s [affecting.name]."), \
					             SPAN_NOTICE("You start treating [H]'s [affecting.name].") )
			var/used = 0
			for (var/datum/wound/W in affecting.wounds)
				if (W.internal)
					continue
				...
				if (W.current_stage <= W.max_bleeding_stage)
					...
					user.visible_message(SPAN_NOTICE("\The [user] places a bruise patch over \a [W.desc] on [H]'s [affecting.name]."), \
					                              SPAN_NOTICE("You place a bruise patch over \a [W.desc] on [H]'s [affecting.name].") )
				else
					user.visible_message(SPAN_NOTICE("\The [user] places a bandaid over \a [W.desc] on [H]'s [affecting.name]."), \
					                              SPAN_NOTICE("You place a bandaid over \a [W.desc] on [H]'s [affecting.name].") )
				W.bandage()
				playsound(src, pick(apply_sounds), 25)
				used++
				H.bitten = 0
