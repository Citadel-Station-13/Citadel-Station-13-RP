...
/obj/item/reagent_scanner/afterattack(atom/target, mob/user, clickchain_flags, list/params)
	...
	if(!isnull(target.reagents))
		if(!(target.flags & OPENCONTAINER)) // The idea is that the scanner has to touch the reagents somehow. This is done to prevent cheesing unidentified autoinjectors.
