...
/obj/item/analyzer/attack_self(mob/user, datum/event_args/clickchain/e_args)
	. = ..()
	if(.)
		return
	if (user.stat)
...
/obj/item/mass_spectrometer/attack_self(mob/user, datum/event_args/clickchain/e_args)
	. = ..()
	if(.)
		return
	if (user.stat)
