...
/obj/item/communicator/proc/open_connection_to_communicator(mob/user, var/atom/candidate)
	...
	if(user)
		comm.visible_message("<span class='notice'>[icon2html(thing = src, target = world)] Connecting to [src].</span>")
		to_chat(user, "<span class='notice'>[icon2html(thing = src, target = user)] Attempting to call [comm].</span>")
		sleep(10)
		to_chat(user, "<span class='notice'>[icon2html(thing = src, target = user)] Dialing internally from [station_name()], [system_name()].</span>")
		sleep(20) //If they don't have an exonet something is very wrong and we want a runtime.
		to_chat(user, "<span class='notice'>[icon2html(thing = src, target = user)] Connection re-routed to [comm] at [comm.exonet.address].</span>")
		sleep(40)
		to_chat(user, "<span class='notice'>[icon2html(thing = src, target = user)] Connection to [comm] at [comm.exonet.address] established.</span>")
		comm.visible_message("<span class='notice'>[icon2html(thing = src, target = world)] Connection to [src] at [exonet.address] established.</span>")
		sleep(20)
...
/obj/item/communicator/proc/open_connection_to_ghost(mob/user, var/mob/candidate)
	...
	if(user)
		to_chat(user, "<span class='notice'>[icon2html(thing = src, target = user)] Connecting to [candidate].</span>")
	to_chat(new_voice, "<span class='notice'>[icon2html(thing = src, target = new_voice)] Attempting to call [src].</span>")
	sleep(10)
	to_chat(new_voice, "<span class='notice'>[icon2html(thing = src, target = new_voice)] Dialing to [station_name()], Kara Subsystem, [system_name()].</span>")
	sleep(20)
	to_chat(new_voice, "<span class='notice'>[icon2html(thing = src, target = new_voice)] Connecting to [station_name()] telecommunications array.</span>")
	sleep(40)
	to_chat(new_voice, "<span class='notice'>[icon2html(thing = src, target = new_voice)] Connection to [station_name()] telecommunications array established.  Redirecting signal to [src].</span>")
	sleep(20)
	...
	to_chat(new_voice, "<span class='notice'>[icon2html(thing = src, target = new_voice)] Connection to [src] established.</span>")
	...
	if(user)
		to_chat(user, "<span class='notice'>[icon2html(thing = src, target = user)] Your communicator is now connected to [candidate]'s communicator.</span>")
...
/obj/item/communicator/proc/close_connection(mob/user, var/atom/target, var/reason)
	...
	for(var/mob/living/voice/voice in voice_mobs) //Handle ghost-callers
		if(target && voice != target) //If no target is inputted, it deletes all of them.
			continue
		to_chat(voice, "<span class='danger'>[icon2html(thing = src, target = user)] [reason].</span>")
		visible_message("<span class='danger'>[icon2html(thing = src, target = world)] [reason].</span>")
	...
	for(var/obj/item/communicator/comm in communicating) //Now we handle real communicators.
		...
		comm.visible_message("<span class='danger'>[icon2html(thing = src, target = world)] [reason].</span>")
		visible_message("<span class='danger'>[icon2html(thing = src, target = world)] [reason].</span>")
...
/obj/item/communicator/proc/request(var/atom/candidate)
	...
	if(L)
		to_chat(L, "<span class='notice'>[icon2html(thing = src, target = L)] Communications request from [who].</span>")
...
/obj/item/communicator/proc/del_request(var/atom/candidate)
	...
	if(us)
		to_chat(us, "<span class='notice'>[icon2html(thing = src, target = us)] Declined request.</span>")
...
/obj/item/communicator/see_emote(mob/living/M, text)
	var/rendered = "[icon2html(thing = src, target = world)] <span class='message'>[text]</span>"
...
/obj/item/communicator/show_message(msg, type, alt, alt_type)
	var/rendered = "[icon2html(thing = src, target = world)] <span class='message'>[msg]</span>"
	for(var/obj/item/communicator/comm in communicating)
		var/turf/T = get_turf(comm)
		if(!T) return
...
/obj/item/communicator/proc/connect_video(mob/user,obj/item/communicator/comm)
	...
	if(!(src in comm.communicating) || !comm.camera) //You called someone with a broken communicator or one that's fake or yourself or something
		to_chat(user, "<span class='danger'>[icon2html(thing = src, target = user)]ERROR: Video failed. Either bandwidth is too low, or the other communicator is malfunctioning.</span>")

	to_chat(user, "<span class='notice'>[icon2html(thing = src, target = user)] Attempting to start video over existing call.</span>")
	sleep(30)
	to_chat(user, "<span class='notice'>[icon2html(thing = src, target = user)] Please wait...</span>")

	video_source = comm.camera
	comm.visible_message("<span class='danger'>[icon2html(thing = src, target = world)] New video connection from [comm].</span>")
	watch_video(user)
	update_icon()
...
/obj/item/communicator/proc/end_video(var/reason)
	...
	. = "<span class='danger'>[icon2html(thing = src, target = world)] [reason ? reason : "Video session ended"].</span>"
