...
/obj/item/transfer_valve/ui_data(mob/user, datum/tgui/ui, datum/tgui_embed_context/embed_context)
	var/list/data = list()
...
/obj/item/transfer_valve/ui_act(action, list/params, datum/tgui/ui, datum/tgui_embed_context/embed_context)
	if(..())
		return
	. = TRUE
	switch(action)
