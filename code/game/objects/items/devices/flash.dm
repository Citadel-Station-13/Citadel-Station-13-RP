...
//attack_as_weapon
/obj/item/flash/process_mob_melee(mob/M, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
	flash_mob(M, user)
...
//attack_as_weapon
/obj/item/flash/synthetic/process_mob_melee(mob/M, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
	. = ..()
	if(!broken)
		broken = 1
		to_chat(user, "<span class='warning'>The bulb has burnt out!</span>")
