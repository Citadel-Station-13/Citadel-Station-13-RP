...
/*****************
 * Uplink TGUI FOR UPLINK WOOP WOOP
 *****************/
...
/obj/item/uplink/hidden/ui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui)
	if(!active)
		toggle()
...
/obj/item/uplink/hidden/ui_data(mob/user, datum/tgui/ui, datum/ui_state/state)
	if(!user.mind)
		return
...
/obj/item/uplink/hidden/ui_status(mob/user, datum/ui_state/state)
	if(!active)
		return UI_CLOSE
	return ..()
