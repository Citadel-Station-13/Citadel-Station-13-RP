...
/obj/item/camerabug/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	. = ..()
	if(.)
		return
...
/obj/item/bug_monitor/attack_self(mob/user, datum/event_args/actor/clickchain/e_args)
	. = ..()
	if(.)
		return
	if(operating)
