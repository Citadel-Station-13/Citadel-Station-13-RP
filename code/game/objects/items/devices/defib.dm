...
/obj/item/shockpaddles/proc/can_revive(mob/living/carbon/human/H) //This is checked right before attempting to revive
	...
	if(H.isSynthetic())
		if(H.health + H.getOxyLoss() + H.getToxLoss() <= config_legacy.health_threshold_dead)
			return "buzzes, \"Resuscitation failed - Severe buildup of toxins and metabolic stress detected. Begin manual repair before attempting again.\""

	else if(HUSK in H.mutations)
		return "buzzes, \"Resuscitation failed - Severe tissue damage makes recovery of patient impossible via defibrillator. Begin manual repair before attempting again.\""
	
	else if(!H.can_defib)
		return "buzzes, \"Subject biological state incompatible with defibrillation.\""
