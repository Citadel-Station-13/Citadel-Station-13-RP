	...
	if(new_state != old_state)
		switch(new_state)
			if(PROXIMITY_OFF_CAMERANET)
				to_chat(carrier, SPAN_NOTICE("[icon2html(thing = src, target = carrier)] Now outside of camera network."))
				SEND_SOUND(carrier, sound('sound/machines/defib_failed.ogg'))
			if(PROXIMITY_NONE)
				to_chat(carrier, SPAN_NOTICE("[icon2html(thing = src, target = carrier)] Now within camera network, AI and cameras unfocused."))
				SEND_SOUND(carrier, sound('sound/machines/defib_safetyOff.ogg'))
			if(PROXIMITY_NEAR)
				to_chat(carrier, SPAN_WARNING("[icon2html(thing = src, target = carrier)] Warning: AI focus at nearby location."))
