...
/area/awaymission/LateInitialize()
	...
	if(!valid_spawn_turfs.len)
		to_chat(world.log, "Error! [src] does not have any turfs!")
		return TRUE
...
/area/awaymission/proc/spawn_mob_on_turf()
	if(!valid_mobs.len)
		to_chat(world.log, "[src] does not have a set valid mobs list!")
		return TRUE
...
/area/awaymission/proc/spawn_flora_on_turf()
	if(!valid_flora.len)
		to_chat(world.log, "[src] does not have a set valid flora list!")
		return TRUE
