...
/datum/antagonist/ninja/proc/generate_ninja_directive(side)
	var/directive = "[side=="face"?"[using_map_legacy().company_name]":"A criminal syndicate"] is your employer. "//Let them know which side they're on.
	switch(rand(1,19))
		if(1)
		...
		if(5)
			directive += "We are currently negotiating with [using_map_legacy().company_name] [using_map_legacy().boss_name]. Prioritize saving human lives over ending them."
		if(6)
		...
		if(9)
			directive += "A free agent has proposed a lucrative business deal. Implicate [using_map_legacy().company_name] involvement in the operation."
		if(10)
		...
		if(13)
			directive += "Some disgruntled [using_map_legacy().company_name] employees have been supportive of our operations. Be wary of any mistreatment by command staff."
		if(14)
		...
		if(16)
			directive += "The Spider Clan has been bargaining with a competing prosthetics manufacturer. Try to shine [using_map_legacy().company_name] prosthetics in a bad light."
		if(17)
