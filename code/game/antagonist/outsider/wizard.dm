...
/datum/antagonist/wizard/equip(mob/living/carbon/human/wizard_mob)
	...
	if(wizard_mob.backbag == 3)
		wizard_mob.equip_to_slot_or_del(new /obj/item/storage/backpack/satchel/norm(wizard_mob), slot_back)
	if(wizard_mob.backbag == 4)
		wizard_mob.equip_to_slot_or_del(new /obj/item/storage/backpack/satchel(wizard_mob), slot_back)
	if(wizard_mob.backbag == 5)
		wizard_mob.equip_to_slot_or_del(new /obj/item/storage/backpack(wizard_mob), slot_back)
...
/datum/antagonist/wizard/check_victory()
	...
	if(!survivor)
		...
		to_chat(world, SPAN_ALERTWARNING("The [(current_antagonists.len>1)?"[role_text_plural] have":"[role_text] has"] been killed by the crew!"))
