...
/datum/dna/proc/ResetUIFrom(mob/living/carbon/human/character)
	...
	var/hair = RSsprite_accessories.legacy_hair_lookup.Find(character.h_style)

	//! Facial Hair
	if(!character.f_style)
		character.f_style = "Shaved"
	var/beard	= RSsprite_accessories.legacy_facial_hair_lookup.Find(character.f_style)

	//! Demi Ears
	var/ear_style = 0
	if(character.ear_style)
		ear_style = RSsprite_accessories.legacy_ears_lookup.Find(character.ear_style.type)

	//! Demi Tails
	var/tail_style = 0
	if(character.tail_style)
		tail_style = RSsprite_accessories.legacy_tail_lookup.Find(character.tail_style.type)

	//! Demi Wings
	var/wing_style = 0
	if(character.wing_style)
		wing_style = RSsprite_accessories.legacy_wing_lookup.Find(character.wing_style.type)
	...
	SetUIValueRange(DNA_UI_EAR_STYLE,      ear_style + 1,  RSsprite_accessories.legacy_ears_lookup.len  + 1,  1)
	SetUIValueRange(DNA_UI_TAIL_STYLE,    tail_style + 1,  RSsprite_accessories.legacy_tail_lookup.len + 1,  1)
	SetUIValueRange(DNA_UI_PLAYERSCALE,  size_multiplier,     player_sizes_list.len,  1)
	SetUIValueRange(DNA_UI_WING_STYLE,    wing_style + 1,  RSsprite_accessories.legacy_wing_lookup.len + 1,  1)
	...
	SetUIValueRange(DNA_UI_HAIR_STYLE,  hair,  RSsprite_accessories.legacy_hair_lookup.len,       1)
	SetUIValueRange(DNA_UI_BEARD_STYLE, beard, RSsprite_accessories.legacy_facial_hair_lookup.len,1)
