...
/mob/proc/playsound_local(turf/turf_source, soundin, vol as num, vary, frequency, falloff, is_global, channel = 0, pressure_affected = TRUE, sound/S, preference, envwet, envdry, manual_x, manual_y, distance_multiplier = 1)
	if(isnull(envwet))															//IT'S FUCKING LOCAL, WHY WOULD IT BE GLOBAL
...
/client/proc/playtitlemusic(vol = 85)
	if(!SSticker || !all_lobby_tracks.len || !media)
		return
	//set waitfor = FALSE
	//UNTIL(SSticker.login_music) //wait for SSticker init to set the login music

	if(is_preference_enabled(/datum/client_preference/play_lobby_music))
		var/datum/track/T = pick(all_lobby_tracks) //this really should be handled by SSticker!
		media.push_music(T.url, world.time, clamp(vol/100, 0, 1))
...
/proc/get_sfx(soundin)
	if(istext(soundin))
		switch(soundin)
			...
			if ("thunder")
				soundin = pick('sound/effects/thunder/thunder1.ogg', 'sound/effects/thunder/thunder2.ogg', 'sound/effects/thunder/thunder3.ogg', 'sound/effects/thunder/thunder4.ogg',
			...
			if ("hunger_sounds")
				soundin = pick('sound/vore/growl1.ogg','sound/vore/growl2.ogg','sound/vore/growl3.ogg','sound/vore/growl4.ogg','sound/vore/growl5.ogg')

			if("classic_digestion_sounds")
				soundin = pick(
