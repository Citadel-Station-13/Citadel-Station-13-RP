...
/turf/simulated/floor/smooth_icon()
	if(flooring)
		if(flooring.smoothing_flags)
			smoothing_flags = flooring.smoothing_flags
		if(flooring.smoothing_groups)
			smoothing_groups = flooring.smoothing_groups
		if(flooring.canSmoothWith)
			canSmoothWith = flooring.canSmoothWith
	// Must be last so the new values are used.
	return ..()
