...
/atom/proc/say_overhead(var/message, whispering, message_range = 7, var/datum/language/speaking = null, var/list/passed_hearing_list)
	...
	if(length(speech_bubble_hearers))
		INVOKE_ASYNC(src, /atom/movable/proc/animate_chat, message = message, speaking = speaking, small = whispering, heard = passed_hearing_list | speech_bubble_hearers, 3 SECONDS)
