...
/obj/machinery/can_interact(mob/user)
	...
	if(isanimal(user))
		...
		if(user_as_animal.humanoid_hands)
			is_dextrous = TRUE

	if(!issilicon(user) && !is_dextrous && !user.has_hands)
	...
	if(issilicon(user)) // If we are a silicon, make sure the machine allows silicons to interact with it
		...
		return user.can_interact(src) //AIs don't care about petty mortal concerns like needing to be next to a machine to use it, but borgs do care somewhat
