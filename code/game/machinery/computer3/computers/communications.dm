	...
	if("MessageCentCom" in href_list)
		...
		if(authenticated==2)
			if(centcomm_message_cooldown)
				...
				return
			var/input = sanitize(input("Please choose a message to transmit to [SSmapping.legacy_map_config.boss_short] via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination.  Transmission does not guarantee a response.", "To abort, send an empty message.", ""))
			if(!input || !interactable())
				return
			CentCom_announce(input, usr)
			to_chat(usr, "Message transmitted.")
			log_game("[key_name(usr)] has made a [SSmapping.legacy_map_config.boss_short] announcement: [input]")
	...
	if (authenticated)
		...
		if (authenticated==2)
			...
			if(computer.emagged == 0)
				dat += "<BR>\[ <A HREF='?src=\ref[src];MessageCentCom'>Send an emergency message to [SSmapping.legacy_map_config.boss_short]</A> \]"
			else
