...
/obj/machinery/lapvend/proc/scan_id(var/obj/item/card/id/C, var/obj/item/I)
	...
	if(!CH)
		to_chat(usr, "[bicon(src)]<span class='warning'>No valid account number is associated with this card.</span>")
		return
	if(CH.security_level != 0) //If card requires pin authentication (ie seclevel 1 or 2)
		if(vendor_account)
			...
			if(D)
				...
				to_chat(usr, "[bicon(src)]<span class='warning'>Unable to access vendor account. Please record the machine ID and call [GLOB.using_map.boss_short] Support.</span>")
		else
			to_chat(usr, "[bicon(src)]<span class='warning'>Unable to access vendor account. Please record the machine ID and call [GLOB.using_map.boss_short] Support.</span>")
	else
