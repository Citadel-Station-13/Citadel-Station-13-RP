	...
	if(CH.security_level != 0) //If card requires pin authentication (ie seclevel 1 or 2)
		if(vendor_account)
			...
			if(D)
				...
				to_chat(usr, "[icon2html(thing = src, target = usr)]<span class='warning'>Unable to access vendor account. Please record the machine ID and call [SSmapping.legacy_map_config.boss_short] Support.</span>")
		else
			to_chat(usr, "[icon2html(thing = src, target = usr)]<span class='warning'>Unable to access vendor account. Please record the machine ID and call [SSmapping.legacy_map_config.boss_short] Support.</span>")
	else
