...
/obj/machinery/holoposter/update_icon()
	...
	if(machine_stat & BROKEN)
		...
		if((z in GLOB.using_map.station_levels) && SSsecurity_level.get_current_level_as_number()) // 0 is fine, everything higher is alert levels
			icon_state = "attention"
			examine_addon = "It warns you to remain calm and contact your supervisor as soon as possible."
			new_color =  "#AA7039"
			alerting = TRUE
		else if(alerting && !SSsecurity_level.get_current_level_as_number()) // coming out of alert
			alerting = FALSE
			set_rand_sprite()
			return
