...
/proc/InvalidPlayerTurf(turf/T as turf)
	return !(T && (T.z in SSmapping.legacy_map_config.player_levels))
...
/mob/living/carbon/human/tracking_status()
	...
	if(. == TRACKING_NO_COVERAGE)
		...
		if(T && (T.z in SSmapping.legacy_map_config.station_levels) && hassensorlevel(src, SUIT_SENSOR_TRACKING))
