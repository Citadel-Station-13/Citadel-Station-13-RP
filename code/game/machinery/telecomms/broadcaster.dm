...
/obj/machinery/telecomms/processor/Initialize(mapload)
	...
	component_parts += new /obj/item/stock_parts/manipulator(src)
	component_parts += new /obj/item/stock_parts/micro_laser/high(src)
  ...
  /* ###### Begin formatting and sending the message ###### */
	if (length(heard_masked) || length(heard_normal) || length(heard_voice) || length(heard_garbled) || length(heard_gibberish))
	  ...
	  /* --- Some miscellaneous variables to format the string output --- */
		...
		var/part_a = "<span class='[frequency_span_class(display_freq)]'>[icon2html(thing = radio, target = world)]<b>\[[freq_text]\][part_b_extra]</b> <span class='name'>" // goes in the actual output
  ...
  /* ###### Begin formatting and sending the message ###### */
	if (length(heard_normal) || length(heard_garbled) || length(heard_gibberish))
	  ...
	  /* --- Some miscellaneous variables to format the string output --- */
		...
		var/part_b = "</span><b> [icon2html(thing = radio, target = world)]\[[freq_text]\][part_b_extra]</b> <span class='message'>" // Tweaked for security headsets -- TLE
		var/part_blackbox_b = "</span><b> \[[freq_text]\]</b> <span class='message'>" // Tweaked for security headsets -- TLE
		var/part_c = "</span></span>"
