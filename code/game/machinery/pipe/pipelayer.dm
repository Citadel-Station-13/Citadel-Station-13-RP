...
/obj/machinery/pipelayer/attack_hand(mob/user as mob)
	...
	if(panel_open)
		...
		if(answer == "Yes")
			...
			user.visible_message("<span class='notice'>[user] removes [amount_ejected] sheet\s of [SSmaterials.material_display_name_by_id(MATERIAL_ID_STEEL)] from the \the [src].</span>",
				"<span class='notice'>You remove [amount_ejected] sheet\s of [SSmaterials.material_display_name_by_id(MATERIAL_ID_STEEL)] from \the [src].</span>")
		return
	if(!metal && !on)
		to_chat(user, "<span class='warning'>[src] doesn't work without metal.</span>")
		return
	on = !on
...
/obj/machinery/pipelayer/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if(istype(W, /obj/item/pipe))
		...
		if(!W.matter || W.matter[MATERIAL_ID_STEEL] < pipe_cost * SHEET_MATERIAL_AMOUNT)
			to_chat(user, "<span class='warning'>\The [W] doesn't contain enough [SSmaterials.material_display_name_by_id(MATERIAL_ID_STEEL)] to recycle.</span>")
			...
			qdel(W)
		return
	if(istype(W, /obj/item/stack/material) && W.get_material_name() == MATERIAL_ID_STEEL)
...
/obj/machinery/pipelayer/proc/eject_metal()
	...
	while (metal >= 1)
		var/datum/material/M = SSmaterials.material_by_id(MATERIAL_ID_STEEL)
...
/obj/machinery/pipelayer/proc/layPipe(var/turf/w_turf,var/M_Dir,var/old_dir)
	...
	P.matter = list(MATERIAL_ID_STEEL = pipe_cost * SHEET_MATERIAL_AMOUNT)
	P.attackby(W , src)
