...
/obj/machinery/pipelayer/attack_hand(mob/user as mob)
	...
	if(panel_open)
		...
		if(answer == "Yes")
			...
			user.visible_message("<span class='notice'>[user] removes [amount_ejected] sheet\s of [/datum/material/steel] from the \the [src].</span>",
				"<span class='notice'>You remove [amount_ejected] sheet\s of [/datum/material/steel] from \the [src].</span>")
		return
	if(!metal && !on)
...
/obj/machinery/pipelayer/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if(istype(W, /obj/item/pipe))
		...
		if(!W.matter || W.matter[/datum/material/steel] < pipe_cost * SHEET_MATERIAL_AMOUNT)
			to_chat(user, "<span class='warning'>\The [W] doesn't contain enough [/datum/material/steel] to recycle.</span>")
			...
			qdel(W)
		return
	if(istype(W, /obj/item/stack/material) && W.get_material_name() == /datum/material/steel)
...
/obj/machinery/pipelayer/proc/layPipe(var/turf/w_turf,var/M_Dir,var/old_dir)
	...
	P.matter = list(/datum/material/steel = pipe_cost * SHEET_MATERIAL_AMOUNT)
	P.attackby(W , src)
