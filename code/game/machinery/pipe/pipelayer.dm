...
/obj/machinery/pipelayer/attack_hand(mob/user as mob)
	...
	if(panel_open)
		...
		if(answer == "Yes")
			...
			user.visible_message("<span class='notice'>[user] removes [amount_ejected] sheet\s of [MATERIAL_ID_STEEL] from the \the [src].</span>",
				"<span class='notice'>You remove [amount_ejected] sheet\s of [MATERIAL_ID_STEEL] from \the [src].</span>")
		return
	if(!metal && !on)
...
/obj/machinery/pipelayer/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if(istype(W, /obj/item/pipe))
		...
		if(!W.matter || W.matter[MATERIAL_ID_STEEL] < pipe_cost * SHEET_MATERIAL_AMOUNT)
			to_chat(user, "<span class='warning'>\The [W] doesn't contain enough [MATERIAL_ID_STEEL] to recycle.</span>")
