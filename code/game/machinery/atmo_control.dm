...
/obj/machinery/air_sensor/process()
	if(on)
		...
		if(output>4)
			...
			if(total_moles > 0)
				if(output&4)
					signal.data["oxygen"] = round(100*air_sample.gases[/datum/gas/oxygen]/total_moles,0.1)
				if(output&8)
					signal.data["phoron"] = round(100*air_sample.gases[/datum/gas/phoron]/total_moles,0.1)
				if(output&16)
					signal.data["nitrogen"] = round(100*air_sample.gases[/datum/gas/nitrogen]/total_moles,0.1)
				if(output&32)
					signal.data["carbon_dioxide"] = round(100*air_sample.gases[/datum/gas/carbon_dioxide]/total_moles,0.1)
			else
