...
/obj/machinery/air_sensor/process(delta_time)
	if(on)
		...
		if(output>4)
			...
			if(total_moles > 0)
				if(output&4)
					signal.data["oxygen"] = round(100*air_sample.gas[/datum/reagent/gas/oxygen]/total_moles,0.1)
				if(output&8)
					signal.data["phoron"] = round(100*air_sample.gas[/datum/reagent/gas/phoron]/total_moles,0.1)
				if(output&16)
					signal.data["nitrogen"] = round(100*air_sample.gas[/datum/reagent/gas/nitrogen]/total_moles,0.1)
				if(output&32)
					signal.data["carbon_dioxide"] = round(100*air_sample.gas[/datum/reagent/gas/carbon_dioxide]/total_moles,0.1)
			else
