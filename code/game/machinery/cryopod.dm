...
/obj/machinery/cryopod/proc/despawn_occupant(mob/to_despawn, silent = FALSE)
	if(to_despawn.has_alerting_status_effect())
		var/datum/status_effect/effect = to_despawn.has_alerting_status_effect()
		log_and_message_admins("[key_name(to_despawn)] still had a long lasting status effect([effect.identifier]) while cryoing, have an eye out should they respawn.")
