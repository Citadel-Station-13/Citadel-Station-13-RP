...
/obj/machinery/cryopod/proc/despawn_occupant(var/mob/to_despawn, silent = FALSE)
	...
	if(!silent)
		...
		announce.autosay("[to_despawn.real_name], [to_despawn.mind.role_alt_title], [on_store_message]", "[on_store_name]", announce_channel, SSmapping.legacy_map_config.get_map_levels(z, TRUE, om_range = DEFAULT_OVERMAP_RANGE))
