...
/obj/machinery/computer/cryopod/ui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui)
	...
	if(!ui)
		ui = new(user, src, "CryoStorageVr", storage_name) // Use our own template for our custom data
		ui.open()

/obj/machinery/computer/cryopod/ui_data(mob/user, datum/tgui/ui, datum/ui_state/state)
	var/list/data = ..()
