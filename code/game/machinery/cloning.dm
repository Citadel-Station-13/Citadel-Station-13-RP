...
/obj/machinery/clonepod/proc/get_biomass()
	...
	if(LAZYLEN(containers))
		for(var/obj/item/reagent_containers/glass/G in containers)
			biomass_count += G.reagents.reagent_volumes[casted.id]
...
/obj/machinery/clonepod/proc/remove_biomass(amount = CLONE_BIOMASS)		//Just in case it doesn't get passed a new amount, assume one clone
	var/left = amount
	for(var/obj/item/reagent_containers/glass/container in containers)
		var/removing = min(left, container.reagents.get_reagent_amount(/datum/reagent/nutriment/biomass))
		if(!removing)
			continue
		left -= removing
		container.reagents.remove_reagent(/datum/reagent/nutriment/biomass, removing)
	return left == 0
