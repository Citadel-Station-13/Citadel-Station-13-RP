...
/obj/machinery/gear_painter/ui_data(mob/user)
	...
	if(inserted)
		...
		.["item"]["sprite"] = icon2base64(get_flat_icon_simple(inserted,dir=SOUTH,no_anim=TRUE))
...
/obj/machinery/gear_painter/proc/build_preview()
	if(inserted) //sanity
		...
		switch(active_mode)
			if(COLORMATE_MATRIX)
				...
				if(!check_valid_color(cm, usr))
					return get_flat_icon_simple(inserted, dir=SOUTH, no_anim=TRUE)
		...
		var/icon/preview = get_flat_icon_simple(inserted, dir=SOUTH, no_anim=TRUE)
