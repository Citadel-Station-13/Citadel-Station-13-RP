...
/obj/machinery/computer/timeclock/proc/makeOnDuty(var/newrank, var/newassignment)
	...
	if(newjob)
		...
		announce.autosay("[card.registered_name] has moved On-Duty as [card.assignment].", "Employee Oversight", channel, zlevels = SSmapping.get_connected_levels(get_z(src)))
	return
...
/obj/machinery/computer/timeclock/proc/makeOffDuty()
	...
	if(ptojob)
		...
		announce.autosay("[card.registered_name], [oldtitle], has moved Off-Duty.", "Employee Oversight", channel, zlevels = SSmapping.get_connected_levels(get_z(src)))
	return
