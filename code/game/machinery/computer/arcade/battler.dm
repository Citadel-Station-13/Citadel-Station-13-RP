...
//		gamerSkill = usr.mind.get_skill_level(/datum/skill/gaming)
	...
	if (href_list["close"])
		...
		if(obj_flags & OBJ_FLAG_EMAGGED)
			Reset()
			obj_flags &= ~OBJ_FLAG_EMAGGED
	...
	if(enemy_hp <= 0)
		if(!gameover)
			...
			if(obj_flags & OBJ_FLAG_EMAGGED)
				...
				prizevend(user)
				xp_gained += 50
			//SSblackbox.record_feedback("nested tally", "arcade_results", 1, list("win", (obj_flags & OBJ_FLAG_EMAGGED ? "emagged":"normal")))
		...
		if(obj_flags & OBJ_FLAG_EMAGGED)
			...
			if (istype(living_user))
				living_user.gib()
		//SSblackbox.record_feedback("nested tally", "arcade_results", 1, list("loss", "hp", (obj_flags & OBJ_FLAG_EMAGGED ? "emagged":"normal")))
