	...
	switch(src.state)
		if(STATE_DEFAULT)
			if (src.authenticated)
				...
				if (src.authenticated==2)
					if(src.emagged == 0)
						dat += "<BR>\[ <A HREF='?src=\ref[src];operation=MessageCentCom'>Send an emergency message to [using_map_legacy().boss_short]</A> \]"
					else
...
/proc/call_shuttle_proc(var/mob/user)
	...
	if(deathsquad.deployed)
		to_chat(user, "[using_map_legacy().boss_short] will not allow the shuttle to be called. Consider all contracts terminated.")
		return
	...
	if(SSemergencyshuttle.going_to_centcom())
		to_chat(user, "The emergency shuttle may not be called while returning to [using_map_legacy().boss_short].")
		return
...
/proc/init_shift_change(var/mob/user, var/force = 0)
	...
	if(SSemergencyshuttle.going_to_centcom())
		to_chat(user, "The shuttle may not be called while returning to [using_map_legacy().boss_short].")
		return
	...
	if(!force)
		if(SSemergencyshuttle.deny_shuttle)
			to_chat(user, "[using_map_legacy().boss_short] does not currently have a shuttle available in your sector. Please try again later.")
			return
