		...
		if("MessageCentCom")
			if(src.authenticated==2)
				if(centcomm_message_cooldown)
					...
					return
				var/input = sanitize(input("Please choose a message to transmit to [SSmapping.legacy_map_config.boss_short] via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination.  Transmission does not guarantee a response. There is a 30 second delay before you may send another message, be clear, full and concise.", "To abort, send an empty message.", ""))
				if(!input || !(usr in view(1,src)))
					return
	...
	switch(src.state)
		if(STATE_DEFAULT)
			if (src.authenticated)
				...
				if (src.authenticated==2)
					if(src.emagged == 0)
						dat += "<BR>\[ <A HREF='?src=\ref[src];operation=MessageCentCom'>Send an emergency message to [SSmapping.legacy_map_config.boss_short]</A> \]"
					else
