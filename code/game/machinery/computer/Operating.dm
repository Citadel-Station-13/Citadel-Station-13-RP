...
/obj/machinery/computer/operating
	...
	var/oxyAlarm = 10 //oxy damage at which the computer will beep
...
/obj/machinery/computer/operating/ui_data(mob/user)
	...
	if(occupant)
		...
		occupantData["minHealth"] = occupant.maxHealth - 200
...
/obj/machinery/computer/operating/ui_interact(action, params)
	if(..())
		return TRUE
	...
	if(table && table.check_victim())
		if(verbose)
			if(patientName!=table.victim.name)
				...
				global_announcer.autosay("Patient detected, analyzing...", "[src]", "Medical")
				victim = table.victim
				global_announcer.autosay("[victim.real_name], Blood Type: [victim.dna.b_type], [victim.stat ? "Unconcious" : "Awake"]","[src]", "Medical")
				SStgui.update_uis(src)
			if(nextTick < world.time)
				nextTick=world.time + OP_COMPUTER_COOLDOWN
				if(crit && victim.health <= (table.victim.maxHealth / 2))
				...
				if(healthAnnounce && ((victim.health / victim.maxHealth) * 100) <= healthAlarm)
					global_announcer.autosay("[table.victim.real_name], [round(((table.victim.health / table.victim.maxHealth) * 100))]% health.","[src]", "Medical")
