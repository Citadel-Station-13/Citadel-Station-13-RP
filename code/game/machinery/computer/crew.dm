...
/datum/crewmonitor/ui_data(mob/user)
	...
	if(!set_z)
		...
		set_z = T.z
	var/list/zdata = update_data(set_z)
	. = list()
	.["sensors"] = zdata
	...
	for(var/mob/living/carbon/human/H in living_mob_list)
		...
		if ((H.z == 0 || H.z == z) && istype(H.w_uniform, /obj/item/clothing/under) || nanite_sensors)
			...
			if (nanite_sensors || ((U.has_sensor > 0) && U.sensor_mode))
				pos = H.z == 0 || (nanite_sensors || U.sensor_mode == 3) ? get_turf(H) : null
				...
				if (nanite_sensors || U.sensor_mode >= 1)
					life_status = (!H.stat ? TRUE : FALSE)
				else
					life_status = null

				if (nanite_sensors || U.sensor_mode >= 2)
				...
				if (nanite_sensors || U.sensor_mode >= 3)
					if (!pos)
						pos = get_turf(H)
				...
				var/total_list = list("name" = name, "assignment" = assignment, "ijob" = ijob, "life_status" = life_status, "oxydam" = oxydam, "toxdam" = toxdam, "burndam" = burndam, "brutedam" = brutedam, "totaldam" = totaldam, "area" = area, "pos_x" = pos_x, "pos_y" = pos_y, "can_track" = TRUE)
...
/datum/crewmonitor/ui_act(action,params)
	...
	switch (action)
		if ("select_person")
			...
			. = TRUE
		if("setZLevel")
			set_z = clamp(params["mapZLevel"], MAP_MINZ, MAP_MAXZ)
