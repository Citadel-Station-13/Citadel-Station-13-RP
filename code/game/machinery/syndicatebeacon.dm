...
/obj/machinery/syndicate_beacon/attack_hand(mob/user, datum/event_args/clickchain/e_args)
	usr.set_machine(src)
...
/obj/machinery/power/singularity_beacon/attack_hand(mob/user, datum/event_args/clickchain/e_args)
	if(anchored)
...
/obj/machinery/syndicate_beacon/virgo/attack_hand(mob/user, datum/event_args/clickchain/e_args)
	usr.set_machine(src)
	var/dat = "<font color=#005500><i>Scanning [pick("retina pattern", "voice print", "fingerprints", "dna sequence")]...<br>Identity confirmed,<br></i></font>"
	if(istype(user, /mob/living/carbon/human) || istype(user, /mob/living/silicon/ai))
		if(is_special_character(user))
