/proc/sql_poll_population()
	...
	if(!SSdbcore.Connect())
		...
		var/datum/db_query/query = SSdbcore.NewQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("population")] (playercount, admincount, time) VALUES (:pc, :ac, NOW())",
			list(
...
/proc/sql_report_death(mob/living/carbon/human/H)
	...
	if(!SSdbcore.Connect())
		...
		var/datum/db_query/query = SSdbcore.NewQuery(
			"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("death")] (name, byondkey, job, special, pod, tod, laname, lakey, gender, bruteloss, fireloss, brainloss, oxyloss, coord) VALUES \
...
/proc/sql_commit_feedback()
	...
	if(!SSdbcore.Connect())
		...
		var/datum/db_query/max_query = SSdbcore.RunQuery(
			"SELECT MAX(roundid) AS max_round_id FROM [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("feedback")]",
			list(),
		)
		...
		for(var/datum/feedback_variable/item in content)
			...
			var/datum/db_query/query = SSdbcore.NewQuery(
				"INSERT INTO [fetch_local_or_tDB_PREFIX_TABLE_NAMEhrow("feedback")] (id, roundid, time, variable, value) VALUES (null, :rid, Now(), :var, :val)",
				list(
					"rid" = newroundid,
