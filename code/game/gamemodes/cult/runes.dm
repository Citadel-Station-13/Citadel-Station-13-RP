	...
	while(target in converting)
		...
		if (!target.can_feel_pain())
			...
			target.visible_message(\
				SPAN_WARNING("[target] writhes in pain as the markings below [TT.him] glow a bloody red."), \
...
/obj/effect/rune/proc/emp(U, range_red) //range_red - var which determines by which number to reduce the default emp range, U is the source loc, needed because of talisman emps which are held in hand at the moment of using and that apparently messes things up -- Urist
	...
	playsound(U, 'sound/items/Welder2.ogg', 25, TRUE)
...
/obj/effect/rune/proc/drain()
	...
	usr.visible_message(\
		SPAN_DANGER("Blood flows from the rune into [usr]!"),
	...
	if(drain>=50)
		user.visible_message(
			SPAN_DANGER("[user]'s eyes give off eerie red glow!"),
...
/obj/effect/rune/proc/raise()
	...
	if(!cult.can_become_antag(corpse_to_raise.mind) || jobban_isbanned(corpse_to_raise, "cultist"))
		...
		for(var/mob/observer/dead/ghost in player_list)
			if(ghost.mind == corpse_to_raise.mind)
				to_chat(ghost, SPAN_GHOSTALERT("The cultist [usr.real_name] is trying to \
	...
	if(corpse_to_raise.client)
		...
		corpse_to_raise.visible_message(
			SPAN_WARNING("[corpse_to_raise]'s eyes glow with a faint red as [TU.he] stand[TU.s] up, slowly starting to breathe again."),
		...
		body_to_sacrifice.visible_message(
			SPAN_DANGER("[body_to_sacrifice] is torn apart, a black smoke swiftly dissipating from [TT.his] remains!"),
...
/obj/effect/rune/proc/ajourney() //some bits copypastaed from admin tools - Urist
	if(usr.loc == src.loc)
		...
		usr.visible_message(
		SPAN_WARNING("[usr]'s eyes glow blue as [TU.he] freeze[TU.s] in place, absolutely motionless."),
...
/obj/effect/rune/proc/manifest()
	...
	for(var/mob/observer/dead/O in this_rune.loc)
		...
		if(O.mind && O.mind.current && O.mind.current.stat != DEAD)
			continue
		if(!(O.client.prefs.be_special & BE_CULTIST))
			continue
		ghost = O
		break
	if(!ghost)
	...
	if(D)
		D.visible_message(
		SPAN_DANGER("[D] slowly dissipates into dust and bones."),
...
/obj/effect/rune/proc/talisman()
	var/obj/item/paper/newtalisman
	var/unsuitable_newtalisman = FALSE
...
/obj/effect/rune/proc/mend()
	...
	runedec+=10
	user.visible_message(
		SPAN_DANGER("\The [user] keels over dead, [TU.his] blood glowing blue as it escapes [TU.his] body and dissipates into thin air."),
...
/obj/effect/rune/proc/communicate()
	. = TRUE // Default output is 1. If the rune is deleted it will return TRUE
	...
	if(istype(src,/obj/effect/rune)) ///When invoked as rune, flash and stun everyone around.
		...
		if(N)
			for(var/mob/O in viewers(T, null))
				O.show_message(SPAN_BOLDWARNING("[] invokes a talisman at [], but they are unaffected!"), 1)
		else
			for(var/mob/O in viewers(T, null))
				O.show_message(SPAN_BOLDWARNING("[] invokes a talisman at []"), 1)
