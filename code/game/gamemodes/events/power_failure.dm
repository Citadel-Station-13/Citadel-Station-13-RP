...
/proc/power_failure(var/announce = 1)
	...
	for(var/obj/machinery/power/smes/S in machines)
		...
		if(current_area.type in skipped_areas || !(S.z in SSmapping.legacy_map_config.station_levels))
			continue
	...
	for(var/obj/machinery/power/apc/C in machines)
		if(!C.is_critical && C.cell && (C.z in SSmapping.legacy_map_config.station_levels))
			C.cell.charge = 0
...
/proc/power_restore(var/announce = 1)
	...
	for(var/obj/machinery/power/apc/C in machines)
		if(C.cell && (C.z in SSmapping.legacy_map_config.station_levels))
