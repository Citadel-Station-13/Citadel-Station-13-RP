...
/datum/objective/anti_revolution/demote/find_target()
	...
	if(target && target.current)
		var/datum/gender/T = GLOB.gender_datums[target.current.get_visible_gender()]
		explanation_text = "[target.current.real_name], the [target.assigned_role]  has been classified as harmful to [using_map_legacy.company_name]'s goals. Demote [T.him] to assistant."
	else
		explanation_text = "Free Objective"
	return target
...
/datum/objective/anti_revolution/demote/find_target_by_role(role, role_type = 0)
	...
	if(target && target.current)
		var/datum/gender/T = GLOB.gender_datums[target.current.get_visible_gender()]
		explanation_text = "[target.current.real_name], the [!role_type ? target.assigned_role : target.special_role] has been classified as harmful to [using_map_legacy.company_name]'s goals. Demote [T.him] to assistant."
	else
		explanation_text = "Free Objective"
	return target
