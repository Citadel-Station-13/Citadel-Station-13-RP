...
/client/proc/response_team()
	...
	if(send_emergency_team)
		to_chat(usr, "<span class='danger'>[SSmapping.legacy_map_config.boss_name] has already dispatched an emergency response team!</span>")
		return
...
proc/trigger_armed_response_team(var/force = 0)
	...
	if(!prob(send_team_chance))
		command_announcement.Announce("It would appear that an emergency response team was requested for [station_name()]. Unfortunately, we were unable to send one at this time.", "[SSmapping.legacy_map_config.boss_name]")
		can_call_ert = 0 // Only one call per round, ladies.
		return
	if(silent_ert == 0)
		command_announcement.Announce("It would appear that an emergency response team was requested for [station_name()]. We will prepare and send one as soon as possible.", "[SSmapping.legacy_map_config.boss_name]")
