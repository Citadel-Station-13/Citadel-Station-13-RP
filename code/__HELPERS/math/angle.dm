...
/proc/get_visual_angle(atom/start, atom/end)
	if(!start || !end)
		return 0
	var/dy =(WORLD_ICON_SIZE * end.y + end.pixel_y) - (WORLD_ICON_SIZE * start.y + start.pixel_y)
	var/dx =(WORLD_ICON_SIZE * end.x + end.pixel_x) - (WORLD_ICON_SIZE * start.x + start.pixel_x)
	if(!dy)
		return (dx >= 0) ? 90 : 270
	. = arctan(dx/dy)
	if(dy < 0)
...
/proc/get_visual_angle_raw(start_x, start_y, start_pixel_x, start_pixel_y, end_x, end_y, end_pixel_x, end_pixel_y)
	var/dy = (WORLD_ICON_SIZE * end_y + end_pixel_y) - (WORLD_ICON_SIZE * start_y + start_pixel_y)
	var/dx = (WORLD_ICON_SIZE * end_x + end_pixel_x) - (WORLD_ICON_SIZE * start_x + start_pixel_x)
	if(!dy)
		return (dx >= 0) ? 90 : 270
	. = arctan(dx/dy)
	if(dy < 0)
