	...
	for(var/current_list in all_products)
		...
		for(var/entry in current_list[1])
			...
			if(!(applies_to & NIF_SYNTHETIC))
				...
				context = " (Syn Only)"
			var/name = "[initial(NS.name)] \[[initial(NS.complexity)]\][context]"
...
//Anyone can browse nifsofts, and unlock them so others can buy. The alternative is handing the ID over and that's honestly worse.
...
/obj/machinery/vending/nifsoft_shop/proc/bootleg(mob/user)
	if(!ishuman(user))
		return 0

	var/mob/living/carbon/human/H = user
	if(H.nif && H.nif.stat == NIF_WORKING && H.nif.nif_flags & NIF_IGNORE_RESTRICTIONS)
