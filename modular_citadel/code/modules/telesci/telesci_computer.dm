...
/obj/machinery/computer/telescience/interact(mob/user)
	...
	if(!telepad)
		...
		t += "<div class='statusDisplay'>Destabilisation: [potency]</div>"
...
/obj/machinery/computer/telescience/proc/teleport(mob/user)
	...
	if(!(z_co in using_map.player_levels))
		telefail()
		temp_msg = "ERROR!<BR>Sector is outside known time and space!"
		return
	if(teles_left > 0)
		doteleport(user)
	else
...
//			inserted_gps.locked_location = last_target
			temp_msg = "ERROR!<BR>Feature currently unavailable."
		else
