...
export const TelecommsPDALog = (props, context) => {
  ...
  const [tab, setTab] = useSharedState(context, 'tab', 'pdalog-servers');
  const valid = selected && selected.status && authenticated;
  if (hack_status) {
    return (
      // should have used en -> jp unicode -> other encoding method->utf8
      <Window theme="ntos" resizable width={727} height={510}>
            ...
            <i>
              {silicon && !hack_status.emagging ? (
                <Fragment>
                  ...
                  It will take 20 seconds for every character that the password
                  has.
                  <br />
                  In the meantime, this console can reveal your true intentions
                  if you let someone access it. Make sure no humans enter the
      ...
      <Window.Content scrollable>
        <Fragment>
          ...
          {tab === 'pdalog-servers' ? (
            ...
            <Fragment>
              {tab === 'pdalog-message' && authenticated && <TeleLogs />}
              {tab === 'pdalog-reqmsg' && authenticated && (
                <TeleLogs msgs_log />
              )}
              {tab === 'pdalog-custommsg' && authenticated && <CustomMsg />}
            </Fragment>
          )}
        </Fragment>
      </Window.Content>
...
export const TeleLogs = (props, context) => {
  ...
  const { act, data } = useBackend(context);
  const { message_logs = [], recon_logs = [] } = data;
              ...
              {!!msgs_log && (
                <Fragment>
                  ...
                  <LabeledList.Item
                    label="Priority"
                    color={
                      message.priority in prioritycolorMap
