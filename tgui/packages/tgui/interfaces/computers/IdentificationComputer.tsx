...
export const IdentificationComputer = (props, context) => {
  ...
  const [currentTab, setCurrentTab] = useLocalState<number>(
    context,
    'currentTab',
    0,
  );
    ...
    <Window width={500} height={700}>
      <Window.Content scrollable>
        <Section title="Authentication">
          <IDSlot
            card={IDCardOrDefault(data.auth_card)}
            onClick={() => act('auth')}
          />
        </Section>
        <Tabs>
        ...
        {currentTab === 0 && (
          <>
            <Section title="Target">
              <IDSlot
                card={IDCardOrDefault(data.modify_card)}
                onClick={() => act('modify')}
              />
            </Section>
            <Section>
              {data.authed_cardmod ? (
                data.modify_card ? (
                  ...
                  <NoticeBox warning>Please insert target card.</NoticeBox>
                )
              ) : (
                <NoticeBox warning>
