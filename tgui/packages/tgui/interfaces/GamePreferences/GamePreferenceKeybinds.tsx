...
interface GamePreferenceKeybindScreenProps
  extends GamePreferenceKeybindMiddlware {
  readonly addBind: (
    id: string,
    key: GamePreferenceKeybindDescriptor,
...
const preprocessKeybinds = (
  keybinds: GamePreferenceKeybind[],
...
const computeBoundKeys = (
  keysBound: Record<string, string[]>,
...
const HOTKEY_MODE_DESCRIPTION = (
  <>
    ...
    <br />
    If it is on <b>Hotkeys</b>, the game will always attempt to force you to map
    focus, meaning keypresses are sent directly to the map instead of the input.
    You will still be able to use the command bar, but you need to tab to do it
    every time you click on the game map.
    <br />
    If it is on <b>Input</b>, the game will not force focus away from the input
    bar, and you can switch focus using TAB between these two modes: If the
    input bar is pink, that means that you are in non-hotkey mode, sending all
    keypresses of the normal alphanumeric characters, punctuation, spacebar,
    backspace, enter, etc, typing keys into the input bar. If the input bar is
    white, you are in hotkey mode, meaning all keypresses go into the
    game&apos;s keybind handling system unless you manually click on the input
    bar to shift focus there.
    <br />
    Input mode is the closest thing to the old input system.
    <br />
    <b>IMPORTANT:</b> While in input mode&apos;s non hotkey setting (tab
    toggled), Ctrl + KEY will send KEY to the keybind system as the key itself,
    not as Ctrl + KEY. This means Ctrl + T/W/A/S/D/all your familiar stuff still
    works, but you won&apos;t be able to access any regular Ctrl binds.
    <br />
  </>
);

export const GamePreferenceKeybindScreen = (
  props: GamePreferenceKeybindScreenProps,
  context,
) => {
  ...
  // damnit
  const keysByKeybind: Record<string, string[]> = computeBoundKeys(
    props.bindings,
  );
  ...
  const [activeCapture, setActiveCapture] = useLocalState<InfernoNode | null>(
    context,
    'activeKeyCapture',
    null,
  );
    ...
    <Section fill scrollable>
      <Stack vertical>
        <Stack.Item>
          ...
          {Object.entries(sortedByCategory)
            .sort(([c1, k1], [c2, k2]) => c1.localeCompare(c2))
                  ...
                  {keybinds.map((keybind) => {
                    ...
                    return (
                      <Table.Row
                        key={keybind.id}
                        height={
                          KEYBIND_ROW_HEIGHT_I_FUCKING_HATE_TABLES_WEBVIEW_WHEN
                        }
                        style={{
                      ...
                      >
                        ...
                        {[0, 1, 2].map((i) => {
                          let bind: string | null = boundKeys[i] || null;
                          return (
                            <Table.Cell
                              width="20%"
                              key={i}
          ...
          <Stack vertical>
            <Stack.Item>
              <Box textAlign="center">
                Capturing key-binding for <b>{this.props.keybind.name}</b>.
                <br />
                {!!this.props.existing && (
                  <>
                    Existing keybind is <b>{this.props.existing}.</b>
                    <br />
                  </>
                )}
                Press <b>Esc</b> to{' '}
                {this.props.existing ? 'removing existing bind' : 'cancel'}.
              </Box>
            </Stack.Item>
            <Stack.Item>
              {this.state.alt && 'Alt-'}
              {this.state.ctrl && 'Ctrl-'}
              {this.state.shift && 'Shift-'}
              {this.state.numpad && 'Numpad'}
              {this.state.terminal ? keyCodeToByond(this.state.terminal) : ''}
            </Stack.Item>
          </Stack>
        </Section>
      </Dimmer>
