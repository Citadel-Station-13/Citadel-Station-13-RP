...
interface GamePreferenceToggleScreenProps
  extends GamePreferenceTogglesMiddleware {
...
const collectPreferenceToggles = (
  toggles: Record<string, GamePreferenceToggleSchema>,
...
export const GamePreferenceToggleScreen = (
  props: GamePreferenceToggleScreenProps,
  context,
) => {
  const collected: Record<string, string[]> = collectPreferenceToggles(
    props.toggles,
  );
  return (
                ...
                {keys.map((key) => {
                  ...
                  return (
                    <Table.Row key={toggle.key}>
                      ...
                      <Table.Cell>
                        <Button.Checkbox
                          checked={props.states[toggle.key]}
                          onClick={() =>
                            props.toggleAct(
                              toggle.key,
                              !props.states[toggle.key],
                            )
                          }
                          content={
                            props.states[toggle.key]
