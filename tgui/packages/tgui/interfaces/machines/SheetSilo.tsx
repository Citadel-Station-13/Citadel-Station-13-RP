...
export const SheetSilo = (props, context) => {
  ...
  const [dropAmounts, setDropAmounts] = useLocalState<Record<string, number>>(
    context,
    'dropAmounts',
    {},
  );
    ...
    <Window width={350} height={500} title="Materials Silo">
      <Window.Content>
        <Section fill title="Storage" scrollable>
          <Stack vertical fill>
            {Object.entries(data.stored)
              .sort(([k1, v1], [k2, v2]) =>
                (
                  data.materialContext.materials[k1]?.name || 'AAAAA'
                ).localeCompare(
                  ...
                  <Stack>
                    ...
                    >
                      <Box style={{ 'align-self': 'center' }} bold>
                        {data.materialContext.materials[k]?.name || 'UNKNOWN'} -{' '}
                        {v}
                      </Box>
                    </Stack.Item>
                    <Stack.Item>
                      <Button
                        icon="minus"
                        onClick={() =>
                          setDropAmount(k, (dropAmounts[k] || 1) - 1)
                        }
                      />
                    </Stack.Item>
                    <Stack.Item>
