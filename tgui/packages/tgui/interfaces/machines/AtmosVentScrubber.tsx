...
export const AtmosVentScrubberControl = (
  props: AtmosVentScrubberControlProps,
) => {
  return (
    <Section
      {...props}
    ...
    >
      <LabeledList>
        ...
        <LabeledList.Item label="Mode">
          <Button
            icon={props.state.siphon ? 'sign-in-alt' : 'filter'}
      ...
      {!props.state.siphon && (
        <AtmosFilterList
          ...
          selectId={(id) =>
            props.idToggle?.(id, !props.state.scrubIDs.includes(id))
          }
          selectGroup={(group) =>
            props.groupToggle?.(group, !(props.state.scrubGroups & group))
          }
        />
      )}
    </Section>
