...
export const AtmosTrinaryFilterControl = (
  props: AtmosTrinaryFilterControlProps,
  context,
) => {
  return (
    <Section title="Filter" {...props}>
      <AtmosFilterList
        ...
        selectedGroups={
          typeof props.filtering === 'number'
            ? (props.filtering as number)
            : AtmosGasGroupFlags.None
        }
        selectedIds={
          typeof props.filtering === 'string' ? [props.filtering as string] : []
        }
      ...
      additionalListItems={
        <LabeledList.Item label="Flow">
          <NumberInput
            ...
            value={data.rate}
            onChange={(e, val) => act('rate', { rate: val })}
            unit="L/s"
          />
        </LabeledList.Item>
      }
