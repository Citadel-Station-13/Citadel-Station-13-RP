...
const ACCOUNT_SECURITY_DESCRIPTIONS: AccountSecurityDescription[] = [
  {
    level: 0,
    desc: 'Only account number required, automatically scanned from ID in proximity.',
  },
...
const LoginElement = (props, context) => {
  const { act, data } = useBackend<ATMContext>(context);
  const [epin, setPin] = useLocalState<number>(context, 'epin', 0);
  const [eacc, setAcc] = useLocalState<number>(context, 'eacc', 0);
  return (
    <Flex width={200} ml={2} wrap>
      <Flex.Item>
        ...
        <LabeledList>
          ...
          <Button
            onClick={() =>
              act('attempt_authentication', { pin: epin, acc: eacc })
            }
            icon="key"
          >
            {' '}
...
const LockedElement = (props, context) => {
  ...
  return (
    ...
    >
      ...
      <Flex.Item>
        <Box textAlign="center" fontSize="32">
          Welcome to this Nanotrasen Automatic Teller Machine. Unfortunately, we
          are unable to service your requests at this time. This terminal has
          been taken out of service due to a security incident. Please contact
          the Command department for more information.
        </Box>
      </Flex.Item>
      <br />
      <Flex.Item>
      ...
      <br />
      <Button onClick={() => act('logout')} icon="key">
        Logout
      </Button>
    </Flex>
  );
...
const ATMElement = (props, context) => {
  const { act, data } = useBackend<ATMContext>(context);
  const [TransferTarget, setTransferTarget] = useLocalState<number>(
    context,
    'TransferTarget',
    1,
  );
  const [TransferAmount, setTransferAmount] = useLocalState<number>(
    context,
    'TransferAmount',
    1,
  );
  const [TransferPurpose, setTransferPurpose] = useLocalState<String>(
    context,
    'TransferPurpose',
    '',
  );
  const [WithdrawAmount, setWithdrawAmount] = useLocalState<number>(
    context,
    'WithdrawAmount',
    1,
  );
  const [EWallet, setEWallet] = useLocalState<boolean>(
    context,
    'EWallet',
    false,
  );
  const [Security, setSecurity] = useLocalState<number>(
    context,
    'Security',
    data.current_account_security_level,
  );
  ...
  if (data.acc_suspended) {
    return (
      ...
      >
        ...
        <Flex.Item>
          <Box textAlign="center" fontSize="32">
            Welcome to this Nanotrasen Automatic Teller Machine. Unfortunately,
            we are unable to service your requests at this time. This account
            has been suspended by the authority of the authorized feduciary
            aboard this facility. Please contact the Command department for more
            information.
          </Box>
        </Flex.Item>
        <br />
  ...
  return (
    <Flex justify="space-around" direction="column" ml={2} mr={2}>
      <Flex.Item>
        ...
        Current Funds: <b>{data.balance}</b> cr
        <br />
        Inserted ID:{' '}
        <Button
          onClick={() => act('eject_card')}
        ...
        <br />
        For your security, you will be logged out in <b>{data.logout_time}.</b>
        <br />
      </Flex.Item>
      <Divider />
      <Flex.Item>
        ...
        >
          <LabeledList>
            <LabeledList.Item label="Security Setting">
              <Button.Checkbox
                ...
                onClick={() => {
                  ...
                  act('change_security_level', {
                    new_security_level: Security,
                  });
                }}
              >
                Minimal Security
              ...
              <Button.Checkbox
                ...
                onClick={() => {
                  setSecurity(AccountSecurityLevels.SECURITY_LEVEL_MED);
                  act('change_security_level', {
                    new_security_level: Security,
                  });
                }}
              ...
              <Button.Checkbox
                ...
                onClick={() => {
                  setSecurity(AccountSecurityLevels.SECURITY_LEVEL_MAX);
                  act('change_security_level', {
                    new_security_level: Security,
                  });
                }}
            ...
            <LabeledList.Item label="Setting Information">
              {
                ACCOUNT_SECURITY_DESCRIPTIONS.find(
                  (option) => option.level === Security,
                )?.desc
              }
            </LabeledList.Item>
          </LabeledList>
        ...
        <Collapsible
          ...
          headerProps={{ icon: 'money-bill' }}
        >
          <LabeledList>
            {Object.entries(data.transaction_log).map(
              ([numString, logEntry]) => {
                return (
        ...
        <Collapsible
          ...
          headerProps={{ icon: 'money-bill-alt' }}
        >
          <LabeledList>
            <LabeledList.Item label="Amount">
              <Input
