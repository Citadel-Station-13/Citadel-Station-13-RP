...
export const SpawnSearch = () => {
  ...
  const filterItems = () => {
    ...
    if (isRegex !== regexSearch) { act('setRegexSearch', { regexSearch: regexSearch }); }
    ...
    if (finalizer === '*' || finalizer === '!') { filterQuery = filterQuery.slice(0, filterQuery.length - 1); }
      ...
      <Window.Content>
        <Section
          ...
          onKeyDown={(event) => {
            ...
            if (keyCode === KEY_N && event.altKey) { act('setNameSearch', { searchNames: !searchNames }); }

            if (keyCode === KEY_F && event.altKey) { act('setFancyTypes', { fancyTypes: !fancyTypes }); }
          }}
        >
          <Stack fill vertical>
            <Stack.Item grow>
