...
const StackCraftingEntry = (props: StackCraftingEntryProps, context) => {
  const [amt, setAmt] = useLocalState<number>(
    context,
    props.recipe.ref,
    ...
    <Stack>
      <Stack.Item grow={1}>
        <Button.Confirm
          ...
          disabled={
            props.recipe.cost * (amt / props.recipe.resultAmt) > props.stackAmt
          }
        />
      </Stack.Item>
            ...
            <Button
              ...
              onClick={() =>
                setAmt(
                  Math.max(
                    Math.min(
                      amt + props.recipe.resultAmt,
                      (props.stackAmt / props.recipe.cost) *
                        props.recipe.resultAmt,
                    ),
                    props.recipe.resultAmt,
                  ),
...
export const StackCrafting = (props, context) => {
  ...
  return (
    <Window
      width={425}
      height={Math.max(300, 30 + approximateEntries * 30)}
      title={`${data.name}`}
    >
      <Window.Content>
        <Section
        ...
        >
          <Stack vertical>
            {searchText && searchText.length >= 2 ? (
              <>
                {data.recipes
                  .filter((r) => r.name.toLowerCase().includes(searchString))
                  ...
                  .map((r) => (
                    <Stack.Item key={r.ref}>
                      <StackCraftingEntry
                        recipe={r}
                        craft={(ref, amt) =>
                          act('craft', { recipe: ref, amount: amt })
                        }
                        stackName={data.name}
                        stackAmt={data.amount}
                      />
                    </Stack.Item>
                  ))}
              </>
