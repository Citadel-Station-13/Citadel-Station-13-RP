...
export const TGUICardMod = (props: CardModProps, context) => {
  ...
  const [department, setDepartment] = useLocalState<string | null>(
    context,
    'dept',
    null,
  );
  ...
  const modifiable: Array<Access> = data.access.filter(
    (a) =>
      (a.region & data.modify_region && a.type & data.modify_type) ||
      ...
      {mode === 1 && (
        ...
        >
          <LabeledList>
            ...
            <LabeledList.Item label="Assignment / Title">
              {data.can_rank ? (
                <Input
