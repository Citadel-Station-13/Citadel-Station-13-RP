        ...
        <svg background-size="200px" width="200" height="400">
          ...
          <g transform="translate(0, 0) scale(0.8 0.8)">
            {reagentPeaks.map((peak) => (
              ...
              <polygon
                key={peak.name}
                points={`${((peak.mass - 10) / graphUpperRange) * 500},265 ${(peak.mass / graphUpperRange) * 500},${250 - (peak.volume / maxAbsorbance) * 250} ${((peak.mass + 10) / graphUpperRange) * 500},265 `}
                opacity="0.6"
                style={`fill:${peak.color}`}
              />
            ))}
            <polygon
              points={`${(lowerRange / deltaRange) * 500},265 ${(lowerRange / deltaRange) * 500},0 ${(upperRange / deltaRange) * 500},0 ${(upperRange / deltaRange) * 500},265`}
              opacity="0.2"
              style={`fill:blue`}
            />
            <line
