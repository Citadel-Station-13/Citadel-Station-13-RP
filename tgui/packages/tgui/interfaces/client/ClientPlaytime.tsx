...
export const ClientPlaytime = (props, context) => {
  ...
  if (data.playtime === null) {
    return (
      <Window width={400} height={200} title="Playtime Viewer">
        <NoticeBox warning>
          Something went wrong while loading your playtime. Is the database
          connected?
        </NoticeBox>
      </Window>
    );
  ...
  let highest: number = rawPlaytimeNumbers.length
    ? rawPlaytimeNumbers.reduce((a, b) => Math.max(a, b))
    : 1;
  let sorted = Object.keys(data.playtime).toSorted(
    (a, b) => itIsntNullDamnit[b] - itIsntNullDamnit[a],
  );
          ...
          <LabeledList>
            {sorted.map((key) => (
              <LabeledList.Item key={key} label={key}>
                <ProgressBar
                  value={itIsntNullDamnit[key]}
                  maxValue={highest}
                >{`${round(itIsntNullDamnit[key] / 60, 0.1)}h`}</ProgressBar>
              </LabeledList.Item>
            ))}
          </LabeledList>
        </Section>
