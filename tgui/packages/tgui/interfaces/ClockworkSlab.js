...
export const ClockworkSlab = (props, context) => {
  ...
  const [tab, setTab] = useSharedState(context, 'tab', 'Application');
  const scriptInTab = (scripture && scripture[tab]) || [];
  const tierInfo = (tier_infos && tier_infos[tab]) || {};

  return (
    <Window theme="clockcult" width={800} height={420}>
          ...
          <Section
            ...
            <Section level={2}>
              ...
              <Box as={'span'} textColor={'#DAAA18'}>
                Scriptures in <b>yellow</b> are related to construction and
                building.
              </Box>
              <br />
              <Box as={'span'} textColor={'#6E001A'}>
                Scriptures in <b>red</b> are related to attacking and offense.
              </Box>
              <br />
              <Box as={'span'} textColor={'#1E8CE1'}>
                Scriptures in <b>blue</b> are related to healing and defense.
              </Box>
              <br />
              <Box as={'span'} textColor={'#AF0AAF'}>
                Scriptures in <b>purple</b> are niche but still important!
              </Box>
              <br />
...
export const CSScripture = (props, context) => {
  const { act, data } = useBackend(context);
  const { power_unformatted = 0 } = data;
  const { scriptInTab } = props;

  return scriptInTab?.map((script) => (
    <Table.Row key={script.name} className="candystripe">
      <Table.Cell italic={!!script.important} color={script.fontcolor}>
        <b>{script.name}</b>
        {`
              ${script.descname}
              ${script.invokers || ''}
...
export const CSTutorial = (props, context) => {
  ...
  return (
    <Section
      ...
      <Box>
        {HONOR_RATVAR ? (
          ...
          <Fragment>
            ...
            <NoticeBox warning>
              NOTICE: This information is out of date. Read the Ark &amp; You
              primer in your backpack or read the wiki page for current info.
            </NoticeBox>
            These pages serve as the archives of Ratvar, the Clockwork
            Justiciar. This section of your slab has information on being as a
            Servant, advice for what to do next, and pointers for serving the
            master well. You should recommended that you check this area for
            help if you get stuck or need guidance on what to do next.
            <br /> <br />
            <NoticeBox info>
              Disclaimer: Many objects, terms, and phrases, such as Servant,
              Cache, and Slab, are capitalized like proper nouns. This is a
              quirk of the Ratvarian language do not let it confuse you! You are
              free to use the names in pronoun form when speaking in normal
              languages.
            </NoticeBox>
          </Fragment>
        )}
      ...
      <Divider />
      <Box>
        <Box as={'span'} textColor={'#BE8700'} fontSize={2.3}>
          {rec_section?.title
            ? rec_section.title
      ...
      <Box>
        ...
        <br />
        <Box as={'span'} italic>
          You can have up to five scriptures bound to action buttons for easy
          use.
        </Box>
        <br />
        <br />
        {rec_binds?.map((bind) => (
          <Fragment key={bind.name ? bind.name : 'none'}>
            A <b>Quickbind</b> slot ({rec_binds.indexOf(bind) + 1}), currently
            set to&nbsp;
