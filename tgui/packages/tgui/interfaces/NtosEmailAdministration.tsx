...
const MainMenu = (props) => {
  ...
  return (
    <Section title="Welcome to the NTNet Email Administration System">
      <Box italic mb={1}>SECURE SYSTEM - Have your identification ready</Box>
      <Button
        fluid
        icon="plus"
        onClick={() => act("newaccount")}>
        Create New Account
      </Button>
      <Box bold mt={1} mb={1}>Select account to administrate</Box>
...
const EmailError = (props) => {
  ...
  return (
    <Section title="Message" buttons={<Button icon="undo" onClick={() => act("back")}>Back</Button>}>
      {error}
    </Section>
  );
};
...
const ViewAccount = (props) => {
  ...
  return (
    <Section title={"Viewing " + current_account + " in admin mode"} buttons={
      <Button
        icon="undo"
        onClick={() => act("back")}>
        Back
